---
kind: intervention
intervention_id: lean-plan-files
pipeline: feature
provenance: gated
...
# Intervention: lean-plan-files

Hypothesis: shipping `lean-plan-files` (feature pipeline) moves `undeclared` in direction `undeclared` within 20 post-ship runs.
