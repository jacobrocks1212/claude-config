---
kind: intervention
intervention_id: spec-excerpt-scoped-plans
pipeline: feature
provenance: gated
...
# Intervention: spec-excerpt-scoped-plans

Hypothesis: shipping `spec-excerpt-scoped-plans` (feature pipeline) moves `undeclared` in direction `undeclared` within 20 post-ship runs.
