...
signal_independence: mixed — the wedge-backstop 'block' (containment-refusal) events
  are hook-emitted, but the fix's effect (nested WU workers no longer force-spun on
  execute-plan cycles that use the workstation sub-subagent split) is an independent
  behavioral change observable as fewer block events in the ledger
baseline:
  status: not-computable
  reason: recorded out-of-band by a dispatched harden subagent under a CONCURRENT
    non-harden cycle (decision-2-6-uncovered-row-reroute-to-mcp-test) — --record-intervention
    was cycle-refused (refuse_if_cycle_active; the allow_hardening_subagent carve-out
    did not apply because the active cycle was not a hardening class), so the telemetry
    baseline window could not be frozen. Re-baseline via the orchestrator handback
    (see hardening-log Round 109 Return).
