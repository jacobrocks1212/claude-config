...
**Implementation Notes (2026-06-09, Phase 5 Batch 2 — WU-2/3/6/7/8):**
- **WU-2 (5b):** Authored `agents/cognito-intra-file-consistency.md` (153 lines, net-new). Frontmatter `name: cognito-intra-file-consistency`, `model: opus`, `color: teal`. `Read`s BOTH `~/.claude/skills/_components/reuse-discovery-protocol.md` and `pr-review-reuse-agent-scaffold.md` at runtime. Three explicit scaffold overrides: output prefix `intrafile-{cluster-slug}.json`; baseline = the host file's own `main` version (explicitly NOT `manifest.baselines[]`); extra verdicts `inconsistent`(→nit)/`consistent`(→dropped-with-trail). Two-axis workflow: (i) intra-file duplication via `get_file_structure`/`find_symbol_usages` to locate the in-file member + `get_callers` for `refactor` blast radius; (ii) surrounding-code consistency emitting `inconsistent`/`consistent`. `consistent` (like `acceptable-new`) must carry a non-empty `negative_search_trail` (R4 gate).
- **WU-3 (5c):** `review-pr.md` Step 5b gained a second clustering pass concurrent with the reuse pass — selects all substantively-modified substantive files (all tiers; excludes test/config/generated; `manifest.baselines[]` NOT required), ≤6 clusters, fans out one `cognito-pr-review:cognito-intra-file-consistency` per cluster in parallel with Step 5, writing `intrafile-{cluster-slug}.json`. Explicitly notes prep (`prep-pr.ts`) is NOT modified. Updated the line-15 one-liner + the Architecture ASCII diagram (new stage on the same concurrent brace).
- **WU-6 (5f):** `synthesizer-v2.md` gained an omittable `## Intra-File Consistency` section keyed on `source "intrafile"`, placed between `## Reuse & Duplication` and `## Re-Review Status` (verified ordering 116<128<140). Renders `**{verdict}** — [{file}:{line}] → in-file candidate: {candidate}` under `### Important`/`### Minor`. Narrative guideline + omission rule added.
- **WU-7 (5g):** `review-pr-buddy.md` Surface-Findings enumeration gained an `**Intra-file reuse & consistency** (source:"intrafile")` bullet after the reuse bullet; Phase 2 curated-doc note names `## Intra-File Consistency` + the matching omission rule.
- **WU-8 (5h):** Two rules appended to `code-consistency.yaml` — `intrafile-block-duplication` (important) and `intrafile-convention-divergence` (minor), both FLAG+ESCALATE to the **intra-file** consistency stage (never assert a confirmed duplication). Matching `rule_weights` (0.7, data_points 0) added to `weights.yaml`. `/rebuild-agents` (manual, Phase 2 surgical-insertion precedent) embedded both into `agents/sweep.md` under `### Code Consistency Rules` between the `RULES_START`(64)/`RULES_END`(2086) markers (rules at lines 1970/1975, `**Effective:** 0.56`). Only `sweep.md` touched among agents by the rebuild.
- **Optional doc-hygiene polish (from review):** `review-pr-buddy.md:208` `Sections:` enumeration gained `Intra-File Consistency` (positioned to mirror the synthesizer template); `review-pr.md:374` wording tightened from `acceptable` to `acceptable-new` to match WU-2's standardized verdict name (post-process already drops all three).
- **Gates (all green, orchestrator-reverified):** `npx tsc --noEmit` over `scripts/` clean (Batch 2 changed no `.ts`; confirmed no regression from Batch 1); both `code-consistency.yaml` + `weights.yaml` parse; sweep rules confirmed inside the RULES markers; `git status` shows exactly the 7 Batch-2 plugin files; subagent dispatch name `cognito-pr-review:cognito-intra-file-consistency` matches the agent frontmatter `name`.
- **Review verdict:** PASS (two optional non-blocking polish items surfaced were applied; ground-truth verified `yes` for WU-2/3/6/7/8; full end-to-end string-trace confirmed every machine identifier is the unhyphenated `intrafile` with no `intra-file` drift in load-bearing keys).
- **Runtime verification DEFERRED (manual/live):** a live `review-pr`/`review-pr-buddy` run on a real PR exercising the Opus intra-file agent (real `intrafile-*.json`, parallel agent-output timestamps, a real `negative_search_trail`, a real sweep escalation, and the rendered `## Intra-File Consistency` section) is the Runtime Verification checklist above — left unchecked pending a live run. All mechanical equivalents are green.
