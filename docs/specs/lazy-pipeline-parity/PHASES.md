...
- [ ] **Extend `user/scripts/test_lazy_parity.py`**: add a live zero-drift assertion for each of the four new pairs (`audit_pair(repo_root, pair_name=<name>)` asserts zero drift). The suite now covers all five pairs.
...
**Verification** *(pytest — run by the implementer):*
- [ ] `pytest user/scripts/test_lazy_parity.py -q` — all tests green (fixtures + all five live pairs).
- [ ] `python3 user/scripts/lazy_parity_audit.py --repo-root .` — exits 0 (zero drift across all five pairs).
...
- [ ] `pytest user/scripts/` — full suite green (all prior phase tests remain passing; no regressions).
...
- [ ] If `--check-parity` was implemented: `python3 user/scripts/lint-skills.py --check-parity` — exits 0.
