...
#### Implementation Notes (Phase 1 — 2026-06-13)

**Review verdict:** Complete. Both F1a and F1b shipped test-first under all standing gates.

**F1b decision: SHIPPED (the spec's default recommendation).** No concrete blocker surfaced. Scoped exactly as specified:
- **cycle-class only** — `find_auto_readmit_entry` (lazy_core) matches `class == "cycle"` and nothing else; the guard re-asserts the exclusion at its boundary (defence-in-depth). A hardening-class suffix NEVER auto-readmits — the depth-1 cap (`_hardening_cap_deny_reason`) is untouched and verified unchanged by a test.
- **pure-suffix only** — `dispatched_norm.startswith(entry_norm)` with a non-empty remainder, after identical `normalize_prompt_for_hash` normalization. An exact match would have hit `lookup_emission`; an in-body edit is not a prefix, so it never qualifies and still denies with the F1a reason.
- **audited** — every readmit writes an `auto_readmit: true` line to the deny ledger (same JSONL stream as denies, distinguished by the flag; pre-`acked: true` so it owes no hardening debt and never inflates `pending_hardening()` / blocks `--run-end`). Never silent.
- **fail-open** — the whole auto-readmit path (`_try_auto_readmit`) is wrapped so any error (including a `consume_nonce` TOCTOU race or a poisoned consume) falls through to the NORMAL deny, never a spurious allow (verified by a test that poisons `consume_nonce`).

**Integrity tradeoff acknowledged:** a trailing suffix is read LAST by the subagent and can override the verbatim prompt's tail clauses — this softens turn-routing's "hand-composed prompts are unexecutable" guarantee. Mitigated by the three-way scoping above (cycle-class-only + pure-suffix-only + audited) and the next-marked-run live-verification row. The tradeoff was judged acceptable per the spec's default.

**Default-invisibility:** the new `prompt_norm` registry field is written only under a marker (via `register_emission` reached through `register_emission_if_marked`), and the auto-readmit branch only fires on a marked run with a registry hit — so both `--test` smokes stayed byte-identical (no baseline regen).

**Fresh gate counts:** `test_lazy_core.py` 311/311 (was 304; +7 F1 tests); `test_hooks.py` 24/24 (was 23; +1 bash pipe-test); `lazy-state.py --test` PASS (byte-identical); `bug-state.py --test` PASS (byte-identical); `lint-skills.py --check-projected --check-capabilities` clean.

**Files touched:** `user/scripts/lazy_guard.py` (F1a `_CORRECTIVE_RECIPE` wording + `_try_auto_readmit` branch), `user/scripts/lazy_core.py` (F1b `prompt_norm` registry field + `append_auto_readmit_event` + `find_auto_readmit_entry`), `user/scripts/test_lazy_core.py` (+7 tests), `user/scripts/test_hooks.py` (+1 bash pipe-test).

**Deviations:** none. The `auto_readmit` event uses the deny-ledger JSONL stream (the spec's "or a sibling telemetry line" — the same-stream option was chosen so retro graders read one stream, per the Phase 1 Integration Notes).
...
#### Implementation Notes (Phase 2 — 2026-06-13)

**Review verdict:** Complete. All three deliverables shipped test-first under all standing gates.

**Consume-oracle mechanism + new signature-file key.** The "did a dispatch occur between two probes" oracle is the registry **consumed-emission count** — the validate-deny guard calls `consume_nonce` on every ALLOW (one consume per dispatch, A5). A new `lazy_core.consumed_emission_count()` helper reads the prompt registry (read-only, `create=False`) and returns the number of entries with `consumed: True`. `update_repeat_counts` reads it twice around a re-read and persists it in the SAME per-pipeline signature file under a new additive key **`consume_count`**. When (a) a run marker is present, (b) the step signature is unchanged, AND (c) the prior file recorded a `consume_count` equal to the current consumed-count → no dispatch landed between the probes → the HEAD-blind `step_count` is **HELD** (not incremented). A genuine oscillation still trips because a real dispatch (hence a consume) lands between its repeats, raising the count.

**Marker-gating + default-invisibility.** `consume_count` is written ONLY on a marked probe (`read_run_marker()` non-None). On the no-marker path the key is never written and the debounce is fully inert, so the persisted record shape and behavior stay byte-identical to pre-Phase-2 — both `--test` smokes confirmed byte-identical (the only lazy-smoke diff is the per-run random tempdir name, which changes between two consecutive runs of the SAME code; the bug smoke is exactly identical). Baselines were NOT regenerated.

**HEAD-blindness preserved.** No HEAD reset was added to `step_count`; the debounce keys strictly on dispatch occurrence (consume-count delta), never on commits. Legacy tolerance mirrors the `head`/`step_*` migrations: a file with no `consume_count` key cannot prove a re-read, so `step_count` increments as before and the key is added on the next marked write. `peek=True` never persists (the consume key is not written under peek) and never advances.

**Fresh gate counts:** `test_lazy_core.py` 316/316 (was 311; +5 Phase-2 debounce tests); `test_hooks.py` 24/24; `lazy-state.py --test` PASS (byte-identical); `bug-state.py --test` PASS (byte-identical); `lint-skills.py --check-projected --check-capabilities` clean.

**SKILLs mirrored:** the Step 1a "probe ONCE per cycle (F2 double-probe debounce)" note is byte-identical across all three coupled skills (`lazy-batch`, `lazy-bug-batch`, `lazy-batch-cloud` — verified by matching md5).

**Files touched:** `user/scripts/lazy_core.py` (`consumed_emission_count` helper + `update_repeat_counts` debounce/`consume_count` key + docstring), `user/scripts/test_lazy_core.py` (+5 tests + `_record_consume`/`_write_marker_in` helpers), `user/skills/lazy-batch/SKILL.md`, `user/skills/lazy-bug-batch/SKILL.md`, `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md` (mirrored Step 1a note).

**Deviations:** none. The signature-file schema gained exactly one additive key (`consume_count`), documented in the `update_repeat_counts` docstring alongside the `head`/`step_*` migration notes per the Integration Notes for Next Phase.
