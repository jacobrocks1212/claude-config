...
- Plan-scoped `deliverables_done` slices `### Phase N` sections by the plan's `phases:` set — plans without `phases:` get feature-level semantics deliberately.
  - ⏩ **Forward pointer (2026-06-15, commit pending — see `docs/specs/lazy-validation-readiness/PHASES.md` §"`deliverables_done` → plan-WU source of truth").** This PHASES-phase-level read of plan-scoped `deliverables_done` was SUPERSEDED. Plan-scoped `deliverables_done` now reads the **plan part's own `- [ ] WU-N` checkboxes** (machine source of truth since write-plan ISSUE-6), NOT the PHASES.md `### Phase N` slices — eliminating the cross-part and cross-phase-attribution false-fails the phase-slice read suffered (d8-effect-chains review). The phase-slice path (`_phases_text_scoped_to` + `_unchecked_wus_in_plan_scope`) is RETAINED only as the legacy fallback for pre-ISSUE-6 plans with no `- [ ] WU-N` rows (verdict reports `deliverables_source: "phases-fallback …"`).
...
#### Implementation Notes (Phase 10 — three parallel agents: WU-2+3, WU-1+5, WU-4)
**Completed:** 2026-06-11
**Review verdict:** PASS (orchestrator re-ran all gates fresh in BOTH repos — claude-config 214/214 + both --test baselines unregenerated + lint OK; AlgoBooth `check-docs-consistency` rc=0 repo-wide, 16/16 checker vitest; addenda proven end-to-end through the AlgoBooth `.claude/skill-config` symlink with a synthetic execute-plan emission — `INVARIANTS.md`/`load_full` present, zero residue; live peek emission confirmed `step_repeat_count` emitted and write-plan correctly receives NO audio addenda)
**Work completed:**
- **WU-2 (TDD, +7):** `lazy_core.update_repeat_counts()` computes BOTH counters in one read/write of the shared per-pipeline file — `step_repeat_count` keyed on `(feature_id, current_step)` with NO head-advance reset (the discriminator test proves a commit between identical-step probes resets `repeat_count` but NOT `step_count`); legacy-file fallback; `update_repeat_count` kept as a compatible wrapper. Emitted under `--repeat-count`/`--repeat-count-peek` (peek non-mutating for both). T6 warning at ≥3 in all three batch SKILLs.
- **WU-3 (TDD, +6):** `emit_cycle_prompt` reads optional `<repo_root>/.claude/skill-config/cycle-prompt-addenda.md` via the EXISTING template parser (no duplicated grammar); matching sections append after base / before loop block; residue in an addenda section refuses naming the addenda file (isolation re-bind attributes the source). Absent-file byte-identity pinned by test. AlgoBooth addenda seeded with the audio-INVARIANTS section (`skills=execute-plan,retro-feature,mcp-test`); never-hand-append prose in lazy-batch Step 1d + lazy-dispatch-template.
- **WU-1+5 (prose, additive-only, anchors preserved):** gate-owned row ban + reachability-smoke rule canonically in `phases-runtime-verification.md`, surfaced in spec-phases (authoring site) + add-phase (corrective-phase author, smoke rule cross-referenced to its Step 3.5) + write-plan (which also now SKIPS/flags gate-owned rows it encounters instead of planning work for them).
- **WU-4 (AlgoBooth `2e315e4e`, TDD 16/16):** rule `phase.deliverables.gate-owned-action` (per-row `deliverableRows` added to the PHASES parser); pre-sweep evidence: **5 corpus hits** (the 3 investigation rows PLUS `d2-choke-groups:345` + `d8-track-pattern-interaction:787`, both historical `[x]` rows — D7: swept all five rather than weakening the rule); `clap-target-export:341` + "mark this box"/"complete the form" negatives NOT flagged; archive pattern anchored to the literal `/_archive` token after a repo-wide scan excluded one false positive. Atomic explicit-path commit, clean fast-forward push, live session had nothing in flight.
**Live-run convergence note:** the live `/lazy-batch` session independently committed an identical-text fix for the d8-live-looping row (`64e7f1e3`) minutes before WU-4's commit — the agents converged on the same rewording; WU-4's commit landed the rule + the other four rows.
**Pitfalls & guidance:**
- The addenda file is parsed with the SAME grammar/contract as the base template — addenda edits are subject to the same residue refusal; keep them token-free or use the 14 documented tokens.
- `step_repeat_count` deliberately ignores HEAD advance — do NOT "fix" that symmetry with `repeat_count`; oscillation-with-commits is exactly what it exists to see.
- The gate-owned lint runs only on PHASES.md phase sections (plan files are out of scope by checker design) — the authoring ban in write-plan covers the plan-file side.
**Carried follow-ups (future candidates, NOT blocking):**
- State-script refusal to emit the same routing a 4th time (escalate the T6 warning to a mechanical halt) once `step_repeat_count` proves reliable in live runs.
- Opportunistic workstation scheduling of deferred reachability-smoke rows (state-script change, deliberately out of scope here).
- **Circuit-breaker horizon:** the next corrective phase fires the +50% breaker — route further hardening through `/realign-spec` + a fresh decomposition.
**Files modified:** claude-config `17fca2e` (13 files: scripts ×4, batch SKILLs ×3, dispatch-template, phases-runtime-verification, spec-phases/write-plan/add-phase SKILLs, NEW algobooth cycle-prompt-addenda.md); AlgoBooth `2e315e4e` (checker + tests + 4 PHASES sweeps).
...
#### Implementation Notes (Phase 11 — three subagents + orchestrator prose; breaker-overridden phase)
**Completed:** 2026-06-11
**Review verdict:** PASS (orchestrator re-ran all gates fresh after all agents landed: `test_lazy_core.py` 241/241 (was 214 at phase start), both `--test` smoke suites exit 0 with baselines UNREGENERATED, `lint-skills.py` OK; AlgoBooth checker vitest 33/33 + `qg:docs-consistency` rc=0 repo-wide verified by the implementing agents)
**Work completed:**
- **WU-1a (TDD):** `lazy_core.validation_escalation(meta)` predicate + shared `VALIDATION_ESCALATION_SUFFIX`; both Step-3 blocked terminals (lazy-state ~L1299, bug-state ~L758) add `validation_escalation: true` + the notify suffix ONLY when `blocker_kind == "mcp-validation"` and `retry_count >= 2` (string-digit tolerant, YAML-bool rejected, missing fields → no escalation). Non-escalated output byte-identical (existing fixtures all carry retry_count 0).
- **WU-1b/c/d (prose):** escalation step 1a + apply-prompt ESCALATION clause in `blocked-resolution.md`; same rule in `halt-resolution.md`'s blocked row (single-dispatch wrappers); `## Seam Enumeration` contract (per-seam `probed-OK | probed-FAIL | unprobed`) at the BLOCKED-authoring loci — mcp-test SKILL + cycle-base-prompt skill-mcp-test-common (stays within R14; `{item_label}` is a bound token, no new tokens); seam-audit HARD requirement in add-phase Step 4 authoring notes.
- **WU-2 + WU-4c (prose):** SPEC-example capability audit (enumerate constructs consumed by SPEC code examples + mandatory negative-evidence grep + ledger row per construct; explicitly-rejected capability = planning-time halt) canonical in `phases-runtime-validation.md`, surfaced in spec-phases Step 2.7; anti-pattern sharpened — a spike executed as a static trace VIOLATES the gate (d8 WU-9.0 named).
- **WU-3 (prose):** terminal-MCP-stacking ban canonical in `phases-runtime-verification.md` (>2 consecutive N/A phases + single terminal integration phase = red flag → vertical tracer-bullet smoke at the earliest phase where the chain exists), surfaced as a spec-phases authoring-discipline bullet.
- **WU-4a/b (prose):** runtime-spike evidence rule canonical in `phases-update.md` step 1 (runtime/spike rows tick ONLY with a cited runtime artifact; a static trace never closes them; impossible-this-cycle → unticked + `NEEDS_RUNTIME:` note), reinforced in execute-plan's completion-tick bullet; write-plan spike WUs must state their closing artifact.
- **WU-4d (AlgoBooth, TDD 33/33):** warning rule `phases.validated-assumptions.static-confirmation` — flags a Validated-Assumptions row ONLY when column 2 matches static (`code[- ]read|\bstatic\b`) AND lacks every accepted token (`grep|runtime|spike|code-provable`, word-boundary). The initial broad version flagged 12 corpus rows (mostly `runtime + code-read` combos); a precision follow-up dropped it to 2 honest pure-code-read rows in d8-live-looping (NOT edited — the live session owns that file). AlgoBooth commits `321441aa` + `8b63c017`, both pushed.
- **WU-5 (TDD + lockstep):** `phase_count_at_retro` authored by retro Step 6c (new sub-step 2b), documented in `sentinel-frontmatter.md`, mirrored in AlgoBooth `SENTINEL_SCHEMAS` (optional + intFields — no unknown-keys warning); `lazy_core.retro_staleness()` single shared helper; Step-8 stale re-route in BOTH lazy-state AND bug-state (`"... (stale — N phases added since retro)"`); `apply_pseudo` backstop refusal for BOTH `__mark_complete__` and `__mark_fixed__` (zero writes, positioned AFTER the receipt-noop / BEFORE the coherence gate — staleness must refuse before the coherence auto-flip writes). Grandfathering: field-less RETRO_DONE keeps exact current behavior (pinned by baselines + dedicated tests). `retro-feature/SKILL.md` needed NO edit (it only checks sentinel existence; `/retro` authors it).
**Scope correction caught at implementation time:** the drafted WU-5 claimed "bugs have no retro step" — wrong (`bug-state.py` has STEP_RETRO). Extended to full bug-pipeline parity (WU-5e) in a follow-up agent; the locking test was rewritten intent-preservingly (`test_apply_pseudo_mark_fixed_refuses_stale_retro_zero_writes`).
**Pitfalls & guidance:**
- The staleness backstop sits BEFORE the coherence gate deliberately (the coherence auto-flip WRITES PHASES.md; a stale-retro refusal must be zero-writes). Do not reorder.
- The escalation payload key is added post-hoc to the state dict ONLY in the escalation case — the byte-identity invariant for non-escalated output depends on it.
- `ACCEPTED_EVIDENCE_RE` has a known looseness: a pure-code-read row whose justification text contains the word "runtime" (e.g. "NOT runtime-coupled") passes via the negation text. Accepted for an advisory rule.
- e2e fixtures for the new routing live in `test_lazy_core.py` via the importlib `_load_state_script` pattern — NOT as `--test` smoke fixtures (those would shift the byte-pinned baselines).
- PowerShell 5.1 `Get-Content -Raw`/`Set-Content` round-trips CORRUPT UTF-8-no-BOM files (ANSI decode → mojibake on every em-dash). This file was corrupted once during this phase and restored from git — use the Edit tool for doc edits, never PS string surgery.
**Carried follow-ups (future candidates, NOT blocking):**
- Mechanical seam-audit enforcement: the state script could require a seam-audit attestation in the corrective phase before routing write-plan when `validation_escalation` was flagged.
- The 2 remaining d8-live-looping static-confirmation warnings clear when that feature's ledgers are next touched (live session owns the file).
- Negation-aware `ACCEPTED_EVIDENCE_RE` (exclude "runtime-coupled"/"no runtime smell" mentions from accepted evidence).
**Files modified:** claude-config (this commit): `user/scripts/{lazy_core,lazy-state,bug-state,test_lazy_core}.py`, `user/skills/_components/{blocked-resolution,halt-resolution,phases-runtime-validation,phases-runtime-verification,phases-update,sentinel-frontmatter}.md`, `user/skills/_components/lazy-batch-prompts/cycle-base-prompt.md`, `user/skills/{spec-phases,write-plan,execute-plan,add-phase,retro}/SKILL.md`, `repos/algobooth/.claude/skills/mcp-test/SKILL.md`, this PHASES.md. AlgoBooth: `321441aa`, `8b63c017` (checker + tests, pushed).
