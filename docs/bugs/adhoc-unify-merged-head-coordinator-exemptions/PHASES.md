...
**Deliverables:**
- [ ] `lazy-state.py` guard prologue rewritten to a single `_emit_exempt_reason = lazy_core.dispatch.coordinator_arbitrated_emission(_emit_marker, state.get("feature_id"), lazy_core.claude_state_dir() / "leases.json")`; the guard becomes `if _emit_marker is not None and _emit_exempt_reason is None:`; the two `elif _emit_is_lane` / `elif _emit_is_lease_held` observability branches become a single `elif _emit_exempt_reason is not None:` that selects the existing per-reason diag text (lane vs. lease) — preserving both messages' content.
- [ ] `bug-state.py` given the byte-parallel identical replacement (coupled-pair mirror), including its mirror comments updated to point at the shared predicate as the single home.
- [ ] The two mirror-comment blocks and the `lazy_coord` import annotations (`lazy-state.py:77-82`, `bug-state.py:97-100`) updated to reflect that the exemptions now live in one shared predicate (the import annotation still holds — `has_live_lease` is still consumed, now transitively via the predicate; keep or update the note accordingly).
- [ ] `lazy_parity_audit.py --repo-root .` re-run to exit 0; if the parity manifest (`user/scripts/lazy-parity-manifest.json`) pins the content of the changed mirrored blocks, reconcile it to the unified form so the pair stays audited-parallel.
- [ ] Tests: the Phase-1 predicate unit tests continue GREEN; add (or extend) a `test_dispatch.py` assertion that the caller-facing reason→diagnostic mapping covers `"lane"` and `"lease"` (guarding the diag-preservation contract). No new baseline rows.
