...
**Deliverables:**
- [ ] `test_hooks.py` fixture: subagent-shaped PreToolUse payload (`agent_id` present) + recursive `Agent` call → asserts `permissionDecision: deny`; main-thread-shaped payload (`agent_id` absent) + same call → asserts allow (no output). This fixture pins the confirmed field name. [VERIFY: `ls user/scripts/test_hooks.py`]
- [ ] `user/hooks/lazy-cycle-containment.sh` — re-target the recursion/lifecycle deny logic on `agent_id` presence: deny `Agent`/`Task` dispatch, `_STATE_PY_RE` + `LOOP_FORMATION_FLAGS` routing, `/lazy-batch` invocation, and `LIFECYCLE_PATTERNS` (`dev:kill`/`dev:restart`) **when `payload.get("agent_id")` is present**; allow all of them when absent. Preserve the marker-gated 2nd-feature tripwire + commit-ceiling backstop (`feature_id`/`commit_tally` still read from the marker). Keep the fail-OPEN-via-empty-output contract and the bash fast-path (now: fast-allow when `agent_id` absent AND marker absent). [VERIFY: `grep -n "tool_name == \"Agent\"" user/hooks/lazy-cycle-containment.sh`]
- [ ] `user/scripts/lazy_core.py` — `refuse_if_cycle_active()` (`:5175`) gains an `agent_id`-aware path: refuse the `CYCLE_REFUSED_OPS` when the caller is a subagent (the script learns this from an env var the hook/dispatch sets, or from the marker as the fallback carrier — pick the mechanism the fixture confirms is available to a Python subprocess). The orchestrator (main-thread) is never refused. Keep `CYCLE_REFUSED_OPS` and the hook deny-set in documented lockstep. [VERIFY: `grep -n "def refuse_if_cycle_active" user/scripts/lazy_core.py`]
- [ ] Update the lockstep comment block above `CYCLE_REFUSED_OPS` (`lazy_core.py:5159`) and the hook header comment to describe the `agent_id` trip and the retained marker-gated tripwires, so the two stay auditable. [VERIFY: `grep -n "CYCLE_REFUSED_OPS MUST stay in lockstep" user/scripts/lazy_core.py`]
- [ ] Tests: `test_hooks.py` covers deny-when-subagent / allow-when-main-thread for each op class (recursive Agent, routing flag, lifecycle pattern). `test_lazy_core.py` covers `refuse_if_cycle_active` allow-main-thread / refuse-subagent. Update `lazy-state.py --test` / `bug-state.py --test` baselines only if the refusal output text changed.
...
**Deliverables:**
- [ ] `user/scripts/lazy-state.py` — `--cycle-begin` handler (`:5261`) snapshots run identity (`read_run_marker().started_at` [VERIFY: `grep -n "def read_run_marker" user/scripts/lazy_core.py`]) and the current HEAD SHA into the cycle marker. Additive fields; absent run marker → snapshot null run-identity (degrade gracefully, no crash). Mirror the same additive snapshot in `bug-state.py`'s `--cycle-begin`.
- [ ] `user/scripts/lazy_core.py` — extend `write_cycle_marker()` (`:5018`) signature/body to persist the new `run_started_at` + `begin_head_sha` fields (additive; default null so existing callers/fixtures are unbroken). [VERIFY: `grep -n "def write_cycle_marker" user/scripts/lazy_core.py`]
- [ ] `user/scripts/lazy_core.py` — new `detect_cycle_bracket_friction(marker, current_run_started_at, current_head_sha, sub_skill, *, now=None)` returning a friction descriptor (or None) implementing D1(a) cycle-bracket-break and D1(b) unexpected-commits. The expected-commit budget per `sub_skill` is conservative (most sub-skills commit 0–1 times; a torn bracket OR commits-beyond-budget trips it). Pure function, fully unit-testable.
- [ ] `user/scripts/lazy_core.py` — new `append_friction_ledger_entry(reason_head, detail, now=None)` that appends to the SAME `lazy-deny-ledger.jsonl` with a `kind: "process-friction"` discriminator and `acked: false`, so `pending_hardening()`/`oldest_unacked_deny()`/`--run-end` gate consume it unchanged. [VERIFY: `grep -n "def append_deny_ledger_entry" user/scripts/lazy_core.py`]
- [ ] `user/scripts/lazy-state.py` (+ `bug-state.py`) — `--cycle-end` handler (`:5270`) calls `detect_cycle_bracket_friction(...)` BEFORE clearing the marker; on a non-None descriptor it calls `append_friction_ledger_entry(...)` then clears the marker as usual. `--cycle-end` stays idempotent and never crashes on a missing/partial marker.
- [ ] `user/scripts/lazy_core.py` — `build_hardening_emit_command()` (`:6332`) / `oldest_unacked_deny()` handle a `process-friction` entry: bind `trigger_kind=process-friction` and surface the friction detail in the `--context` summary instead of `denied_prompt_summary`/`denial_reason`. [VERIFY: `grep -n "def build_hardening_emit_command" user/scripts/lazy_core.py`]
- [ ] Tests: `test_lazy_core.py` — `detect_cycle_bracket_friction` fixtures (clean bracket → None; torn bracket → descriptor; over-budget commits → descriptor; degraded null-run-identity → None, no crash); `append_friction_ledger_entry` round-trips into the deny ledger and bumps `pending_hardening()`; `build_hardening_emit_command` emits `trigger_kind=process-friction`. `lazy-state.py --test` / `bug-state.py --test` cycle-marker fixtures updated for the additive fields + a torn-bracket → ledger-entry path; regenerate both baselines via `_normalize_smoke_output`.
