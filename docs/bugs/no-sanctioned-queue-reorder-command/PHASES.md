...
**Deliverables:**
- [x] `_components/blocked-resolution.md` — the "Defer this {ITEM}; continue the rest of the queue" path (lines ~147–152) replaces the hand-`Edit {SPEC_ROOT}/queue.json` instruction with an inline `{STATE_SCRIPT} --reorder-queue --id <id> --to tail` Bash call. Keep the "LEAVE BLOCKED.md IN PLACE / do NOT neutralize" semantics (the defer keeps the item blocked, just off the queue head). One edit; the `{STATE_SCRIPT}` token covers both pipelines.
- [x] `user/skills/lazy-batch/SKILL.md` HARD CONSTRAINT 1 (line 22) — update the "queue reorder" reference: an operator-directed reorder is now a deterministic `--reorder-queue` Bash call (out-of-cycle), NOT a dispatched Opus apply-resolution subagent. Preserve the rule that the orchestrator never hand-edits `queue.json`.
- [x] `user/skills/lazy-bug-batch/SKILL.md` — mirror the HARD CONSTRAINT 1 prose update (coupled pair; the SKILL.md analog of the same constraint). ⚖ scope-class: covered by inheritance (its HARD CONSTRAINT 1 explicitly inherits `/lazy-batch`'s allowlist+prose verbatim; the only tabulated divergence is the `docs/bugs/` sentinel dir) + the shared `blocked-resolution.md` Defer edit propagating via the `{STATE_SCRIPT}` token. No divergent standalone prose to edit — adding duplicate inherited prose would risk drift.
- [x] `user/scripts/CLAUDE.md` "CLI surface" — add a `--reorder-queue --id <id> --to {tail|head|remove|<index>}` entry documenting it as operator-only / out-of-cycle (gated by `refuse_if_cycle_active`), present on BOTH `lazy-state.py` and `bug-state.py`, mirroring the `--enqueue-adhoc` documentation style.
- [x] Re-run `python3 ~/.claude/scripts/project-skills.py` and `python3 ~/.claude/scripts/lint-skills.py` so the projected output reflects the component edit and the skills lint clean.
- [x] Reverse-reference: the SPEC already cross-references the consuming contracts; no spin-off legs were created (the fix is fully in-scope).

**Implementation Notes (P5 — 2026-06-20):**
- `blocked-resolution.md`: rewired the Defer enactment from a hand-`Edit {SPEC_ROOT}/queue.json` instruction to an inline `{STATE_SCRIPT} --repo-root {repo_root} --reorder-queue --id <id> --to tail` Bash call (calls `lazy_core.reorder_queue` + `_atomic_write`, gated by `refuse_if_cycle_active`). "LEAVE BLOCKED.md IN PLACE / do NOT neutralize" semantics preserved. ONE shared, `{STATE_SCRIPT}`-tokenized edit → drives BOTH `/lazy-batch` (token → `lazy-state.py`) and `/lazy-bug-batch` (token → `bug-state.py`).
- `lazy-batch/SKILL.md` HARD CONSTRAINT 1: the Defer (queue reorder) resolution is now documented as a deterministic out-of-cycle `--reorder-queue` Bash call the orchestrator runs directly, NOT a dispatched Opus apply-resolution subagent; the no-hand-edit-`queue.json` rule is preserved (the orchestrator calls the script). Source/SPEC/PHASES-editing resolution paths (e.g. `/add-phase`) still dispatch.
- `lazy-bug-batch/SKILL.md`: NO edit — covered by inheritance + the shared component (⚖ scope-class, see deliverable above).
- `user/scripts/CLAUDE.md` "CLI surface": added the `--reorder-queue --id <id> --to {tail|head|remove|<index>}` entry (operator-only / out-of-cycle, gated, present on both scripts, parity-guarded, replacing the BLOCKED.md round-trip), mirroring the `--enqueue-adhoc` doc style.
- Verification: `project-skills.py` exit 0 (80 skills, no errors), `lint-skills.py` exit 0 (no broken/embedded patterns). NOTE: `blocked-resolution.md` is referenced by both SKILLs via `See ~/.claude/skills/_components/blocked-resolution.md` (runtime load), NOT `!cat`-injected — so the projection does not inline-expand it; the single component edit IS the propagation mechanism, exactly as planned.
- Review verdict: PASS. Implementation done across all 5 phases — validation tail (`/mcp-test` → coverage audit) pending; `__mark_fixed__` gate owns the SPEC Fixed flip + FIXED.md receipt.
