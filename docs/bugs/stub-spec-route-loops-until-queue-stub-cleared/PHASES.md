...
**Implementation Notes (2026-06-20):** Phase 1 implemented INLINE (Agent-less cycle subagent), test-first.
- **Work completed:** Added `lazy_core.clear_queue_stub(queue_path, feature_id)` (mirrors `reorder_queue`: load → validate-list → find by id → pop `"stub"` → `_atomic_write`; `_die` on missing-id/malformed; byte-stable no-op when absent; returns `{cleared, feature_id, queue_length}`). Factored `is_stub_spec`'s SPEC-text marker checks into the shared predicate `_spec_text_has_stub_marker(spec_text)` (behavior-preserving — existing stub fixtures stay green). Added the `_stub_is_queue_flag_only(spec_text, queue_entry)` discriminator (queue flag True AND no SPEC-text marker) and wired the clear-and-advance at the Step-4.5 branch: discriminator True → clear via `lazy_core` + `_diag` + fall through to Step 5; else byte-identical Step-4.5 dispatch.
- **Tests:** new smoke fixture `stub-queue-flag-lone-survivor` (asserts advance to Step 5 + on-disk flag cleared); existing `stub-queue-flag-only` fixture given a real `> Draft (pre-Gemini)` marker (asserts Step 4.5 retained + flag preserved); 4 `clear_queue_stub` unit tests in `test_lazy_core.py`. Baseline regenerated via `_normalize_smoke_output` (1 line added). All gates green: `lazy-state.py --test`, `bug-state.py --test`, `test_lazy_core.py` (714/714).
- **Files modified:** `user/scripts/lazy_core.py`, `user/scripts/lazy-state.py`, `user/scripts/test_lazy_core.py`, `user/scripts/tests/baselines/lazy-state-test-baseline.txt`.
- **⚖ policy: secondary substring tightening (Theory 3) → deferred per SPEC.** The legacy `**Status:** Draft (research stub)` exact-substring (which misses `Draft (research stub — …)` variants) was NOT tightened — it was not trivially free (a separate behavior change with its own test surface), and the SPEC Open Questions explicitly say "do not gate the primary fix on it." It is the SPEC's logged lower-severity follow-up; the queue flag (not this substring) is the surviving marker the loop fix targets.
- **bug-state.py parity rationale:** intentionally NOT mirrored. The bug pipeline has no stub/research/Gemini step (`user/scripts/CLAUDE.md`), so `clear_queue_stub` — though it lives in shared `lazy_core` — is invoked only by `lazy-state.py`. `bug-state.py --test` stays green (the shared-module change does not regress it). This is the correct divergence, not a parity gap; `lazy_parity_audit.py` requires no bug-side mirror for it.
