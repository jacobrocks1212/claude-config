...
- [ ] `ccstatusline` installed and confirmed working on this machine (branch/model/context% parity with the prior pwsh display, or an accepted replacement display) — precondition for the symlink restore below.
- [ ] Live `~/.claude/settings.json` symlink restored to `user/settings.json` (via `setup.py repair` or `setup.ps1 repair`); prior real file preserved as `.bak`; restore proceeds with the tracked `ccstatusline` default intact (no per-machine `statusLine` content to fold).
...
- [ ] Tests: any pure-logic portion of the `setup.ps1` warn-pass extension (hook-set diff / delegation wiring) covered where unit-testable; the live restore itself is a machine operation verified by MVB below.
