...
- [x] Regenerate `tests/baselines/lazy-state-test-baseline.txt` by piping live `lazy-state.py --test` output through `_normalize_smoke_output` (never by hand) so the new default-off / opt-in-arms / baseline-regression fixtures are captured canonically. (verified: baseline path documented in `user/scripts/CLAUDE.md` → Testing.)
- [x] Confirm `tests/baselines/bug-state-test-baseline.txt` is UNCHANGED (bug pipeline is untouched — if it shifts, that is a regression to investigate, not regenerate).
- [x] Re-confirm `python3 user/scripts/lazy_parity_audit.py` stays green (no `bug-state.py` mirror owed — the budget machinery is feature-only; grep-confirmed at planning time).
...
- **2026-06-22 — plan part 2 (WU-1 + WU-2), executed INLINE (no-Agent dispatch override).** Review verdict: PASS (both batches).
- **WU-1 (doc reword — 3 surfaces):** Rewrote opening of `user/scripts/CLAUDE.md` `--per-feature-cycle-cap` CLI block from "override the dynamically-computed per-feature ceiling / Default (flag absent): L_task = …" to "OFF by default — the guard never arms; opt-in arms a fixed ceiling N; max_cycles is sole default budget." Three composite-softening descriptions re-framed as "under the opt-in flag". Rewrote `user/skills/lazy-batch/SKILL.md` line 66 to "arms the per-feature budget guard with a fixed ceiling N; OFF by default". Mirrored exactly to `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md` line 84; "NO cloud divergence" note preserved. Budget-trip notification (§1c.6/479) unchanged.
- **WU-2 (baseline regen):** Regenerated `tests/baselines/lazy-state-test-baseline.txt` via `_normalize_smoke_output` in a hermetic subprocess (LAZY_STATE_DIR isolated). New `default-off + opt-in-arms` fixtures confirmed captured at line 94. `bug-state-test-baseline.txt` confirmed UNCHANGED (zero git diff). `lazy_parity_audit.py` exits 0. All quality gates green: `test_lazy_core.py` 767/767 (was 766/767 — the previously-expected `test_lazy_state_test_output_matches_baseline` failure now PASS), `lazy-state.py --test` all PASS, `bug-state.py --test` all PASS, `lint-skills.py --check-projected --check-capabilities` OK, `project-skills.py` 80 skills 0 errors.
- **Files modified:** `user/scripts/CLAUDE.md`, `user/skills/lazy-batch/SKILL.md`, `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md`, `tests/baselines/lazy-state-test-baseline.txt`.
- **Review verdict:** PASS (WU-1 inline review PASS; WU-2 inline review PASS).
