...
- **This is a verify-and-reconcile PHASES.md for an out-of-band-landed fix.** Both SPEC prongs plus the requested regression suite are already implemented and green on disk (see the banner at the top and the per-phase citations). Deliverables are marked `[x]` because they are genuinely satisfied on disk, each with the file:line / commit that satisfies it — not because a fresh edit landed this cycle. The executor's role across all three phases is to run `test_hooks.py` green and confirm the static citations, then tick; the `__mark_fixed__` gate owns closure.
