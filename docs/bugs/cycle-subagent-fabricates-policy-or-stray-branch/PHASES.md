...
**Deliverables:**
- [ ] `write_run_marker` (`lazy_core.py:7645`) writes a new `work_branch` field, resolved via the existing `_emit_work_branch(repo_root)` (`lazy_core.py:5101`). Legacy markers lacking the field read as `None` (back-compat, same pattern as `attended`/`per_feature_forward_cycles`).
- [ ] A `lazy_core` read helper returns the marker's `work_branch` (or `None`) from `read_run_marker()` output — the CLI and hook share it; no re-derivation of branch identity outside this helper.
- [ ] `lazy-state.py` gains `--marker-work-branch`: `set_active_repo_root(args.repo_root)` first (repo-keyed), then read the marker; print the stored `work_branch` to stdout and exit 0 if a live marker with a branch is present, exit 1 if absent/stale/no-branch. Read-only (no state-dir creation), mirroring the `--marker-present` handler (`lazy-state.py:7217`).
- [ ] `bug-state.py` gains the identical `--marker-work-branch` query (parity — both pipelines write sentinels and both can strand on a stray branch).
- [ ] Tests in `test_lazy_core.py`: marker now carries `work_branch`; `--marker-work-branch` prints it when present and exits 1 when absent; legacy marker (no field) degrades to exit 1 / `None`, not a crash.
...
- [ ] The deny message NAMES the work branch and instructs: switch back to `<work_branch>` and write the sentinel there (a deny without the corrective branch just loops the retry — mirrors the noncanonical-blocker deny's corrective-name discipline).
- [ ] **Fail-OPEN on every error path:** no python / no marker / exit-1 (no known work branch) / non-sentinel target / branch-query failure / malformed payload → ALLOW (emit nothing, exit 0). Terminal `exit 0` (a PreToolUse non-zero is a hard error; deny is JSON only).
- [ ] Python resolution python3→python (WSL vs. Windows git-bash); inline `-c` body (NOT heredoc — stdin-binding hazard); same skeleton as the model hook.
- [ ] Register in `user/settings.json` as a second command inside the EXISTING `"matcher": "Write|Edit"` PreToolUse block (lines 104–112), alongside `block-noncanonical-blocker-write.sh` (timeout 5).
- [ ] Tests in `test_hooks.py`: deny when HEAD is a stray branch under a live marker; allow when HEAD == work branch; fail-OPEN when no marker / no python / non-sentinel target / malformed JSON; deny message contains the work-branch name.
