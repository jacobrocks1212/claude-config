...
**Deliverables:**
- [x] `user/skills/lazy-batch/SKILL.md` §1d.1 — add a fourth trigger class "process-friction" (a `kind: process-friction` deny-ledger entry from a torn cycle bracket / unexpected commits), routing the same hardening dispatch as the existing three. [VERIFY: `grep -rn "validate-deny" user/skills/lazy-batch/SKILL.md`]
- [x] `user/skills/harden-harness/SKILL.md` — add the process-friction trigger to Triggers 1–4 (now 1–5) and to the Step-2 root-cause taxonomy guidance (this class is `missing-contract` + `hook-defect` per Proven Finding #1). [VERIFY: `grep -n "Triggers" user/skills/harden-harness/SKILL.md`]
- [x] `user/skills/lazy-bug-batch/SKILL.md` — mirror the §1d.1 process-friction trigger (bug pipeline runs the same `bug-state.py` ledger). [VERIFY: `ls user/skills/lazy-bug-batch/SKILL.md`]
...
- [x] `user/skills/_components/lazy-batch-prompts/dispatch-hardening.md` + `user/skills/_components/hardening-dispatch.md` — add `trigger_kind=process-friction` as a valid value and document the friction-signal `--context` bindings (the cycle-bracket-break / unexpected-commit summary). [VERIFY: `ls user/skills/_components/hardening-dispatch.md`]
- [x] Update `user/scripts/CLAUDE.md` and/or `CLAUDE.md` lazy-pipeline docs if the trigger surface description references "routing-layer only" (now process-friction is also a trigger). [VERIFY: `grep -rn "routing-layer" user/scripts/CLAUDE.md CLAUDE.md`] — grep returned empty; no "routing-layer only" assertion existed in either CLAUDE.md; no edit required.

**Implementation Notes (2026-06-16 — Phase 3 / plan part 3 complete):**
- **Trigger wording added to `lazy-batch/SKILL.md` §1d.1:** "**Trigger 4 — process-friction (a `kind: process-friction` deny-ledger entry):** If the probe returns `route_overridden_by: "pending-hardening-debt"` and the oldest unacked ledger entry carries `kind: process-friction` (written by `--cycle-end` on a torn cycle bracket or unexpected commits), emit a hardening dispatch with `trigger_kind=process-friction`. Use the `hardening_emit_command` from the probe JSON verbatim — it already binds `friction_reason` and `friction_detail` in the `--context` keys instead of `denied_prompt_summary`/`denial_reason` (the `build_hardening_emit_command` function in `lazy_core.py` handles this automatically based on the entry's `kind`). This trigger fires **even when the runaway's work was salvaged** (D2: signal, not noise)."
- **`harden-harness/SKILL.md`:** Triggers section extended 1–4 → 1–5. Trigger 4 added (process-friction), with root-cause class guidance `missing-contract` + `hook-defect`. Frontmatter description and `trigger_kind` return-format enum updated to include `process-friction`.
- **`lazy-bug-batch/SKILL.md`:** Trigger 4 mirrored identically (uses `bug-state.py --cycle-end` instead of `lazy-state.py`).
- **`lazy-batch-cloud/SKILL.md`:** Trigger 4 added, explicitly noted as shared/not-a-cloud-divergence; notes that `lazy_core.cycle_end_friction_check` is cloud-flag-independent.
- **`hardening-dispatch.md`:** `trigger_kind` table updated to include `process-friction`; added a `process-friction` context-binding notes section explaining how `build_hardening_emit_command` maps `friction_reason`→`denied_prompt_summary` and `friction_detail`→`denial_reason` automatically.
- **`dispatch-hardening.md`:** Comment header updated to enumerate valid `trigger_kind` values including `process-friction`; description updated from "validate-deny/no-route/inject-hook-error" to include process-friction.
- **`--context` keys for process-friction (from Phase 2 `build_hardening_emit_command`):** `trigger_kind=process-friction`, `denied_prompt_summary=<friction_reason>` (e.g. `cycle-bracket-break` or `unexpected-commits`), `denial_reason=<friction_detail>` (human-readable signal description). These are bound automatically by `build_hardening_emit_command` when it sees `kind: process-friction` in the ledger entry — the orchestrator consumes `hardening_emit_command` verbatim from the probe.
- **Coupled-pair rule:** `lazy-batch` ↔ `lazy-batch-cloud` — both carry Trigger 4 with identical prose (the process-friction trigger is NOT a cloud divergence; `lazy_parity_audit.py` exits 0, confirming no unexplained drift).
- **Gates green:** `project-skills.py` (79 skills, 0 errors), `lint-skills.py --check-projected --check-capabilities` (OK — no broken patterns, no capability pollution), `lazy_parity_audit.py --repo-root .` (exit 0, no unexplained drift).
