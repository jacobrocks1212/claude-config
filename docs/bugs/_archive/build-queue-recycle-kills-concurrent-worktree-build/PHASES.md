...
- **WU-3 (batch 2):** Gated BOTH recycle callsites on occupancy. Runner `finally` (`build-queue-runner.ps1:150-153`) computes `Get-BuildQueueOccupancy -StateRoot $StateRoot -SelfSeq $Seq` → `$otherBuildActive` → `Reset-CompilerServer -OtherBuildActive $otherBuildActive`, and sets `$recycleSkippedReason = 'concurrent-build-active'` when other-active (`$null` otherwise). Wrapper release path (`build-queue.ps1:462-463`) gates its own recycle the same way. Extended the `hygiene` result sub-object with `recycle_skipped_reason` (after `vbcscompiler_recycled`, which stays a `[bool]`); the `$recycleSkippedReason` default is declared before the `try` so it is in scope at result-body construction. Every call `Get-SafeValue`-wrapped → an occupancy-read error yields `0` → recycle proceeds (fail toward recycle); never blocks release. Updated the runner's `results/<seq>.json` schema doc comment.
- **WU-4 (batch 2):** `build-queue-status.ps1:170-181` reads `recycle_skipped_reason` tolerantly (`Get-SafeValue … $null`, back-compat with pre-WU-3 result files) and folds it into the `recycled=…` slot → `recycled=False (skipped: concurrent-build-active)` when present. Rendered NEUTRALLY (plain `Write-Output`); the existing red (`log-failure-override`) / yellow (`no-output`) branches are untouched — a concurrency skip is expected, not an alarm. Smoke-verified both with-field and without-field (unchanged) output.
- **Propagation:** `recycle_skipped_reason` is an additive/optional field; the only other `results/<seq>.json` `hygiene` consumer (`repos/cognito-forms/.claude/scripts/test-filtered.ps1`) reads only `hygiene.build_fidelity` and is unaffected.
- **Gate (Phase 2):** `build-queue-hygiene.Tests.ps1` → 58 passed / 3 failed (3 pre-existing child-scope-quirk failures; no new failures — WU-3/WU-4 add no tests, the decision is unit-tested in WU-2). All four scripts parse-check clean.
- **Review verdicts:** WU-2 PASS, WU-3 PASS, WU-4 PASS (2026-07-02).
...
**Deliverables:**
- [x] `build-queue-enforce.sh` `_BYPASS_RE` (`:76`) recognizes `BUILD_QUEUE_BYPASS=1` when it prefixes the build invocation even behind a leading `cd …&&` (or a `;`/pipeline segment) — i.e. per-segment bypass detection, matching the now-unanchored deny's segment awareness. Keep the scope gate (Cognito worktree) and fail-open contract intact. *(Implemented as `_suppress_bypassed_segments` — a bypassed segment is blanked from the deny scan, so ONLY that segment is bypassed; the leading whole-command fast-path allow is retained unchanged.)*
- [x] Deny messages (`_redirect_reason`, `:175-239`) state the constraint explicitly — that the bypass token must lead the build command (or, once fixed, that the recognized forms include a `cd …&&` prefix) — so an agent's first bypass attempt succeeds.
- [x] Guard against the inverse regression: a real un-bypassed build behind `cd …&&` must STILL be denied (don't let segment-aware bypass detection re-open the enforcement escape the `cd-prefix-bypass` fix closed). *(Per-segment suppression only; pinned by `test_bqe_denies_bypass_in_other_segment` + `test_bqe_denies_bypass_token_as_argument`.)*
- [x] Tests: `cd … && BUILD_QUEUE_BYPASS=1 <build>` → allowed; `cd … && <build>` (no token) → denied; bare `BUILD_QUEUE_BYPASS=1 <build>` → allowed (unchanged); `NAME=val BUILD_QUEUE_BYPASS=1 <build>` → allowed (unchanged). *(5 new `test_bqe_*` cases in `user/scripts/test_hooks.py`; full suite 138 passed 2026-07-09.)*
...
- [x] <!-- verification-only --> Hook unit cases: the four bypass/deny combinations above resolve as specified (fed as JSON payloads to the hook, asserting allow vs deny). *(Executed 2026-07-09: `python -m pytest test_hooks.py -k bqe` → 26 passed — JSON payloads piped to the hook, allow/deny asserted.)*
...
**Deliverables:**
- [x] `/msbuild` + `/mstest` skill docs (`repos/cognito-forms/.claude/skills/{msbuild,mstest}/SKILL.md`) state that for build ops the real transcript is `<seq>.build.log` (and `<seq>.build.err.log`), not the empty `<seq>.log` — so agents read the right file when diagnosing a stale/failed build. *(New "Log files" section in each.)*
- [x] `build-filtered.ps1` (`:24-26`) detects the `--no-restore` no-op: when restore is skipped but `project.assets.json` is missing for the target (a wiped `obj/`), it auto-restores (or errors loudly with a clear message) instead of silently building nothing. Keep `--no-restore` the default for the normal incremental case (per SPEC: don't flip the default). *(`Get-ProjectsMissingAssets` probe — .sln targets parse the project list, single-project targets check their own dir; on a miss the build WARNs naming the wiped projects and drops `--no-restore` for that run so it restores inline. Actual path: `repos/cognito-forms/.claude/scripts/build-filtered.ps1`; script restructured to the dot-sourceable `Invoke-Main` pattern from the sibling `test-filtered.ps1` fix.)*
- [x] Tests: a build against a wiped `obj/` with the default flags either restores-then-builds or fails with the diagnostic — never silently no-ops. *(Unit level: new `build-filtered.Tests.ps1` Pester suite — 9 passed 2026-07-09 — asserts wiped-obj detection and that the arg decision drops `--no-restore` on a miss. A real `/msbuild` against a wiped obj/ remains the runtime-verification row below.)*
