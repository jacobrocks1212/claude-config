...
**Deliverables:**
- [x] `_collect_production_binding_smells(module_source: str) -> list[dict|str]` — pure collector: `ast.parse` the source, walk every top-level `def test_ensure_runtime_production_*`, and within each scan for a `Call` whose function is named `ensure_runtime` carrying a `keyword` arg `boot_alive` or `restart`. Return a sorted, stable list naming each offending `(test_name, injected_kwarg)`. Allow-list the legitimate external-collaborator kwargs (`probe`, `stale_check`, `sidecar_check`, `frontend_probe`, `read_lock`, `live_session_id`, `kernel_start_time_fn`, `sleep`, `write_lock`, `recover_identity`, `config`) — these are NEVER flagged. AST over regex (ignores docstring/comment occurrences), matching the `_collect_orphaned_test_names` rationale.
- [x] `test_ensure_runtime_production_tests_derive_not_inject_signal()` — positive self-checking meta-test: reads `Path(__file__).read_text()`, asserts `_collect_production_binding_smells(...) == []` on the live suite (the cited tests already derive the signal through the real closures, so this PASSES today). On a future regression — a new `test_ensure_runtime_production_*` that passes `boot_alive=`/`restart=` — it FAILS naming the offender and the injected kwarg.
- [x] `test_production_binding_guard_detects_signal_injection()` — negative fixture: feed synthetic module source containing a `test_ensure_runtime_production_injects()` whose body calls `ensure_runtime(..., boot_alive=lambda: True)`, assert the collector reports it by name (proving non-vacuous), and feed a sibling that injects only `probe=`/`stale_check=` and assert it is NOT flagged (allow-list proof).
- [x] Register all three (collector is a helper, two tests) in `_TESTS` so the manual `--test`/`__main__` runner collects them.
- [x] Tests: the two meta-tests above ARE the verification; run `pytest user/scripts/test_lazy_core.py -q` and `python user/scripts/test_lazy_core.py` (manual runner) green.
...
**Deliverables:**
- [x] Extend `_collect_production_binding_smells` (or add `_collect_spawn_double_smells(module_source)` sharing the AST walk) to detect, within a `test_ensure_runtime_production_*` function that swaps `lazy_core.subprocess` (assignment `lazy_core.subprocess = <fake>`) AND does NOT inject `restart=`, whether the assigned subprocess double is constructed from `_FakeSubprocess` (the always-succeeds smell) vs `_WindowsSpawnSemanticsSubprocess` (the faithful double). Flag the `_FakeSubprocess` case for spawn-binding tests; never flag the faithful double.
- [x] Encode the discriminator precisely so a NON-spawn-binding production test that legitimately uses `_FakeSubprocess` for the liveness/timing sub-case (e.g. `test_ensure_runtime_production_boot_alive_live_handle_patient_waits`, which exercises `.poll()` liveness, NOT spawn resolution) is NOT a false positive: a spawn-binding test is one whose assertions reference spawn resolution (`shell_spawns`) OR whose intent is the spawn-invocation path. Resolve the boundary conservatively — prefer a name/marker convention (`*_spawn_*` / a registry tag) over over-broad AST inference, recording the chosen discriminator inline. (⚖ scope-class: see policy note below.)
- [x] `test_spawn_binding_production_tests_use_faithful_double()` — positive meta-test: assert the live suite is clean (`test_ensure_runtime_production_restart_spawns_via_shell_on_windows_cold_boot` already uses `_WindowsSpawnSemanticsSubprocess`, so PASSES today).
- [x] `test_spawn_double_guard_detects_always_succeeds_double()` — negative fixture: synthetic spawn-binding production test using an always-succeeds `_FakeSubprocess` is reported; one using the faithful double is not.
- [x] Register the new test(s) in `_TESTS`.
- [x] Tests: `pytest user/scripts/test_lazy_core.py -q` + manual runner green.
...
#### Implementation Notes (execution — 2026-06-23)

- **Work completed (all 3 phases, executed INLINE — zero Agent dispatches, test-first):** added two pure AST collectors + four guard tests to `user/scripts/test_lazy_core.py`, all registered in `_TESTS`; documented the convention + manual smoke step in `user/scripts/CLAUDE.md`.
- **Landed names (Phase 3 doc references these):**
  - Phase 1 collector: `_collect_production_binding_smells(module_source)` (+ shared helpers `_iter_production_binding_test_defs`, `_call_is_ensure_runtime`).
  - Phase 1 tests: `test_ensure_runtime_production_tests_derive_not_inject_signal` (positive), `test_production_binding_guard_detects_signal_injection` (negative).
  - Phase 2 collector: `_collect_spawn_double_smells(module_source)` (+ helpers `_assigns_lazy_core_subprocess_double`, `_names_used_in`).
  - Phase 2 tests: `test_spawn_binding_production_tests_use_faithful_double` (positive), `test_spawn_double_guard_detects_always_succeeds_double` (negative).
  - Allow-list / signal constants: `_PRODUCTION_BINDING_ALLOWED_KWARGS`, `_PRODUCTION_BINDING_SIGNAL_KWARGS`, `_ALWAYS_SUCCEEDS_SPAWN_DOUBLE`, `_FAITHFUL_SPAWN_DOUBLE`.
- **Phase 2 discriminator NARROWED at execution (finding):** the plan/PHASES contemplated `*_spawn_*` name-substring OR `shell_spawns` as the spawn-binding discriminator. The positive meta-test, run against the LIVE suite, proved the name-substring clause false-positives `test_ensure_runtime_production_wrapper_exits_early_patient_waits_one_spawn` — a liveness/timing test whose `_spawn` refers to the `.spawns` COUNT (patient-wait-no-respawn), not spawn RESOLUTION, and which legitimately uses `_FakeSubprocess`. The discriminator was narrowed to the **`shell_spawns` assertion ONLY** (the attribute only the faithful `_WindowsSpawnSemanticsSubprocess` exposes). This is tighter and still subsumes the genuine spawn-resolution test `test_ensure_runtime_production_restart_spawns_via_shell_on_windows_cold_boot`. (⚖ scope-class — same product behavior, conservative boundary; no NEEDS_INPUT.)
- **Note on test set:** the live suite has FOUR `test_ensure_runtime_production_*` tests (the plan's 5-name list was advisory/drifted; e.g. `..._m4_wrapper_exits_early...` is named `test_ensure_runtime_m4_...`, not `..._production_...`). The collectors enumerate by the actual `test_ensure_runtime_production_` prefix, so the count is self-correcting.
- **Gates:** `pytest user/scripts/test_lazy_core.py -q` → 814 passed; `python user/scripts/test_lazy_core.py` (manual `_TESTS` runner) → 809/809 passed; `python user/scripts/lazy-state.py --test` → all smoke tests passed (no shared-import breakage). The four new tests are collected by both runners (the existing `test_no_orphaned_test_functions` guard would fail on an unregistered new test — it passed).
- **Review verdict:** PASS (inline review; non-vacuity confirmed — the negative fixtures genuinely report synthetic violators and the allow-listed siblings are not flagged; the positive meta-tests assert `[]` on the live suite).
- **Status:** top-level set to **In-progress** — implementation done, validation tail (`/mcp-test` → coverage audit) pending; `__mark_fixed__` owns the terminal flip + `FIXED.md`.
