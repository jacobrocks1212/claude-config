...
**Deliverables:**
- [x] After the canonical Step-3 BLOCKED return block, add: `stray = lazy_core.detect_noncanonical_blocker(spec_path)` and, when non-None, return a `_state(..., current_step="Step 3: mis-named blocker", terminal_reason="blocked-misnamed", notify_message=...)` naming the offending file (basename) and the corrective action.
- [x] Park-mode parity: in the park loop near line 1384 (the `park_blocked` BLOCKED park branch), add a sibling branch that, when `park_blocked` is active and `detect_noncanonical_blocker(spec_path)` returns a stray (canonical `BLOCKED.md` absent), appends `lazy_core.build_parked_entry(feature_id, <stray-path>)` and `continue`s — keeping `--park-blocked` semantics aligned with the canonical blocker. Emit a `_diag` line mirroring the canonical park diag.
- [x] Define the new terminal-reason string as a module constant if neighboring terminals use constants (match the existing `TR_*` / `STEP_*` style in the file); otherwise inline-literal to match the canonical-BLOCKED block's style. Keep it consistent with the local convention.
- [x] Tests: in `lazy-state.py`'s in-file `--test` harness, add fixtures — (a) a feature dir with a stray `BLOCKED_<date>-foo.md` and no canonical file → `blocked-misnamed` terminal; (b) a `BLOCKED_RESOLVED_<date>.md` present alone → does NOT halt (falls through to normal routing); (c) canonical `BLOCKED.md` AND a stray both present → canonical `blocked` terminal precedence (no `blocked-misnamed`).

#### Implementation Notes (Phase 2 — In-progress)

- Wired the detector into `lazy-state.py` Step 3 immediately AFTER the canonical `BLOCKED.md` return (`if blocked_file.exists(): ... return state`) and BEFORE the `NEEDS_INPUT.md` check. The canonical block uses INLINE literals (`current_step="Step 3: blocked"`, `terminal_reason="blocked"`) — no `TR_*`/`STEP_*` constants there — so I inline-literaled `current_step="Step 3: mis-named blocker"`, `terminal_reason="blocked-misnamed"` to match local convention. `notify_message` names the offending basename and instructs rename-to-`BLOCKED.md`-or-neutralize.
- Park-mode parity branch added after the canonical `park_blocked` BLOCKED branch: when `park_blocked` AND canonical absent AND a stray is detected → `_PARKED.append(build_parked_entry(feature_id, stray))` + `_diag` + `continue`, mirroring the canonical park diag.
- 3 `--test` fixtures (`misnamed-blocker-stray` → `blocked-misnamed`; `misnamed-blocker-resolved-only` → Step 5 normal routing; `misnamed-blocker-canonical-precedence` → canonical `blocked`). `python lazy-state.py --test` → all smoke tests passed. Baseline byte-pin drift expected; re-baselined in Phase 5 (not hand-edited).
- **Review verdict:** PASS (inline review — Step-3 wiring + park branch + fixtures; precedence verified by the canonical-precedence fixture returning `blocked` not `blocked-misnamed`).
