...
- [x] `test_containment_allows_pathspec_scoped_commit_with_foreign_staged_path` — marker `feat-A`; staged fixture `["docs/bugs/feat-A/SPEC.md", "docs/bugs/feat-B/FIXED.md"]`; command `git commit docs/bugs/feat-A/SPEC.md -m "fix"` → **allow** (the incident; RED before the fix because the whole-index read lands `feat-B` in `offending`).
- [x] `test_containment_denies_bare_commit_absorbing_foreign_staged_path` — same marker + staged fixture; command `git commit -m "fix"` (no pathspec) → **still deny** (a bare commit flushes the whole index incl. `feat-B` — the genuine cross-contamination catch; proves the fix is a re-scope, not a blanket allow).
- [x] `test_containment_denies_commit_all_flag_with_foreign_staged_path` — same; command `git commit -a -m "fix"` → **still deny** (`-a` re-stages tracked mods and commits the whole index; must not narrow).
- [x] `test_containment_denies_pathspec_commit_that_names_foreign_path` — command `git commit docs/bugs/feat-B/FIXED.md -m "fix"` → **still deny** (a pathspec that itself includes the foreign path is a real second-feature commit; the foreign path is in the effective set).
- [x] `test_containment_pathspec_message_containing_path_token_not_mistaken_for_pathspec` — command `git commit docs/bugs/feat-A/SPEC.md -m "closes docs/bugs/feat-B/FIXED.md"` → **allow** (the `-m` value is skipped; only the real pathspec `feat-A/SPEC.md` is evaluated) — guards the argument-skipping logic.
...
- [x] <!-- verification-only --> The full hook suite is green on the target platform: `python3 -m pytest user/scripts/test_hooks.py -k containment -q` exits 0 with the five new cases collected and passing, and no previously-passing containment test regressed. **Verified by orchestrator 2026-07-19: `60 passed, 237 deselected`; full `test_hooks.py` suite `297 passed`.**

## Implementation Notes (2026-07-19)

- **Work completed:** Added `_commit_pathspecs(command)` + `_commit_effective_paths(command, staged)` to the inline Python of `user/hooks/lazy-cycle-containment.sh` (module scope, near `_staged_paths`), and rewired the second-feature tripwire's `staged = _staged_paths()` → `staged = _commit_effective_paths(command, _staged_paths())`. Deny site, `_is_carve_out`, `_FEATURE_DIR_RE`, `_path_under_feature`, `COMMIT_CEILING`, and the commit-count backstop are byte-unchanged.
- **Parser shape:** flat, quote-aware whitespace tokenizer (not a real shell parser, same discipline as `_mask_heredoc`/`_normalize_ps_syntax`). A single-/double-quoted span (the `-m` message value) is kept as ONE token, so a message that merely mentions a path is never split into a spurious pathspec fragment (test #5). Value-consuming options are skipped in both `--opt value` and `--opt=value` forms; `--` is the pathspec separator; `-a`/`--all`, empty pathspec set, and unterminated-quote ambiguity all return `None` ⇒ whole-index fallback (deny-safe — never a false-ALLOW of a foreign path).
- **Tests:** 5 new `test_containment_*` cases in `user/scripts/test_hooks.py` (TDD red→green; the incident-repro `..._allows_pathspec_scoped_commit_with_foreign_staged_path` and the `..._message_containing_path_token...` case failed RED against the unmodified hook, both green after the fix). Non-weakening guards (bare / `-a` / foreign-pathspec) stay deny.
- **Docs:** one note added to `user/hooks/CLAUDE.md` (Request-time-vs-marker-armed section) describing the pathspec scoping + the bare/`-a` whole-index fallback.
- **SEAM B (bug symptom reproduction):** the red→green `..._allows_pathspec_scoped_commit...` case reproduces the incident at its actual serving path — the real containment-hook PreToolUse subprocess evaluating the exact shared-index fixture + pathspec-scoped `git commit` command — and now allows where it previously denied.
- **Files modified:** `user/hooks/lazy-cycle-containment.sh`, `user/scripts/test_hooks.py`, `user/hooks/CLAUDE.md`.
- **Completion note:** `**Status:**` flip + `FIXED.md` receipt + archive are gate-owned (`__mark_fixed__`) — not authored here.
