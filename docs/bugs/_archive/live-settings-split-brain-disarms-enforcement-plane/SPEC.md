...
- **D1 — Where does genuinely per-machine content live?** **Resolved** (`NEEDS_INPUT.md`, 2026-07-12): the `settings.local.json` overlay is confirmed **non-viable** — it is a project-scoped-only Claude Code concept, no user-level (`~/.claude/settings.local.json`) merge exists per official docs, so the overlay option is off the table entirely (the open sub-question below is answered: there is no merge precedence to rely on). `env.CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` folds cleanly into the tracked SSOT (landed Phase 1/3). For `statusLine`: chosen path is to **keep the tracked `ccstatusline` default as-is** and install/configure `ccstatusline` on this machine (`npm install -g ccstatusline` or equivalent) rather than folding the pwsh one-liner into the shared SSOT — avoids a cross-machine Windows/pwsh regression risk on any non-Windows/cloud session sharing this tracked file. ~~Open sub-question: confirm Claude Code's settings.json/settings.local.json merge precedence for `statusLine` before relying on the overlay.~~ (answered above — no overlay).
