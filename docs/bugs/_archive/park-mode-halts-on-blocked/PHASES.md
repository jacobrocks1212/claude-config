...
**Deliverables:**
- [x] `parked-flush.md` Step-1 (collect): the `pending_flush` set now includes items whose sentinel is `BLOCKED.md` (still named — not yet resolved), partitioned by `sentinel_kind` into needs-input vs blocked.
- [x] `parked-flush.md` Step-2 (schema check): a blocked-parked item is NOT run through the `## Decision Context` rich-body schema check (that check is needs-input-only). A blocked item is validated against the BLOCKED.md frontmatter schema (`blocker_kind`, `phase`) instead, and never dropped as "malformed `## Decision Context`".
...
- [x] Update the component's pipeline-binding table / coupling note if a new token is needed; otherwise leave the existing `{SKILL}`/`{STATE_SCRIPT}` bindings.
- [x] Run-end digest (Step 5/6 of the flush): blocked-parked resolutions are logged alongside decision flushes (one meta-cycle each), and the PushNotification wording covers blocked items.

**Implementation Notes (2026-06-16):**
- Done. `parked-flush.md` now partitions collected items by `sentinel_kind` into `pending_needs_input[]` (Steps 2–5, the existing `## Decision Context` machinery) and `pending_blocked[]` (new **Step 2.6**, which delegates to `blocked-resolution.md` — the SAME affordance as `{SKILL}` Step 1h). Step 2 explicitly scoped to needs-input only; a blocked item is never run through the decision-context schema check (so never dropped as "malformed"). Step 2.6 validates BLOCKED.md frontmatter, classify-first per completeness-policy §3, honors the step-1a escalation guard, and neutralizes via `--neutralize-sentinel {spec_path}/BLOCKED.md` (→ `BLOCKED_RESOLVED_<date>.md`). `halt-for-manual` STOPs like Step 1h; everything else continues the loop.
- All internal `pending_flush` references in the needs-input branch (Steps 1, 2, 2.4, 2.5, 3) renamed to `pending_needs_input`; Steps 5/6/7 + flush trigger (b) updated to account for BOTH kinds and `queue-exhausted-all-parked`. PushNotification count is `len(pending_needs_input) + len(pending_blocked)`.
- **No new pipeline-binding token needed** — Step 2.6 reuses the consuming skill's existing Step 1h bindings (`{SPEC_ROOT}`, `{ADD_PHASE}`) plus the bindings this component already declares.
- **Verification:** `lint-skills.py` clean; `project-skills.py` clean (79 skills / 90 components, zero errors). Consumer set unchanged — `grep -rl "parked-flush.md"` shows the three orchestrators + the cross-reference docs (decision-resume / sentinel-frontmatter / input-audit-prompt). NOTE: all three orchestrators reference parked-flush.md by PATH ("read and apply …"), not `!cat` injection, so the projector does not inline it — the single-source component edit is the runtime deliverable for all three. Traced trigger (a)/(b)/(c) end-to-end: a blocked-parked item reaches the Step 2.6 resolution affordance (never the missing-`## Decision Context` skip), and neutralization targets `BLOCKED.md`.
