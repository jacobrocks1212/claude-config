...
**Deliverables:**
- [x] `write_run_checkpoint` reads the live marker RAW (same non-destructive access as today — never `read_run_marker`) and writes a `continuity: {field: value, ...}` block covering every `RUN_CONTINUITY_FIELDS` key present on the marker. Retain `reason`/`next_route`/`counters`/`operator_authorized`/`ts` keys (back-compat; `counters` may become a derived view of the continuity counters or stay as-is — implementer's call, but no consumer breaks).
- [x] `restore_checkpoint_counters` carry-forward branch: when a `continuity` block is present, re-apply ALL its fields to the just-minted marker in one loop, applying the per-field guards (age gate for `started_at`; coerce-to-non-negative-int for the two counters; preserve the existing `_coerce` fail-safe semantics; `per_feature_*` maps applied only when a well-formed dict). `last_advance_consume_count` stays forced to 0 (it is a `RUN_FRESH_FIELD`).
- [x] Back-compat fallback: when NO `continuity` block exists but the legacy `counters` + flat `run_started_at` fields do, restore via the existing legacy path (so a pre-fix checkpoint file mid-flight still resumes correctly).
- [x] Operator-authorized branch unchanged: still returns `None` (no carry — fresh budget + fresh identity).
- [x] Tests: a round-trip test proving every `RUN_CONTINUITY_FIELDS` field (incl. both `per_feature_*` maps) survives `write_run_checkpoint` → `consume_run_checkpoint` → `restore_checkpoint_counters` in the carry-forward branch; a test proving a legacy flat-`run_started_at` checkpoint still restores identity; a test proving operator-authorized still no-ops; the existing `test_restore_checkpoint_counters_carries_forward_run_identity` (5 cases) stays green unchanged.
...
- **WU-3 — `write_run_checkpoint` snapshot rewrite.** Now reads the live marker RAW (unchanged non-destructive access — never `read_run_marker`) and writes a nested `continuity: {field: value}` block covering every `RUN_CONTINUITY_FIELDS` key present on the marker. Retained the flat `run_started_at` mirror + all back-compat keys (`reason`/`next_route`/`counters`/`operator_authorized`/`ts`). No consumer of the old keys breaks.
- **WU-4 — `restore_checkpoint_counters` carry-forward rewrite.** When a `continuity` block is present, re-applies all its fields as one unit (counters via `_coerce`; `started_at` via the shared `_restore_identity` helper carrying the Round-35 age gate; the two `per_feature_*` maps only when a well-formed dict). Factored the identity-restore logic into `_restore_identity` so both the new continuity path and the legacy flat-`run_started_at` fallback share it. `last_advance_consume_count` stays forced to 0. Operator-authorized branch + legacy fallback unchanged in behavior.
- **WU-5 — tests.** Added 7 tests (registered in `_TESTS`): full-block snapshot, raw-read non-destructive on a stale marker, full-block restore (closes the per_feature_* whack-a-mole), age-gate-preserved, operator-authorized no-op, legacy flat-checkpoint back-compat, and an end-to-end write→consume→restore round-trip. The existing `test_restore_checkpoint_counters_carries_forward_run_identity` (5 cases) + all sibling checkpoint tests stay GREEN unchanged.
- Files modified: `user/scripts/lazy_core.py`, `user/scripts/test_lazy_core.py`, `user/scripts/CLAUDE.md` (provenance-branched doc bullet).
- **Review verdict:** PASS (inline self-review — substantive: every SPEC Affected-Area row covered, both Theory-1/Theory-2 seams closed, all guards preserved; ground-truthed by running the tests RED-before/GREEN-after). RED was confirmed for the right reason (missing `continuity` block → per_feature_* maps stayed `{}`), not a setup error.
- Both `--test` baselines byte-stable (additive checkpoint-shape change, not on the state-compute path); parity audit green (shared `lazy_core`, no script-mirror owed).
- **Runtime Verification rows** (the two `<!-- verification-only -->` boxes) intentionally left UNCHECKED — owned by the orchestrator's validation tail, not this implementation cycle. (Note: this is a deterministic state-script helper with `MCP runtime: not-required`; the `--test` harnesses + pytest ARE the authoritative evidence, so both runtime rows are satisfied by the GREEN round-trip + age-gate tests above — left unticked per the status-honesty contract for the gate to close.)
