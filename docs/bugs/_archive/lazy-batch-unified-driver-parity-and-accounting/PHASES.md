...
**Implementation Notes (2026-06-17, inline `/execute-plan` under `/lazy-bug-batch`):**
- **Work completed:** Added `lazy_core.advance_forward_cycle(state)` (a new function mirroring `advance_meta_cycle`) plus the SSOT `_FORWARD_ADVANCING_PSEUDO_SKILLS` frozenset. The advance is keyed on a change in the `[feature_id, current_step, sub_skill]` tuple persisted as the marker field `last_advance_state_key` (a JSON list, not a tuple — `json.loads` round-trips tuples to lists, so equality is compared list-vs-list). Wired the call into `lazy-state.py`'s `--apply-pseudo` handler: after a SUCCESSFUL apply whose `name in _FORWARD_ADVANCING_PSEUDO_SKILLS`, it advances using the spec-dir slug as `feature_id` and the pseudo-skill name as `current_step`, wrapped fail-open (a breadcrumb via `_diag`, never blocks the apply).
- **Marker field name:** `last_advance_state_key` (as planned). Value shape: `[feature_id, current_step, sub_skill]` JSON list.
- **Classifier divergence from the plan's hint:** the plan said "`__`-prefixed → meta". The actual classifier routes a `__`-prefixed sub_skill to `forward_cycles` IFF it is in `_FORWARD_ADVANCING_PSEUDO_SKILLS` (the six forward terminals), else meta. This is correct: `__mark_complete__`/`__mark_fixed__` etc. ARE pipeline-advancing and must count toward the forward budget, not meta. A real (non-`__`) sub_skill is always forward.
- **Integration notes:** `advance_forward_cycle` is consume-INDEPENDENT (no `consumed_emission_count` read) — orthogonal to `advance_run_counters`'s consume gate. Both can coexist on the same marker (different advance triggers); the state-key idempotence prevents a re-fire double-count the same way the consume gate does. Legacy markers lacking `last_advance_state_key` default to None → first state change always advances.
- **Pitfalls:** the `--apply-pseudo` advance uses `Path(spec).resolve().parent.name` for `feature_id` — for a bug spec at `docs/bugs/<slug>/SPEC.md` this resolves to `<slug>`, a stable per-item id.
- **Files modified:** `user/scripts/lazy_core.py` (new function + frozenset), `user/scripts/lazy-state.py` (`--apply-pseudo` call site), `user/scripts/test_lazy_core.py` (5 new hermetic tests covering cases a–d + legacy-marker default).
- **Gates:** `test_lazy_core.py` 557/557; `lazy-state.py --test` + `bug-state.py --test` both green (smoke output unchanged — no baseline regen).
...
**Implementation Notes (2026-06-17, inline `/execute-plan` under `/lazy-bug-batch`):**
- **Work completed:** (1) `_load_bug_queue_for_merged` (lazy-state.py) — replaced the silent bare-except with a `_diag(f"merged-view bug-side load failed ({exc}) — degrading to features-only")` breadcrumb that fires before the fail-open `return []`. The `not bug_state_path.exists() → return []` early-out is unchanged (no breadcrumb — expected no-op). (2) `/lazy-batch` Step 1b — amended the `all-features-complete` bullet and ADDED an `all-bugs-fixed` bullet (the unified driver can now have a bug head returning it), plus a dedicated "Option-(b) unified-driver fallthrough" box defining the two-direction probe-the-other-type-before-stopping rule. (3) Mirrored the box + both bullets into `/lazy-batch-cloud` Step 1b, `--cloud` carried on both state scripts (coupled-pair rule); `cloud-queue-exhausted` is explicitly NOT a fallthrough trigger (it is the cloud-defensive stop).
- **Test divergence from the plan's wording:** the plan's second test ("merged-view fixture asserting an on-disk bug surfaces / is loadable in `bug_items`") is ALREADY covered by the pre-existing `test_next_merged_cli_over_two_queue_fixture` (it seeds a `docs/bugs/bug-x/` dir + queue entry and asserts the bug surfaces as the merged head end-to-end through `load_bug_queue`/`_find_open_bug_dirs`). Rather than duplicate it, WU-3 adds the NET-NEW breadcrumb coverage (`test_load_bug_queue_for_merged_breadcrumb_on_load_failure` + a `_no_breadcrumb_on_clean_load` negative). The masking-resolution itself is a DRIVER-LOOP concern (SKILL prose), validated structurally by the parity audit + lint, not a `--test` code path — as the plan/SPEC note.
- **Integration notes:** the breadcrumb appends to the shared `lazy_core._DIAGNOSTICS` list (imported bare into lazy-state.py), so it surfaces in the merged-view `diagnostics` field. Fail-open preserved end-to-end.
- **Pitfalls:** the breadcrumb test loads the hyphenated `lazy-state.py` in-process via importlib (helper `_load_lazy_state_module`) and monkeypatches `importlib.util.module_from_spec` to inject a fake bug-state module whose `load_bug_queue` raises — intercepting ONLY the `_bug_state_for_merged` spec name, passing every other import through.
- **Files modified:** `user/scripts/lazy-state.py` (`_load_bug_queue_for_merged` breadcrumb), `user/skills/lazy-batch/SKILL.md` + `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md` (fallthrough box + bullets, mirrored), `user/scripts/test_lazy_core.py` (2 new breadcrumb tests).
- **Gates:** `test_lazy_core.py` 561/561; `lazy-state.py --test` + `bug-state.py --test` green; `lazy_parity_audit.py --repo-root .` exit 0 (merged-view symmetry preserved); `lint-skills.py --check-projected` clean; `project-skills.py` projects cleanly.
...
**Implementation Notes (2026-06-17, inline `/execute-plan` under `/lazy-bug-batch`):**
- **Work completed:** (1) Added a `__mark_fixed__` pseudo-skill block to `/lazy-batch` Step 1c.5 (peer of `__mark_complete__`), mirroring `/lazy-bug-batch`: two docs-only gates (Gate 1 MCP-coverage audit, Gate 2 completion-integrity, `kind: fixed`/`FIXED.md`), then `bug-state.py --apply-pseudo __mark_fixed__`, then the chained `bug-state.py --repo-root {repo_root} --archive-fixed`, with `ok: false` → `BLOCKED.md (blocker_kind: archive-failure)` + push backstop. `__mark_fixed__` was already in the Step-5 forward-cycle counter list. (2) Mirrored the same block into `/lazy-batch-cloud` Step 1c.5 (`bug-state.py --cloud` for gate state probes; bug validation docs-only → fully reachable in cloud) AND added `__mark_fixed__` to the cloud Step-5 forward-cycle counter list (it was missing there). (3) Added the `(r"--archive-fixed", "bug archive --archive-fixed chain")` predicate to `lazy_parity_audit._MERGED_VIEW_PREDICATES` — reused the existing predicate loop in `audit_merged_view_dispatch_parity`, NO new audit function.
- **NO `lazy_core.py` / `bug-state.py` change** (as the SPEC mandates) — the `apply_pseudo` `is_fixed` trim predicate + the `archive_fixed` CLI were already correct; the gap was purely the missing chained call in the driver prose + the parity guard.
- **Test divergence:** WU-6 adds THREE tests (predicate-present, missing-chain-produces-finding over a temp 2-SKILL fixture, real-drivers-pass) rather than one — the fixture path needed both a positive and negative case to prove the predicate is enforced (RED-for-the-right-reason). `lazy_parity_audit` is hyphen-free so it imports directly; the fixture seeds the two SKILL stubs at the exact repo-relative paths `_MERGED_VIEW_DRIVER_FILES` expects.
- **Files modified:** `user/skills/lazy-batch/SKILL.md` + `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md` (`__mark_fixed__` block, mirrored; cloud counter-list fix), `user/scripts/lazy_parity_audit.py` (new predicate), `user/scripts/test_lazy_core.py` (3 new parity-predicate tests).
- **Gates:** `test_lazy_core.py` 562/562; `lazy_parity_audit.py --repo-root .` exit 0 (full audit incl. the new predicate); `lazy_parity_audit.py --merged-view` exit 0; `lint-skills.py --check-projected` clean; `lazy-state.py --test` + `bug-state.py --test` green.
