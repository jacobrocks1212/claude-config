...
**Deliverables:**
- [ ] Add `assert_sidecar_connected: false` (default off) + `sidecar_status_url` (default `http://localhost:3333/tools/get_sidecar_status`) to `_ENSURE_RUNTIME_DEFAULT_CONFIG` (`:6116`), following the existing config-key pattern. Repo-agnostic default = sidecar assertion skipped.
- [ ] Add a `_default_sidecar_probe(sidecar_status_url) -> bool` helper (stdlib `urllib`, best-effort, never raises — mirrors `_default_runtime_probe` at `:6131`) returning `is_connected` from the `get_sidecar_status` payload (False on any error / missing field).
- [ ] Add a `sidecar_check=None` injected parameter to `ensure_runtime` (`:6206`); when `None` AND `cfg["assert_sidecar_connected"]` is truthy, bind `_default_sidecar_probe`; when the config flag is falsy, bind a no-op `lambda: True` (skip = treated as connected). Thread `sidecar_check` into `_ensure_runtime_m4`.
- [ ] In `_ensure_runtime_m4` **Phase 3: Health** (`:6506-6518`): after `code == 200` and BEFORE the `READY` verdict, call `sidecar_check()`. If it returns False (pipe dead despite HTTP 200), route to `_recover_runtime(cfg, "DEAD", ownership_verified=True, …)` (the existing bounded-recovery path — forces a `dev:restart` that reaps the stale pipe). On recovery exhaustion the existing `_recover_runtime` already returns `BLOCKED` with a `terminal_blocker` → the orchestrator writes `blocker_kind: mcp-runtime-unready` (escalation-immune), NOT a dispatch.
- [ ] Confirm `validation_escalation` (`:328`) is UNCHANGED (predicate is correct — SPEC Finding 3).
- [ ] Tests: hermetic `test_lazy_core.py` fixtures (see Runtime Verification below — these are real unit tests of the gate, the verification surface for this docs/script-only repo).
...
**Deliverables:**
- [ ] In `cycle-base-prompt.md` `@section mcp-test-runtime variant=runtime-up` (:257-268): add an explicit sidecar-readiness check + terminal — "if `get_sidecar_status` reports `is_connected: false` (the runtime is HTTP-healthy but the sidecar pipe is dead — a self-inflicted env transient, NOT a code failure), do NOT write an `mcp-validation` BLOCKED. Return the single line `NEEDS_RUNTIME` (the orchestrator re-boots cleanly in its own session and re-dispatches)." Wording parallels the `no-runtime` variant's `NEEDS_RUNTIME` escape (:284-287).
- [ ] In `cycle-base-prompt.md` `@section skill-mcp-test-common` (:230, :246-255): clarify that the `mcp-validation` BLOCKED terminal is for CODE/engine failures, and a sidecar-pipe-dead state is explicitly EXCLUDED (routes to the runtime-readiness terminal above), so the escalation budget is not charged an env transient.
- [ ] In `mcp-test/SKILL.md` Step 2 orchestrated path (`:151-163`): mirror the distinction — make "appears dead mid-cycle → NEEDS_RUNTIME" (:162) explicitly cover the HTTP-healthy-but-`is_connected: false` case (today "appears dead" is HTTP-framed and obscures the pipe-dead case). Reference the `:186` `get_sidecar_status` probe as the discriminator.
- [ ] In `lazy-batch/SKILL.md` Step 1d.0 prose (`:583-608`): document the sidecar-pipe dimension of `--ensure-runtime` and that a pipe-dead-but-HTTP-healthy runtime routes to `mcp-runtime-unready` (escalation-immune), not a dispatch (SECONDARY — keeps the prose contract in sync with the Phase 1 code).
- [ ] Run `python ~/.claude/scripts/project-skills.py` and spot-check the projected `cycle-base-prompt` expansion (both `_default/` and the algobooth per-repo projection) — confirm the new terminal expanded cleanly, no broken injection.
