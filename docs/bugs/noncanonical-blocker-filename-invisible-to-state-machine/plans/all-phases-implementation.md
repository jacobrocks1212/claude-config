...
- [x] **WU-2.2** — Park-mode parity in `lazy-state.py`'s park loop (~line 1384, the `park_blocked` BLOCKED branch): add a sibling branch — when `park_blocked` is active, canonical `BLOCKED.md` is ABSENT, and `lazy_core.detect_noncanonical_blocker(spec_path)` returns a stray → `_PARKED.append(lazy_core.build_parked_entry(feature_id, <stray-path>))`, emit a `_diag` line mirroring the canonical park diag, and `continue`. Keeps `--park-blocked` semantics aligned with the canonical blocker.
  - **Files:** `user/scripts/lazy-state.py` (~line 1384).

- [x] **WU-2.3** — Add three fixtures to `lazy-state.py`'s in-file `--test` harness: (a) feature dir with stray `BLOCKED_<date>-foo.md`, no canonical → `blocked-misnamed` terminal; (b) `BLOCKED_RESOLVED_<date>.md` alone → does NOT halt (normal routing); (c) canonical `BLOCKED.md` + stray both → canonical `blocked` precedence (no `blocked-misnamed`). Follow the existing fixture pattern in the harness (the park/needs-input fixtures near line 5156 are the template).
