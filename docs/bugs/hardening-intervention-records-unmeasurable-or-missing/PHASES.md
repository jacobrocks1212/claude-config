...
- [ ] r1-r3 backfill: create `harden-2026-07-r1.md`, `-r2.md`, `-r3.md` via the D9 manual path (`--record-intervention --shipped-commit <sha> --shipped-date <date>` → `provenance: backfilled`), each with a measurable `--target-signal` where one exists (r3 was `event:process-friction`-flavored; r1/r2 per their commits) or explicit `--target-signal undeclared` for the genuinely-immeasurable. **(part-2 WU-6 — BLOCKED: `--record-intervention` is orchestrator-only and C3-refused (exit 3) for this contained execute-plan cycle subagent; also covers r31/r32 per part-1's WU-4 scope note. Exact orchestrator commands in `BLOCKED.md`.)**
...
- [ ] Verify: `doc-drift-lint.py --repo-root .` (Phase 3) is coverage-clean over the live tree after the repair. **(Coverage-clean NOW via the retained `doc-drift:deliberate-divergence` markers on the un-backfilled rounds; the markers are removed once the orchestrator lands the backfill records — left unticked until then.)**

**Implementation Notes (2026-07-12, part-2 WU-6 — data repair, PARTIAL / backfill BLOCKED):**
