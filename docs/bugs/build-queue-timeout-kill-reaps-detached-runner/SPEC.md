...
**Scope: crash-safe release (operator-locked)** — shrink the unprotected post-build window instead of trying to trap an untrappable TerminateProcess. The build's truthful result now survives a kill of the runner at any point after the build process exits.

### What changed

- **`user/scripts/build-queue-runner.ps1` — two-phase result write.** The `finally` block now performs a **crash-safe EARLY result write** BEFORE any hygiene work (`Stop-BuildJobTree` → occupancy-gated `Reset-CompilerServer` → the RED-only `Remove-PoisonedArtifacts` sweep): the moment the exit code + build-fidelity classification are known, the truthful result (exit_code, `result_fidelity`, `build_fidelity`, test-op `counts`, op/started_at/duration_seconds) is written atomically with a `hygiene.status: "pending"` sub-object (real hygiene fields defaulted). After hygiene completes, the pre-existing inline write runs as the **FINAL write**, merging the real hygiene fields (`vbcscompiler_recycled`, `recycle_skipped_reason`, `quarantined_artifacts`, `lockers_reaped`, …) with `hygiene.status: "complete"`. A kill mid-sweep now leaves a truthful pending result instead of nothing. `$resultFidelity`/`$counts`/duration moved into the `finally` (they depend only on the exit code + already-complete logs); `duration_seconds` is now captured at build exit (exec-run time per the documented schema, hygiene excluded) and reused verbatim by the final write + stats ring. Hygiene behavior/gating/order, the stats-append position (after the final outcome), the lock release position (AFTER hygiene — it serializes hygiene against the next build; a post-early-write kill still strands the lock, self-healed by the existing next-enqueue 3-dead-tick reclaim), and the try/finally exception semantics (hygiene still runs on build-spawn exceptions; early write is skipped when `$exitCode` is unknown) are unchanged. New: `exit 1` instead of `exit $null` on the degraded spawn-exception path.
- **`user/scripts/build-queue-hygiene.ps1` — `Write-BuildQueueResult`.** New fail-open atomic result writer (temp + `File.Replace`, direct-write fallback) implementing the early write; the runner's final write stays INLINE so the result contract survives a failed hygiene-module dot-source. Pester-testable reference for the result schema.
- **Bare-`True` stdout leak fixed** (`build-queue-runner.ps1` trap + finally): both unassigned `Get-SafeValue { Stop-BuildJobTree … }` returns are now `$null =`-assigned, so no stray `True` reaches `logs/<seq>.log` (which the test-counts regex parses).
- **Adjacent defect found & fixed on the same serving path — test-op counts sharing violation.** The counts parse read `logs/<seq>.log` via `[System.IO.File]::ReadAllText` (FileShare.Read), but that file IS the runner's own redirected stdout whose write handle stays open for the runner's lifetime — a **deterministic sharing violation** (empirically confirmed) that silently nulled `counts` for every live test op, at the old post-hygiene position just as much as at the new early one. Now opened `FileShare.ReadWrite` (the wrapper live-tail pattern).
- **`build-queue-await.ps1` / `build-queue.ps1` — no changes needed.** Await polls for the result file, so it now returns as soon as the early write lands, re-emitting the authoritative `Format-BuildQueueBanner` (which reads only exit code/fidelity/counts — all truthful at the early write; `hygiene.status` is additive and ignored by defensive readers). The wrapper's read-merge-write only ever runs after the runner exits (post-final-write), so the two runner writes and the wrapper merge cannot clobber each other.
- Result-schema doc (runner header) updated with the `hygiene.status: "pending" | "complete"` field and the two-phase contract.

### Regression-test evidence (`user/scripts/build-queue-runner.Tests.ps1`, new)

Serving-path reproduction per the symptom-reproduction gate: the REAL runner is launched detached against a temp StateRoot, with a shim hygiene module (dot-sources the real module; overrides only `Reset-CompilerServer`/`Stop-DllLockers` for machine safety and makes `Remove-PoisonedArtifacts` block 120s to model the multi-minute Cognito RED sweep), a fake RED exec (`exit 1`), and the runner process is **killed with TerminateProcess mid-sweep** — then asserts `results/<seq>.json` EXISTS with truthful `exit_code: 1` + `hygiene.status: "pending"`, `active.lock` honestly still held for the dead-tick reclaim, and `build-queue-await.ps1 -Seq <n>` exits **1 with the exact `RESULT=FAIL` banner** instead of 124. Second kill test covers a RED test op (counts present in the early result; await banner reports `tests=5 failed=2`). Fast-path tests pin the final write merging real hygiene fields over the early write (`status: complete`, quarantine/recycle fields, lock released, stats ring appended) and the no-bare-`True` stdout guarantee; unit tests cover `Write-BuildQueueResult` (atomicity, final-over-early overwrite, legacy no-op field omission, fail-open). **8/8 passed** (Pester 5.6.1, 2026-07-10); full pre-existing suites re-run green: `build-queue-await.Tests.ps1` 8/8, `build-queue-hygiene.Tests.ps1` 172/175 — the 3 failures (`Add-ProcessToBuildJob`/`Stop-BuildJobTree` zero-handle `$false`-vs-`$null`, `Reset-CompilerServer` bool) reproduce identically against the unmodified HEAD module (pre-existing Pester scriptblock-scope gotcha in tests of functions this fix does not touch).
