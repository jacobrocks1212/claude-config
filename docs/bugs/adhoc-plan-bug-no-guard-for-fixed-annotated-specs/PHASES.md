...
**Deliverables:**
- [x] In `bug-state.py` Step 4 (`if not phases_file.exists():`), immediately inside the `if _status == "Concluded":` branch (before the `return … sub_skill=SKILL_PLAN_BUG`), call `lazy_core.is_fixed_unreconciled(spec_dir, repo_root)`. On a hit: write canonical `BLOCKED.md` via `lazy_core._atomic_write` using `format_fixed_unreconciled_blocker`, append a `_diag` breadcrumb ("Concluded SPEC carries a `**Fixed:**` annotation with no receipt — diverted to reconciliation, not plan-bug"), and return `_bug_state(..., terminal_reason=…blocked…)` so the next probe halts at the Step-3 `BLOCKED.md` check.
- [x] Add a `tabulated-divergence` row to `user/scripts/compute-state-routing-parity.json` for this bug-only pre-gate (owner + reason: the feature pipeline has no out-of-pipeline `**Fixed:**` annotation / `Concluded` intermediate, so no `lazy-state.py` mirror is owed).
- [x] Tests: a `bug-state.py --test` in-file fixture (`concluded-fixed-unreconciled-diverts`) asserting the Concluded+`**Fixed:**`+no-receipt fixture routes to the blocked outcome (writes `BLOCKED.md` with `blocker_kind: fixed-unreconciled`, `sub_skill` is NOT `plan-bug`); the existing fixture #24 `concluded-investigation-plan-bug` (plain Concluded-no-annotation → plan-bug) is the no-regression companion. Registered in the script's test list; byte-pinned baseline regenerated via `_normalize_smoke_output`.

**Implementation Notes (2026-07-19):** Pre-gate inserted inside the `if _status == "Concluded":` branch before the plan-bug return — writes the full canonical `BLOCKED.md` (frontmatter incl. `blocker_kind: fixed-unreconciled`) directly via `_atomic_write` (guarded on `not blocked_file.exists()`), `_diag` breadcrumb, returns `TR_BLOCKED`/`STEP_BLOCKED` so the next probe halts at Step-3. HALTS for reconciliation — never auto-writes FIXED.md. New `tabulated-divergence` row `fixed-unreconciled-predispatch-divert` (owner bug-state.py). New fixture + `_assert_fixed_unreconciled_blocked` extra assertion (verifies not-plan-bug + BLOCKED.md blocker_kind). `bug-state.py --test`, `lazy-state.py --test`, `lazy_parity_audit.py`, and the baseline-comparison test all green.
