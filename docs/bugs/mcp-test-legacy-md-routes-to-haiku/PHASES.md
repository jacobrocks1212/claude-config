...
- [ ] Update `user/scripts/CLAUDE.md` → `## mcp-test model-tier routing` to record that `emit_cycle_prompt` now consults `route_mcp_test_tier` on the autonomous cycle-model path (option-b conservative escalation), closing the SPEC's "wired into zero autonomous paths" gap.
