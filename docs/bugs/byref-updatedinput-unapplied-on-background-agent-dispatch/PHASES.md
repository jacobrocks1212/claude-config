...
- [x] `--resolve-ref <nonce>` CLI action on `user/scripts/lazy-state.py` (`build_parser()` + handler): prints the resolved prompt bytes to stdout + exit 0 on hit; prints nothing + exit 1 on miss (nonce absent / TTL-expired / predates run). Subagent-callable (a read; NOT gated by `refuse_if_cycle_active`).
- [x] `--resolve-ref <nonce>` CLI action on `user/scripts/bug-state.py` (coupled-pair mirror — identical behavior; nonce keyed).
- [x] Register `--resolve-ref` in `lazy_parity_audit.py::audit_state_script_parity` (`_RESOLVE_REF_RE` + loop finding) so both scripts are asserted to expose it. NOTE: the coupled *state-script CLI* parity lives in the audit CODE (like every other coupled CLI surface — `--reassert-owner`, `--sync-deps`), NOT `lazy-parity-manifest.json` (which holds SKILL.md pairs only). The `docs/cli/cli-surface.json` registry was regenerated (`cli_surface_gen.py`) for the new flag.
- [x] Tests: `tests/test_lazy_core/test_dispatch.py` — reader returns bytes for a consumed entry, None for missing / TTL-expired / cross-run / still-unconsumed / never-mutates; both scripts' in-file `--test` fixtures exercise `--resolve-ref` (hit → bytes/exit 0 even under a subagent context, miss → exit 1); byte-pinned `--test` baselines regenerated; `lazy_parity_audit.py --repo-root .` green.
...
### Phase 1 — landed 2026-07-19 (execute-plan, inline test-first)

- **WU-1** — `lazy_core.dispatch.resolve_consumed_emission_by_nonce(nonce, *, now=None)` returns the entry's `prompt_raw` (fallback `prompt_norm`) as a STRING for a **consumed**, TTL-fresh, run-start-gated registry entry; None otherwise. Reuses `resolve_emission_by_nonce`'s TTL + run-start gates verbatim, inverting ONLY Gate-1 (require `consumed` truthy). READ-ONLY / fail-safe (any error → None). Registered in the `lazy_core` facade (`__init__.py` name→submodule map). `resolve_emission_by_nonce` byte-untouched. 6 pytest cases in `test_dispatch.py` (happy path, unknown, unconsumed, TTL-expired, predates-run, never-mutates-consumed) + `_TESTS` registration (the dead-coverage guard).
- **WU-2** — `--resolve-ref <nonce>` read action on BOTH `lazy-state.py` and `bug-state.py` (`build_parser()` arg + a `main()` handler placed beside `--marker-work-branch`, so it runs after `set_active_repo_root` and before `compute_state`). Hit → exact bytes + `\n` to stdout + exit 0; miss → nothing + exit 1. Deliberately NOT gated by `refuse_if_cycle_active` — a dispatched subagent MUST be able to run it (proven by the in-file fixture invoking it under `LAZY_CYCLE_SUBAGENT=1` and asserting exit 0). Coupled-CLI parity enforced in `lazy_parity_audit.py::audit_state_script_parity` (`_RESOLVE_REF_RE`). Baselines regenerated via `_normalize_smoke_output`; `docs/cli/cli-surface.json` regenerated via `cli_surface_gen.py`.
- **Integration note for Phase 2:** the landed CLI form is `python3 user/scripts/<lazy-state|bug-state>.py --repo-root <root> --resolve-ref <nonce>` — Phase 2's emitted first-step instruction must name this exact form (flag name + per-pipeline script) so it is copy-runnable.

### Phase 2 — HALTED on a design fork (NEEDS_INPUT.md, 2026-07-19)

WU-3 (self-resolving `@@lazy-ref` wrapper as the surfaced `cycle_prompt_ref`/`dispatch_prompt_ref`) cannot deliver end-to-end **without a PreToolUse dispatch-guard change** — a fork not visible at plan time. **Empirically verified this cycle** by invoking the real `lazy_guard.py` against a wrapped ref: the guard **denies** it (F2a `_REF_RE` full-matches only a BARE token) and does **NOT consume** nonce N, so `resolve_consumed_emission_by_nonce(N)` (WU-1) returns `None` — the subagent's `--resolve-ref N` would fail. The two requirements — the guard consuming N (needs a bare token) and the subagent seeing the resolve instruction (needs the wrapper in the prompt) — conflict unless `lazy_guard.py` is taught to extract+consume a token embedded in a wrapper. That is a security-relevant control-surface change **outside WU-3's plan file scope** and in tension with the SPEC's "guard's ALLOW+consume unaffected" Locked framing. Halted for the operator (`NEEDS_INPUT.md`, `class: product`, `divergence: structural`); recommended path (a): keep the guard unchanged, surface the bare token (guard consumes N as today), and move the resolve-first contract into WU-4's coupled prose + the subagent dispatch briefing. Phase 1 stands complete and independently valuable.
