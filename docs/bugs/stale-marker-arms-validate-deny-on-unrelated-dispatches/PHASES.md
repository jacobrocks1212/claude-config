...
**Deliverables:**
- [x] In `user/scripts/lazy_guard.py::guard()`, at the default-deny tail (the genuine no-route path, currently `_deny_and_ledger(_default_deny_reason(), …)` near line 780) AND/OR the consumed-by-other / stale-entry default-deny branches: when the live marker is UNBOUND (`marker.get("session_id") is None`), route the deny through `_deny_no_ledger(_default_deny_reason())` instead of `_deny_and_ledger(...)` — no ledger append, no hardening debt. The deny JSON shape is byte-identical (the bash wrapper is unaffected — a deny is a deny).
- [x] Scope the no-debt routing PRECISELY: it applies ONLY when the marker is unbound. A deny under a BOUND marker (the owning session dispatching an unregistered/mangled prompt — a genuine validate-deny / harness gap) MUST still `_deny_and_ledger` and accrue debt exactly as today. Do NOT broaden the no-debt path to bound-marker denies.
- [x] Preserve the hardening-cap and ref-token deny paths unchanged — only the GENERIC default-deny under an unbound marker becomes no-debt. (The bare-`@@lazy-ref` unresolved deny and the depth-1 hardening-cap deny keep their existing ledger semantics.)
- [x] Preserve fail-OPEN: any error reading `marker.get("session_id")` falls back to the existing `_deny_and_ledger` behavior (debt-preserving is the safe default — never silently drop a genuine gap).
- [x] Tests: unit/end-to-end fixtures proving a pre-bind (unbound-marker) unrelated deny writes NO ledger entry and does NOT raise `pending_hardening()`, while a bound-marker unregistered-prompt deny still ledgers and DOES raise `pending_hardening()`.

**Implementation Notes (2026-06-19):**
- **Work landed:** Added one DRY helper `_deny_default(marker, reason, *, tool_use_id, sha, prompt)` in `lazy_guard.py` (right after `_deny_and_ledger`): routes through `_deny_no_ledger(reason)` when `marker.get("session_id") is None` (unbound/pre-bind), else `_deny_and_ledger(...)`. Fail-OPEN: any error reading `marker.get(...)` falls back to `_deny_and_ledger` (debt-preserving). Routed the THREE generic `_default_deny_reason()` sites through it: the consumed-by-other (non-hardening) branch, the stale/expired-unconsumed (non-hardening) branch, and the final no-route default-deny. The depth-1 hardening-cap denies, the bare-`@@lazy-ref` unresolved deny, and the transcription-slip `_deny_no_ledger` are UNTOUCHED (the `marker` dict is guaranteed non-None at all three sites — `guard()` returns early when `marker is None`).
- **Scope precision proven by contrast fixtures:** unbound marker + unregistered → deny JSON, zero ledger rows, `pending_hardening()==0`; bound marker (owner) + unregistered → deny JSON, one ledger row, `pending_hardening()==1`; hardening-cap deny under an unbound marker STILL ledgers (scope guard — the cap is not broadened into no-debt).
- **Regression fan-out (pre-D2 deny tests required a bound marker now):** four pre-existing tests asserted a deny under an UNBOUND marker writes a ledger row — true pre-D2, now no-debt by design. Updated each to bind the marker + dispatch as the owner so they remain genuine validate-deny / debt-accruing tests (intent preserved): `test_hooks.py::test_guard_bash_deny_writes_deny_ledger`; `test_lazy_core.py::test_guard_deny_writes_ledger_entry`, `::test_f1b_in_body_edit_still_denies`, `::test_f1b_hardening_class_suffix_never_auto_readmits` (the hardening-suffix one lands on the GENERIC no-route path because the suffix changes the hash — NOT the cap — so it needed the same bound-marker fix).
- **Integration verification (end-to-end real bash hook):** an UNBOUND marker + unregistered dispatch through `lazy-dispatch-guard.sh` returns `permissionDecision: deny` with ZERO `lazy-deny-ledger.jsonl` rows — D1 (gate runs because unbound → present) + D2 (guard denies but no-debt) compose correctly.
- **Files:** `user/scripts/lazy_guard.py` (`_deny_default` helper + 3 routed sites); `user/scripts/test_hooks.py` (3 new D2 fixtures + 1 pre-existing test bound-marker fix); `user/scripts/test_lazy_core.py` (3 pre-existing tests bound-marker fix). No `lazy_core.py` schema change (D2-A reuses the existing `_deny_no_ledger` precedent — no new ledger field).
