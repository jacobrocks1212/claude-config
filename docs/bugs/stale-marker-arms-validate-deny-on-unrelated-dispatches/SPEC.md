...
| Marker ownership model (under-fire) — **SPLIT to follow-up (D3=A), out of scope here** | `user/scripts/lazy_core.py` (`read_run_marker` path B, `bind_marker_session`); `user/scripts/lazy_guard.py` (`_bind_marker_on_allow`) | Single mutable `session_id` slot; overwrite / wrong-session bind silently disarms the owner's guard. Addressed by the spun-off ownership-race bug, not this PHASES.md. |
| Regression net | `user/scripts/test_hooks.py`, `test_lazy_core.py`, `lazy-state.py --test` | New fixtures needed (over-fire scope): an unrelated non-owning-session dispatch must fast-path allow and NOT accrue debt; the owning session's guard must stay armed exactly as today; the unbound-marker pre-bind window must `_deny_no_ledger` (no debt). |

## Resolved Decisions (operator-resolved 2026-06-19 — was Open Questions)

These three product-class design forks were deferred to the planning step and resolved by the operator on 2026-06-19 (see `NEEDS_INPUT_RESOLVED_2026-06-19.md`). `/plan-bug` MUST author `PHASES.md` against these locked choices — do not re-litigate.

- **D1 — Discriminator mechanism → A. Owning-session scoping at the gate.** Pass `--session-id` from `lazy-dispatch-guard.sh` into `lazy-state.py --marker-present`, and have the `--marker-present` handler treat the marker as "present" ONLY when the caller's session is the marker's bound owner (re-enable `read_run_marker` staleness-path-B at the gate). A dispatch from the OWNING orchestrator session runs the full guard exactly as today; a dispatch from any OTHER session (including an interactive operator typing an unrelated spec prompt in the same repo) sees "no marker for me" → fast-path allow, never policed, never ledgered. The two reads (gate's `--marker-present` and the guard's own `read_run_marker(session_id=…)`) must agree. Preserve the sacred invariants: fail-OPEN on any error, never DESTROY the owning run's marker from a non-owner read (Phase 8 WU-8.1), never weaken the depth-1 hardening cap.
- **D2 — Disposition of an unrelated deny → A. Allow-through (gate scoped out).** Under D1, a non-owning-session dispatch never reaches the guard's deny logic at all (the gate fast-path-allows it) — no deny, no ledger entry, no debt. For the narrow unbound-marker pre-bind residual window (marker started, no allow yet → `session_id: None` → path B can't fire), PAIR with a `_deny_no_ledger`-style no-debt deny path so even a pre-bind deny carries no hardening debt. Both reuse existing in-codebase patterns (the transcription-slip `_deny_no_ledger` precedent).
- **D3 — Under-fire fix scope → A. Split the under-fire to a dedicated follow-up bug.** THIS bug is scoped to the over-fire ONLY (D1+D2: session-blind gate + no pipeline discriminator). The residual single-slot marker-ownership race (Symptom 2/3, Theory 2, Proven Finding #2) is spun off to a separate `docs/bugs/` item, cross-referenced in BOTH directions: this origin names the spin-off in `PHASES.md` Implementation Notes; the spin-off names this origin. The spin-off gets its own scoped investigation + deterministic fixture (the intermittent race is hard to fixture). The `/plan-bug` cycle that resolves this authors the spin-off and the reverse-references.

> **Scope note (D3):** Theory 2, Proven Finding #2, the under-fire Reproduction Steps, and the marker-ownership-model row of the Affected-Area table describe the SPLIT-OUT under-fire defect. They remain documented here as the origin record but are OUT OF SCOPE for this bug's `PHASES.md`, which addresses the over-fire (D1+D2) only.
