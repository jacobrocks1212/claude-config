...
- [ ] `CLAUDE.md` (repo root) — add a one-paragraph note in the Skills/Research-adjacent section: claude-config has no `docs/gemini-sprint/` staging; research resume is the direct `RESEARCH.md` drop. Reference `user/skills/ingest-research/SKILL.md` line ~65 ("per-repo adoption" — parameterize via `.claude/skill-config/gemini-sprint.md`) as the escape hatch for a future high-volume case.
- [ ] No change to `user/skills/ingest-research/SKILL.md` — its line ~65 already documents the per-repo adoption escape hatch (read-only reference only).
- [ ] Tests: none — documentation-only change, no executable behavior. Verification is a content read confirming both notes are present and accurate (see Minimum Verifiable Behavior).
