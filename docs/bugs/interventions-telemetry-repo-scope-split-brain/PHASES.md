...
**Deliverables:**
- [x] `drop_efficacy_breadcrumb(...)` accepts and records the covered repo-scope(s) in the breadcrumb payload (extends the current `{run_started_at, ts}` with a `covered_scopes: [<repo_key|root>]` field), written via `_atomic_write`, still marker-gated + fail-open (a write error → `False`, never wedges the flush).
- [x] `efficacy-eval.py` passes the scope it evaluated (its `--repo-root` → `repo_key`) into `drop_efficacy_breadcrumb` at `efficacy-eval.py:1114`; a two-scope run accumulates BOTH scopes across the trio invocations (breadcrumb read-merge-writes coverage, or the run-end gate reads the union of same-run breadcrumbs — implementation detail for `/write-plan`).
- [x] `efficacy_breadcrumb_present` (or a new coverage-aware sibling) returns satisfied ONLY when the interventions-bearing scope (`repo_key(<claude-config>)`) is covered for the CURRENT run (`run_started_at` match preserved); an invocation that covered only the target repo does NOT discharge the gate.
- [x] The `--run-end` gate (`lazy-state.py` + coupled `bug-state.py`) refuses (exit 1, marker + registry LEFT IN PLACE) with a corrective message naming the uncovered interventions scope when coverage is incomplete, exactly parallel to the unacked-hardening refusal; `--efficacy-skip-authorized` still overrides.
- [x] No-live-marker / on-demand invocation still leaves no residue and the gate stays moot (existing `_raw_marker_started_at is None → True/False` semantics preserved).
- [x] COUPLED PAIR: the `--run-end` gate change is mirrored in `lazy-state.py` and `bug-state.py`; run `python3 user/scripts/lazy_parity_audit.py --repo-root .` (exit 0).
- [x] Tests: `test_lazy_core.py` coverage for the coverage-bearing breadcrumb write/read; each script's in-file `--test` harness for the tightened `--run-end` gate (refuses on target-only coverage; passes on two-scope coverage; `--efficacy-skip-authorized` overrides; no-marker moot).
...
**Implementation Notes (2026-07-12):** Landed in two batches under Part 1.

- **WU-2 (breadcrumb coverage, commit `12b9d177`):** `drop_efficacy_breadcrumb(covered_repo_root=None, *, now=None)` now records `covered_scopes` (sorted `repo_key`s) and `interventions_covered` (bool — OR-accumulated across same-run calls) into the LIVE run marker's OWN keyed state dir (resolved by new `_run_marker_state_dir`: active dir if it holds a live marker, else most-recent live foreign keyed sibling — reusing WU-1's scan). Interventions-bearing detection is new `_repo_is_interventions_bearing` (`docs/features/queue.json` `"interventions": true` OR a `docs/interventions/*.md` record; fail-open False). When `covered_repo_root` is omitted it now prefers the marker's OWN recorded `repo_root` over the `active_repo_root()` cwd fallback (a divergent cwd must not be mistaken for the run's scope). `efficacy-eval.py:1114` passes `str(repo_root)`. Marker-gated + fail-open preserved.
- **WU-3 (scope-checking gate, this commit):** `efficacy_breadcrumb_present` is coverage-aware — on a `run_started_at`-matching breadcrumb it returns True ONLY when `interventions_covered is True` (a flush that only ever touched non-interventions-bearing scopes no longer discharges the gate). No-live-marker MOOT (True) and `run_started_at`-mismatch (False) paths byte-identical. The `--run-end` efficacy gate refusal message + telemetry `gate`/`reason` in `lazy-state.py` AND `bug-state.py` (coupled-pair, byte-identical mirror) now name the interventions-bearing-scope coverage requirement and carry the `[interventions-telemetry-repo-scope-split-brain]` tag; `--efficacy-skip-authorized` still overrides.
- **Gates:** `test_lazy_core.py` efficacy subset 14 passed (new `test_efficacy_breadcrumb_present_requires_interventions_coverage` RED→GREEN); `bug-state.py --test` all passed; `lazy_parity_audit.py --repo-root .` exit 0; `efficacy-eval.py --repo-root . --dry-run` exit 0. In-file `--test` gate fixtures use `--efficacy-skip-authorized` (bypass the gate), so the refusal-message edit left `--test` output byte-identical — no baseline regeneration needed. `lazy-state.py --test`'s lone `[apply-pseudo-provisional-refusal] SystemExit: 3` is the known C3 cycle-containment artifact of running under an active cycle marker (a fixture spawns a real `--apply-pseudo` subprocess without `LAZY_STATE_DIR`), NOT a WU-3 regression. `test_lazy_core.py` full suite retains its 75 pre-existing `test_mark_*` failures documented in Phase 1 (WU-3 adds zero new).
- **Intermediate-state note (carried to Part 2):** the tightened gate + orchestrator two-scope flush (Phase 3, Part 2) are designed to ship together. Because the gate keys on the breadcrumb's `interventions_covered` flag, a run whose target repo IS interventions-bearing (claude-config's OWN runs) self-covers and still passes; a run in a non-interventions target (e.g. AlgoBooth) will refuse at `--run-end` until Phase 3's second `--repo-root <claude-config>` flush lands (or `--efficacy-skip-authorized`). The state machine routes to Part 2 immediately after this part flips Complete.
...
**Deliverables:**
- [x] `user/skills/lazy-batch/SKILL.md` §1c.6: the end-of-run flush additionally runs the trio (`efficacy-eval.py` review + `--canary`, and — where applicable — the interventions-bearing evaluation) with `--repo-root <claude-config>` (path resolvable from the skill's config table), NOT only `--repo-root .` in the target repo. State the two-scope contract explicitly (the claude-config-rooted evaluation sees originating telemetry via the Phase-1 merged read; both scopes are attested in the breadcrumb per Phase 2). Doc-write/commit ownership unchanged; NON-BLOCKING failure posture (`⚠ efficacy-eval failed`) preserved.
- [x] Mirror the identical two-scope flush prose into `user/skills/lazy-bug-batch/SKILL.md` and `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md`, honoring each sibling's tabulated divergences (cloud's committed-segment path; the bug orchestrator's §1c.6 analog).
- [x] Update each skill's coupled-pair "Differences" / State Machine notes if the flush shape changed the dispatch surface.
- [x] Re-project + lint: `python3 ~/.claude/scripts/project-skills.py` then `python3 ~/.claude/scripts/lint-skills.py` (clean); `python3 user/scripts/doc-drift-lint.py --repo-root .` (exit 0) if the Hooks/Coupled-pairs tables are touched.
...
**Implementation Notes (2026-07-12, Part 2 WU-4):** Added a dedicated **"TWO-SCOPE flush"** paragraph to each orchestrator's §1c.6 end-of-run flush: after the existing `--repo-root .` efficacy + canary pair, run BOTH evaluators a SECOND time rooted at the claude-config checkout (`--repo-root ~/source/repos/claude-config` workstation; the cloud container's claude-config checkout for `/lazy-batch-cloud`). The paragraph states the contract identically across the trio: records live only in claude-config (follow the FIX) while telemetry accrues in the target repo's keyed dir (follows the RUN); the claude-config-rooted read merges the target-repo telemetry via the Phase-1 merged read; both scopes are attested in the Phase-2 `interventions_covered` breadcrumb (which discharges the tightened `--run-end` gate — a target-only flush no longer satisfies it); when the run's own repo IS claude-config the second invocation is an idempotent no-op; doc-write/commit ownership + the NON-BLOCKING `⚠ efficacy-eval failed` posture are preserved (the second scope's `docs/interventions/` updates commit in the claude-config checkout tree). `lazy-batch` = full paragraph; `lazy-bug-batch` = a mirrored two-scope sentence appended to its compressed §1c.6 reference (pipeline-agnostic); `lazy-batch-cloud` = full paragraph honoring the cloud committed-segment divergence (`docs/telemetry/cloud/*.jsonl`) + its Differences-table efficacy row updated to name the two-scope addition. Gates: `project-skills.py` clean (88 skills / 97 components, no errors), `lint-skills.py` OK (no broken `!cat`), `doc-drift-lint.py --repo-root .` exit 0 (0 findings — the Hooks/Coupled-pairs tables were NOT touched).
