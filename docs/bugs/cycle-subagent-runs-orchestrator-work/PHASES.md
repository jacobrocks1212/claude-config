...
**Deliverables:**
- [x] In `user/scripts/lazy_core.py`, add a marker-clear guard helper (`refuse_cycle_marker_mutation_if_subagent(op_name)`) that refuses (exit 3, corrective stderr, zero side effects) when `LAZY_ORCHESTRATOR` is NOT truthy AND a subagent context is indicated (explicit `LAZY_CYCLE_SUBAGENT`, OR — the reachable signal — the absence of `LAZY_ORCHESTRATOR` while a cycle marker is present). Documented why `--cycle-end` cannot use the plain `refuse_if_cycle_active` (orchestrator clears under a live marker).
- [x] Wire the guard at the ENTRY of the `--cycle-end` handler in `user/scripts/lazy-state.py` BEFORE `cycle_end_friction_check` / `clear_cycle_marker`, and mirror into `user/scripts/bug-state.py`.
- [x] Apply the same guard to the `--cycle-begin` handler in both scripts — a subagent must not arm/re-arm a marker either. Preserve the orchestrator's self-healing overwrite (it asserts `LAZY_ORCHESTRATOR`, so it is never refused).
- [x] In `user/hooks/lazy-cycle-containment.sh`, add `--cycle-end` and `--cycle-begin` to the `LOOP_FORMATION_FLAGS` tuple so the `agent_id` subagent trip denies them.
- [x] Keep `CYCLE_REFUSED_OPS` lockstep: documented in the module comment that `--cycle-end`/`--cycle-begin` are guarded by the dedicated marker-mutation helper rather than added to `CYCLE_REFUSED_OPS` (whose members use the marker-fallback that `--cycle-end` cannot). The C2/C3 deny *scope* still matches: subagent → cannot clear/arm the marker.
- [x] Tests: `lazy_core` unit tests for the new guard (9 — orchestrator allowed, subagent refused with exit 3 and marker untouched, priority order, zero-side-effects); new `--test` fixtures in BOTH `lazy-state.py` and `bug-state.py` asserting a subagent-context `--cycle-end`/`--cycle-begin` is refused (exit 3) and the marker file still exists afterward, plus orchestrator-allowed pairs; 3 hook tests asserting the `agent_id`-present payload denies `lazy-state.py`/`bug-state.py --cycle-end`/`--cycle-begin` and the main-thread payload allows.
...
- [x] `python user/scripts/lazy-state.py --test` and `python user/scripts/bug-state.py --test` green (new refusal fixtures included); byte baselines re-pinned via `_normalize_smoke_output`.
- [x] `python -m pytest user/scripts/ -q` green (lazy_core guard tests + hook test).
- [x] Manual hook check: piped a `{"agent_id":"x","tool_name":"Bash","tool_input":{"command":"python3 lazy-state.py --cycle-end"}}` payload through `lazy-cycle-containment.sh` → `permissionDecision: deny`; the same payload WITHOUT `agent_id` emitted no decision (allow). Confirmed 2026-06-16.

**Implementation Notes (2026-06-16, inline cycle subagent — no Agent tool, test-first per WU):**
- `refuse_cycle_marker_mutation_if_subagent(op_name)` added to `lazy_core.py` (priority: LAZY_ORCHESTRATOR allow → LAZY_CYCLE_SUBAGENT refuse → marker-present-without-orch-env refuse → else noop). Wired at the ENTRY of `--cycle-begin`/`--cycle-end` in BOTH `lazy-state.py` and `bug-state.py`, before any marker write/clear. `--cycle-end`/`--cycle-begin` added to the C2 hook `LOOP_FORMATION_FLAGS`. Module comment in `lazy_core.py` documents why these are NOT in `CYCLE_REFUSED_OPS` (they run while the orchestrator's own marker is present) and that C2/C3 deny scope stays lockstep.
- New subprocess `--test` fixtures (`cycle-marker-mutation-guard`) in both state machines drive the real CLI handler: subagent (no LAZY_ORCHESTRATOR, marker on disk) `--cycle-end`/`--cycle-begin` → exit 3 + marker survives; orchestrator (LAZY_ORCHESTRATOR=1) → exit 0, clears/overwrites. Both byte baselines re-pinned via the `_normalize_smoke_output` helper (not by hand).
- **SPEC Open Question RESOLVED (agent_id on Bash PreToolUse):** the hook's own header + the existing D4 hook tests document that Claude Code injects `agent_id` ONLY when the PreToolUse hook fires from within a subagent (confirmed against the installed hook-input schema). So the C2 hook flag-set DOES fire for a subagent's `lazy-state.py`/`bug-state.py` Bash calls (verified by the manual check + the new hook tests). However, the **load-bearing carrier is the C3 env-var guard (WU-3/WU-4)**: a Python subprocess (lazy-state.py invoked from a subagent's Bash) cannot read the hook-input `agent_id`, so C3 keys on the reachable signals (LAZY_ORCHESTRATOR absent + marker present, or explicit LAZY_CYCLE_SUBAGENT). The C2 hook flag-set is the secondary, belt-and-suspenders layer. Do NOT regress to a marker-only assumption.
- `lazy_parity_audit.py` clean (exit 0) — both twins mirror the guard wiring.
