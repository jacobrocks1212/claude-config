...
3. **The cycle prompt raises the trigger rate by teaching the behavior (Theory 5).** The prompt frames the subagent as a pipeline operator and enumerates the orchestrator-only commands in order to forbid them — handing it the exact playbook. This does not *cause* the bug (the mechanical guards do or don't hold regardless), but it is why the idea is available to a model under variance. A complementary prompt-side mitigation lowers occurrence frequency; it is **not** a substitute for the mechanical fix (a determined model can still read the skills on disk).
...
4. **(Complementary, prompt-side — lowers trigger rate, NOT load-bearing) Stop teaching the subagent the orchestrator's playbook (Theory 5 / Finding 3).** Re-shape `cycle-base-prompt.md` so the cycle prompt reads as a one-shot *worker* brief, not a pipeline-*operator* brief: (a) drop / soften the *"You are advancing one cycle of the autonomous feature pipeline"* opening in favor of *"Run exactly one skill, then stop"*; (b) in `TERMINAL STOP`, state the boundary **without enumerating the orchestrator API** — e.g. "Do not run pipeline/orchestration commands or invoke any `/lazy*` skill; if you finish, are blocked, or are unsure, write your report and STOP" — rather than naming `--run-start`/`--run-end`/`--apply-pseudo`/etc., which doubles as a how-to. Keep the explicit "the harness will DENY these in-flight" assurance (now true once fixes #1–#3 land) but without the command list. This is occurrence-rate reduction layered on top of the mechanical guards — a model can still discover the commands by reading the skills on disk, so it MUST NOT be shipped as the sole or primary fix.
