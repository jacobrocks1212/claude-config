...
**Deliverables:**
- [x] Add an "over-fit detector" subsection to `user/skills/harden-harness/SKILL.md` Step 3, enumerating the four smell signals (any one trips a spin-off): (1) the fix adds a literal phrase/string to a matcher (regex / header list / keyword set) — fitting to observed data, not structure; (2) the root-cause class has recurred ≥2 times in the hardening log (signature match against prior rounds); (3) the agent self-flags the fix as narrow ("this will gap again on the next variant"); (4) the friction is a repeated deterministic dance (toolify candidate per the framework's promotion checklist — in-run detection, NOT an offline-miner call).
- [x] Document the **recurrence threshold** resolved in this cycle (Open Question 1): phrase-match patches spin off on FIRST occurrence; non-phrase recurrence needs ≥2. Make the threshold explicit in the over-fit detector prose.
- [x] Document the **generalization bound** discipline ("most general within reason"): the spun-off spec targets the smallest class that subsumes the observed instance + near neighbors — NOT a speculative rewrite; the problem statement MUST cite the concrete instance(s) as evidence and name the class boundary explicitly.
- [x] Add the **spin-off action** prose: compose a generalized problem statement (the class, not the instance), then invoke `/spec` or `/spec-bug` via `adhoc-enqueue` front-enqueued. Choice rule: structural redesigns + new capabilities → `/spec`; defects/regressions + toolify-this-dance → `/spec-bug`. Reference `_components/adhoc-enqueue.md`'s `--type bug` path for the bug-pipeline route.
- [x] Add the **no-double-blocking** + **self-recursion-guard-preserved** notes: the instance is already fixed so the spin-off never blocks the current run (queued work, surfaced via HARDENING.md + `PushNotification`); a spin-off is a `/spec`/`/spec-bug` enqueue (NOT a recursive hardening dispatch) so it does not trip the existing depth-1 hardening guard.
- [x] Extend the Step 4 HARDENING.md round template to record BOTH the patch AND the spin-off (a `harden(spinoff):` note line + the front-enqueued item id) when the smell trips; extend the Step "Return format" with a `spinoff` field (id + reason, or `none`).
- [x] Wire the toolify-candidate trigger to the upstream framework's deterministic-only bar + promotion checklist (cite `docs/features/unified-pipeline-orchestrator/toolify-bar.md` step 7); state explicitly that harden-harness does its own in-run dance-recurrence detection and spins off a `/spec-bug` per the checklist — it does NOT shell `toolify-miner.py` mid-cycle.
- [x] Tests: in `user/scripts/test_lazy_core.py`, add a deterministic over-fit-detector fixture suite (registered in `_TESTS` — see Phase 5 guard) asserting: (a) a phrase-match fix yields a spin-off decision on first occurrence; (b) a structural (non-phrase) fix with no prior recurrence yields NO spin-off; (c) a non-phrase fix whose class recurred ≥2 in a synthetic hardening-log fixture yields a spin-off; (d) a toolify-dance signal yields a `/spec-bug` spin-off. (If the detector logic is purely prose with no extractable helper, instead add a `lint-skills.py`-checkable assertion that the four signals + threshold + generalization-bound + spin-off-choice-rule strings are present in the SKILL.md — a presence gate, since this repo's MCP gate is operator-exempt and the behavior is LLM-executed prose.)

#### Implementation Notes — Phase 1 (2026-06-17)

**Work completed.** Authored the over-fit detector into `harden-harness/SKILL.md` Step 3 (a new `#### Over-fit detector` subsection placed AFTER the mechanical-fix path so the immediate fix always lands first / never blocks the run). All four smell signals, the SPEC-Open-Question-1 recurrence threshold (phrase-match → first occurrence; non-phrase → ≥2), the generalization-bound discipline (smallest class + cite instance + name class boundary), the `/spec` vs `/spec-bug` choice rule via the `adhoc-enqueue` front-enqueue path, no-double-blocking, and self-recursion-guard-preserved are all in prose. Extended the Step 4 round template with an `**Over-fit spin-off:**` record line (records BOTH patch AND spin-off) and the Return format with a `spinoff` field; updated Outputs item 2 to note the dual emission.

**Test (WU-2).** Since the detector is LLM-executed prose (this repo's MCP gate is operator-exempt), authored the **presence gate** variant: `test_harden_harness_overfit_prose_present` in `test_lazy_core.py`, registered in `_TESTS`, asserting all four signals + threshold + generalization bound + choice rule + adhoc-enqueue + no-double-blocking + self-recursion-guard + the Step-4 round line + the Return `spinoff` field are findable in SKILL.md. Reused the existing `_HARDEN_SKILL_PATH` helper.

**Integration notes.** The `**Over-fit spin-off:**` round line + the Return `spinoff` field are the surfacing contract (later observability keys off them). The spin-off mechanism CONSUMES `adhoc-enqueue.md --type bug` (upstream-owned, not re-implemented) — completing the loop `unified-pipeline-orchestrator` left open at `toolify-bar.md` step 7.

**Pitfalls.** None. Wired the toolify trigger as in-run dance-recurrence detection (NOT an offline `toolify-miner.py` call mid-cycle) per the cross-feature integration note.

**Files modified:** `user/skills/harden-harness/SKILL.md`, `user/scripts/test_lazy_core.py`.
...
**Deliverables:**
- [x] Define the canonical marker as a single Python constant in `user/scripts/lazy_core.py` (e.g. `_VERIFICATION_ONLY_MARKER = "<!-- verification-only -->"`) — the SSOT. Resolve Open Question 2 (per-row HTML-comment marker vs single canonical subsection header) toward the per-row `<!-- verification-only -->` comment form, since it is the most robustly machine-detectable in `remaining_unchecked_are_verification_only` (a row carries its own exemption marker — no heading-scope bookkeeping) and survives novel subsection phrasing by construction. ⚖ if `check-docs-consistency.ts` (AlgoBooth) cannot validate the comment form cleanly, fall back to the canonical subsection-header form — document which was chosen and why in the constant's docstring.
- [x] Rekey `remaining_unchecked_are_verification_only(phases_text)` to treat a `- [ ]` row as verification-exempt when the row (or its enclosing subsection) carries the canonical marker, INDEPENDENT of the bold-header/heading free-text. Preserve the existing fence-awareness and Superseded-phase handling.
- [x] Reduce `_VERIFICATION_SECTION_RE` to a **deprecation shim**: keep the regex but, when it matches a header while the canonical marker is ABSENT from that subsection's rows, surface a diagnostic (append to the `_DIAGNOSTICS` list / emit a warning string) naming the un-migrated producer — do NOT silently pass. The shim still exempts the rows (no regression for un-migrated PHASES.md) but makes the gap visible.
- [x] Update `_components/phases-runtime-verification.md` so `/spec-phases`-authored verification rows emit the canonical marker (reference the SSOT constant by value, with a note pointing at `lazy_core.py`).
- [x] Update `_components/blocked-resolution.md` so `/blocked-resolution`-authored verification / full-chain-seam-audit rows emit the canonical marker.
- [x] Tests (registered in `_TESTS`): (a) a verification subsection with a NEVER-BEFORE-SEEN header text + the marker present → gate passes via marker, no regex growth needed; (b) a verification subsection WITHOUT the marker (un-migrated) → deprecation shim warns (diagnostic emitted), does NOT silently pass clean; (c) lockstep test asserting the marker string referenced in the component prose equals the `lazy_core.py` SSOT constant.

#### Implementation Notes — Phase 2 (2026-06-17)

**Work completed.** Defined `_VERIFICATION_ONLY_MARKER = "<!-- verification-only -->"` as the SSOT constant in `lazy_core.py` (Open Question 2 resolved toward the per-row HTML-comment form; rationale + the `check-docs-consistency.ts` fallback documented in the constant's docstring). Rekeyed `remaining_unchecked_are_verification_only` so a `- [ ]` row is exempt when the ROW or its enclosing SUBSECTION HEADER carries the marker — independent of header free text — with the legacy regex demoted to a deprecation shim that appends a `_DIAGNOSTICS` warning (de-duped per header) when a row is exempted by header-text alone with the marker absent. Fence-awareness and Superseded-phase handling preserved. Producers updated: `phases-runtime-verification.md` (RV + reachability-smoke example rows emit the marker; the old REGEX-LOCKSTEP note rewritten to MARKER-FIRST) and `blocked-resolution.md` (seam-audit / RV escalation rows emit the marker), both referencing the SSOT constant by name.

**Tests (WU-5, all in `_TESTS`).** (a) `test_ruvonly_novel_header_with_marker_passes` — never-before-seen header + per-row marker → True (no regex growth); (b) `test_ruvonly_novel_header_without_marker_warns_and_fails` — regex-matched header, no marker → still exempt (no regression) BUT a `_DIAGNOSTICS` warning is emitted; (c) `test_ruvonly_marker_lockstep_producers_match_ssot` — both producer components reference the marker value AND name the SSOT constant. Plus `test_verification_only_marker_constant_present` (constant exists, per-row form). Test (a) was authored RED before WU-3 (returned False without marker support) — genuine RED-for-the-right-reason.

**Integration notes.** The SSOT constant is the contract — never re-hardcode the marker in a producer; always reference `lazy_core:_VERIFICATION_ONLY_MARKER`. The deprecation shim's `_DIAGNOSTICS` warning is the migration tracker; a future cycle retires the regex once it stops firing across live PHASES.md. The marker is a row annotation, not a sentinel, so it does NOT enter AlgoBooth's `SENTINEL_SCHEMAS` — no `check-docs-consistency.ts` lockstep surfaced (a harness agent cannot edit the target repo anyway; flagged here per plan rule 12 — none required).

**Pitfalls.** Initial implementation called `add_diagnostic` (wrong name) — the shared helper is `_diag`; corrected. Full lazy-family gate run (shared `lazy_core.py`): `test_lazy_core.py` 444/444, `lazy-state.py --test` OK, `bug-state.py --test` OK, `lazy_parity_audit.py --repo-root .` exit 0, lint + project-skills clean.

**Files modified:** `user/scripts/lazy_core.py`, `user/skills/_components/phases-runtime-verification.md`, `user/skills/_components/blocked-resolution.md`, `user/scripts/test_lazy_core.py`, `user/scripts/CLAUDE.md`.
...
**Deliverables:**
- [x] In `verify_ledger`'s feature-level branch (`scoped is False`), treat "no implementation plan present (and none required)" as NOT-a-failure: when `find_implementation_plans(spec_path)` returns zero incomplete plans AND `_has_any_complete_plan` is False AND there is genuinely no implementation plan on disk (only `realign-*.md` / `retro-*.md` / no plans at all), set `plan_complete = True` (absent-by-design) rather than False. Preserve the existing True/False behavior for features that DO have implementation plans (≥1 complete + zero incomplete → True; any incomplete → False).
- [x] Verify the plan-SCOPED branch (`scoped is True`, `--plan <part>`) is unaffected — it reads the named plan's own frontmatter status and is correct as-is; the fix is feature-level only.
- [x] Add a diagnostic so the operator can see the absent-by-design path fired (e.g. a `_DIAGNOSTICS` note "plan_complete: no implementation plan required (absent-by-design)" — additive, never gates).
- [x] Tests (registered in `_TESTS`): (a) a plan-less feature (PHASES.md present, all deliverables checked, NO `plans/*.md` implementation plan) → `verify_ledger(...)` returns `plan_complete:true` and `ok:true` (given clean tree / head match stubbed); (b) a realign-only feature (only `plans/realign-*.md` present, no implementation plan) → `plan_complete:true`; (c) regression guard: a feature WITH an incomplete implementation plan still returns `plan_complete:false` (the fix must not vacuously pass real incomplete plans).

#### Implementation Notes — Phase 3 (2026-06-17)

**Work completed.** Added the **absent-by-design** branch to `verify_ledger`'s feature-level (`scoped is False`) `plan_complete` check in `lazy_core.py`: when the prior rule yields False AND there are zero incomplete plans AND no Complete plan AND `_implementation_plans_exist(spec_path)` is False, set `plan_complete = True` and emit the `_DIAGNOSTICS` note "plan_complete: no implementation plan required (absent-by-design)". Added the new `_implementation_plans_exist` helper — pure existence check that excludes `realign-*`/`retro-*` and the legacy `PLAN.md`, and does NOT filter by status (so an incomplete implementation plan is correctly detected as *present* → regression guard holds). The plan-SCOPED branch (`scoped is True`) is untouched.

**Tests (WU-2, all in `_TESTS`).** (a) `test_verify_ledger_plan_less_feature_absent_by_design_passes` — no `plans/` dir → `plan_complete:true`, `ok:true`; (b) `test_verify_ledger_realign_only_feature_absent_by_design_passes` — only `plans/realign-*.md` → `plan_complete:true`; (c) `test_verify_ledger_incomplete_plan_still_fails_regression_guard` — a `status: Ready` implementation plan still returns `plan_complete:false`, `failing_check='plan_complete'`. Tests (a)+(b) were authored RED before WU-1 (pre-fix branch returned False); (c) was green both before and after (genuine regression guard, non-vacuous).

**Pitfalls.** None. Given the guards (`incomplete_plans == 0 AND not any_complete`), `_implementation_plans_exist` is necessarily False in this branch, but it is kept as an explicit, self-documenting existence assertion matching the spec's "genuinely no implementation plan on disk" wording rather than relying on the implication.
...
**Deliverables:**
- [x] Enumerate (Open Question 3) the exact script-observable conditions that force Sonnet: (1) the resolved scenario is a legacy `.md` and has NO converted `corpus/live/*.yaml` counterpart (first-run conversion needed); (2) the prior verdict was non-definitive (`uncertain` / unrepaired `harness` / `genuine`-after-heal) per a recorded prior `verdict.json` / `MCP_TEST_RESULTS.md`; (3) no scenario exists at all (scenario-authoring needed). Ready-to-run converted YAML with no adverse prior verdict → haiku.
- [x] Add a routing helper to `user/scripts/surface_resolver.py` (e.g. `route_mcp_test_tier(scenario_path, prior_verdict=None, yaml_exists=None) -> "haiku" | "sonnet"`) implementing those conditions deterministically — pure function, no I/O beyond an optional existence check, so it is hermetically testable.
- [x] Update `repos/algobooth/.claude/skills/mcp-test/SKILL.md` "Model tier" section so the tier is chosen by the script signal (cite the helper), and the conditions are documented in prose; remove/retire the reliance on an orchestrator override for the diagnosis/authoring case. Note explicitly that a `.md`-unconverted or prior-non-definitive scenario routes to Sonnet WITHOUT a human/orchestrator call.
- [x] Tests in `user/scripts/test_surface_resolver.py` (its own runner) covering each routing condition: ready YAML + clean prior → haiku; `.md` unconverted → sonnet; prior `uncertain` verdict → sonnet; no scenario → sonnet.

#### Implementation Notes — Phase 4 (2026-06-17)

**Work completed.** Added `route_mcp_test_tier(scenario_path, prior_verdict=None, yaml_exists=None) -> "haiku" | "sonnet"` to `surface_resolver.py` — a pure function whose only I/O is the optional existence check when `yaml_exists is None`. The three Sonnet-forcing conditions (Open Question 3) are enumerated in the docstring: (1) unconverted legacy `.md` (no `.yaml` counterpart), (2) non-definitive prior verdict, (3) no scenario at all. Conditions 1 + 3 collapse to one check ("no ready converted YAML present"). Prior-verdict definitiveness is an ALLOW-LIST (`_DEFINITIVE_MCP_VERDICTS` = all-passing/passing/pass/all_passing) so an unrecognized verdict label fails safe toward Sonnet. WU-4: rewrote the `mcp-test/SKILL.md` Model-tier section to consult the helper (cited by path + symlink), documenting the three conditions and stating explicitly that a `.md`-unconverted or prior-non-definitive scenario routes to Sonnet WITHOUT a human/orchestrator call; `model: haiku` frontmatter kept (haiku stays the happy-path default, the helper escalates).

**Tests (WU-3, in `test_surface_resolver.py` `TestRouteMcpTestTier`).** ready YAML + clean prior → haiku; `yaml_exists=True` override → haiku; legacy `.md` unconverted → sonnet; parametrized non-definitive priors (uncertain/harness/genuine) → sonnet; no scenario at all → sonnet; definitive prior (all-passing) is NOT adverse → haiku. Authored RED first (ImportError on the missing helper), GREEN after the implementation. Suite: 39 passed.

**Pitfalls.** None. `mcp-test/SKILL.md` is AlgoBooth-repo-scoped prose (not a coupled pair → no mirroring) but IS picked up by `project-skills.py` per-repo projection — re-ran it (clean) + lint after editing.

**Files modified:** `user/scripts/surface_resolver.py`, `user/scripts/test_surface_resolver.py`, `repos/algobooth/.claude/skills/mcp-test/SKILL.md`, `user/scripts/CLAUDE.md`.
...
**Deliverables:**
- [x] Add a dead-coverage guard: a function (in `test_lazy_core.py`'s own suite, OR a small standalone `user/scripts/test_coverage_guard.py`) that AST-parses (or reliably greps) `test_lazy_core.py` for every top-level `def test_*` name, compares against the set of names registered in `_TESTS`, and FAILS naming any orphaned (defined-but-unregistered) test function. Choose the AST approach (`ast.parse` over the module source) for robustness over a regex.
- [x] Wire the guard so it runs as part of the standard gate: register it in `_TESTS` itself (self-checking — the guard is collected and run by the same harness it guards) so `python user/scripts/test_lazy_core.py` fails on an orphan. ⚖ policy: guard scope (this-module vs all script test modules) → cover `test_lazy_core.py` (the incident's module + the suite Phases 1-3 extend) as the load-bearing case; note the generalization seam to other `_TESTS`-style modules in a docstring rather than speculatively scanning every file.
- [x] Surface the guard in `.claude/skill-config/quality-gates.md` (and/or `harden-harness/SKILL.md` Step 3 gate list) so a hardening round's gate run includes it — making "tests that never execute" impossible to land silently.
- [x] Tests: (a) a positive test confirming the guard PASSES on the current (correctly-registered) suite; (b) a negative fixture (a synthetic module text with a `def test_orphan` not in its `_TESTS`) confirming the guard DETECTS and reports the orphan by name.

#### Implementation Notes — Phase 5 (2026-06-17)

**Work completed.** Added the dead-coverage guard to `test_lazy_core.py`: a pure collector `_collect_orphaned_test_names(module_source, registered_names)` (`ast.parse` over the module source — robust over regex; top-level only) and the self-checking guard `test_no_orphaned_test_functions` (registered in `_TESTS`, so `python user/scripts/test_lazy_core.py` fails by name on any orphan). The collector EXCLUDES parameterized (pytest-fixture) tests — a `def test_x(tmp_path)` runs under `pytest user/scripts/ -q`, not the manual `_TESTS` runner, so its absence from `_TESTS` is correct, not dead coverage. WU-6: documented the guard in `.claude/skill-config/quality-gates.md` (Python-script-changes gate) and a `harden-harness/SKILL.md` Step 3 note (`test_lazy_core.py` was already in the mandatory gate list).

**Dead-coverage backlog surfaced + fixed (D7 — most-complete in-cycle).** Registering the guard immediately surfaced **98 pre-existing zero-arg `def test_*` functions** authored by prior features but never appended to `_TESTS` — the exact Round-24 dead-coverage class the guard exists to catch (plus 6 parameterized pytest-native tests correctly NOT counted). All 98 pass standalone; I wired them into a dedicated `_TESTS` block so the guard PASSES on a genuinely-clean suite rather than a baseline of tolerated orphans. Suite grew 449 → 547 manual-runner tests (all green). ⚖ policy: pre-existing dead-coverage backlog (tolerate vs fix in-cycle) → fixed in-cycle (98 orphans registered, all passing) — leaving them would have RED-gated the very guard this phase lands.

**Tests (WU-5, in `_TESTS`).** (a) `test_no_orphaned_test_functions` — guard passes on the real fully-registered suite (this is the integration proof that Phases 1-4 registered their tests); (b) `test_dead_coverage_guard_detects_orphan_by_name` — synthetic source with an unregistered `def test_orphan` → collector reports `test_orphan` by name. Authored RED first (the guard initially flagged the 103-orphan backlog), GREEN after the parameterized-exclusion fix + backlog registration.

**Pitfalls.** The first guard pass flagged 103 orphans — 5-6 of them parameterized pytest tests that are NOT manual-runner candidates (false positives). Fixed by excluding any `def test_*` declaring args/posonlyargs/kwonlyargs. The remaining 98 were genuine dead coverage and were registered.

**Files modified:** `user/scripts/test_lazy_core.py`, `.claude/skill-config/quality-gates.md`, `user/skills/harden-harness/SKILL.md`.
