...
- [x] When the queue exhausts and only budget-deferred/evicted items remain, return an honest terminal (reuse / parallel the `queue-exhausted-all-parked` pattern) — NOT a false `all-features-complete`.
- [x] Tests: `lazy-state.py --test` fixtures — (a) ceiling-formula across varying `max_cycles`×`Q_depth` (floor-of-6 honored on small runs, ≤40% cap honored on deep queues, `--per-feature-cycle-cap` overrides); (b) guard trips at computed ceiling and the next ready item is dispatched with the feature reordered to tail; (c) bounded re-trip → eviction on 2nd trip (does NOT loop indefinitely).
...
**Runtime Verification** *(checked by integration test or manual testing):*
- [x] `lazy-state.py --test` shows the ceiling/trip/defer/evict fixtures PASS <!-- verification-only -->
- [x] `compute_per_feature_ceiling` characterized directly in `test_lazy_core.py` (boundary cases: `Q_depth==0`, `max_cycles<15` forcing the 6 floor, deep queue forcing the 40% cap) <!-- verification-only -->
...
**Implementation Notes (2026-06-19/20, Phase 2 landed — INLINE execution under /lazy-batch, no Agent calls):**
- **Ceiling fn (WU-3):** `lazy_core.compute_per_feature_ceiling(max_cycles, ready_queue_depth, override=None)` — `override` short-circuits VERBATIM (incl. a deliberate `0`; only `None` falls through). Formula `max(6, min(C*4//10, (C//Q)*2))`; `Q<=0 → 6` floor (no div-by-zero). Pure. 6 characterization tests in `test_lazy_core.py` (override / 6-floor / deep-queue-6 / 40%-cap-arm / Q==0 / purity).
- **Flag (WU-3):** `--per-feature-cycle-cap N` on `lazy-state.py` argparse → appended `compute_state(... per_feature_cycle_cap=None)` kwarg. MIRRORED on `bug-state.py` as a benign parity parse (bug pipeline does NOT trip in v1; param accepted+ignored, `_ = per_feature_cycle_cap`) — matches the `--type bug` precedent.
- **Trip eval (WU-4):** the guard is MARKER-GATED — `compute_state` reads the run marker via `read_run_marker()`; absent a marker (single `/lazy`, hermetic fixtures without a pinned marker) it NEVER trips and output is byte-identical. The trip branch is the FINAL skip branch in the queue-selection loop (after completion/cloud/device/research/park), mirroring the `--park-*` skip-list shape: on `count >= ceiling` it appends to the run-scoped `_DEFERRED_BUDGET` and `continue`s — a run-scoped reorder that does NOT touch `queue.json` (asserted by the fixture).
- **Re-trip ladder (WU-4):** marker field `budget_deferred: {feature_id: int}` records deferral count. 1st trip (prior_defers < 1) → `action: "defer"`, bump count to 1, re-enters at tail once. 2nd trip on the same feature in the same run → `action: "evict"`, append to marker `budget_evicted: []`; an evicted feature is skipped at the top of the loop for the rest of the run (removed from the live queue, on-disk progress preserved). Bounded — cannot loop indefinitely.
- **Probe field (WU-4):** `budget_guard: {feature_id, count_at_trip, computed_ceiling, action, next_id, sub_skill_phase, commit_hash}` folded into the probe JSON ONLY when a trip fired this probe (else the key is absent — same byte-identical discipline as `parked`). `next_id` back-filled with the dispatched feature; `sub_skill_phase` is the tripped feature's first In-progress (else first) PHASES heading (cheap, best-effort audit context — ⚖ policy: budget_guard.sub_skill_phase value → cheap PHASES current-phase, audit-only no product-behavior difference); `commit_hash` via new fail-open `lazy_core.git_head_short_sha(repo_root)`.
- **Exhaustion terminal (WU-4):** new distinct terminal `queue-exhausted-budget-deferred` (parallel to `queue-exhausted-all-parked`) when `current is None` and the only items skipped were budget-deferred/evicted — NOT `all-features-complete`. Placed AFTER the specific global terminals (cloud/device/research/scoped-id keep precedence) and BEFORE the all-parked/all-complete fallbacks.
- **Files modified:** `user/scripts/lazy_core.py` (`compute_per_feature_ceiling`, `git_head_short_sha`), `user/scripts/lazy-state.py` (`--per-feature-cycle-cap` arg + `compute_state` kwarg + trip-eval branch + `_DEFERRED_BUDGET`/`_BUDGET_GUARD` globals + `_state` surfacing + exhaustion terminal + marker persistence + 5-sub-fixture `--test` block), `user/scripts/bug-state.py` (benign parity flag + kwarg), `user/scripts/test_lazy_core.py` (6 ceiling tests), `user/scripts/tests/baselines/lazy-state-test-baseline.txt` (regenerated; bug-state baseline unchanged).
- **TDD:** WU-3 + WU-4 fixtures written FIRST (RED — `AttributeError`/missing trip), then implemented → 615/615 `test_lazy_core` + all `lazy-state.py --test` GREEN.
- **Integration verified (P1→P2 end-to-end):** the WU-4 `--test` block writes a real marker with `per_feature_forward_cycles[id]` (the P1 map) and drives the P2 trip off it — proving the counter feeds the trip eval through `claude_state_dir()`.
...
**Deliverables:**
- [x] `lazy_core` (or `lazy-state.py`) helper `parse_independent_marker(spec_text, queue_entry) -> bool` — reads an explicit `independent: true` / `no_shared_state: true` from the SPEC frontmatter OR the `queue.json` entry; default `False` when absent. Deterministic on-disk read, no LLM judgment.
- [x] `skip_ahead_ready(candidate, gated_ids, repo_root, spec_dir) -> bool` — two-key predicate: reuse `parse_dep_block` (`lazy-state.py:831`) to get the candidate's deps, filter `kind == "hard"`, return `False` if any hard dep `feature_id` is in `gated_ids` (currently research-pending or BLOCKED); else require `parse_independent_marker` truthy. `soft`/`composes` deps do NOT block (they need the upstream to exist, not be Complete). Resolves dep ids via the existing `resolve_upstream_dir` only insofar as needed to test membership in `gated_ids` (id-set membership is sufficient — no Complete check).
- [x] `--strict-research-halt` arg on `lazy-state.py` (mirrored on `bug-state.py` for parity); threaded as appended `compute_state()` kwarg `strict_research_halt: bool = False`. When set, restore the legacy halt-on-first-gated-head (the pre-feature `--skip-needs-research`-style halt). Default (flag absent) = the new dependency-aware skip-ahead is ON.
- [x] Wire skip-ahead into the `compute_state()` queue-selection loop: when the head candidate is gated (research-pending per the existing `research_pending` peek at `lazy-state.py:1369`, OR BLOCKED), and `--strict-research-halt` is NOT set, add the gated head to a `gated_ids` set + a surfaced `_GATED_HEADS` skip-list and continue to the next candidate; dispatch the first candidate that passes `skip_ahead_ready`. An unmarked or downstream candidate is NOT dispatched (it degrades to today's strict behavior for that item).
- [x] All-gated terminal: when every remaining item is gated, downstream of a gated item, or lacks the `independent: true` marker, return a clean terminal (reuse/parallel `queue-blocked-on-research`) with the gated heads surfaced — NOT a false completion. Log the skip-ahead audit (gated-head id + skipped-to id + the evaluated dep array) per RESEARCH_SUMMARY.
- [x] Tests: fixtures for — skip-ahead advances onto an independent item (no hard dep on head AND `independent: true`); unmarked item NOT skipped onto (degrades to halt); downstream item (hard dep on head, even if marked) NOT skipped onto; all-gated clean terminal; `--strict-research-halt` restores legacy halt-on-first-gated-head.
...
**Runtime Verification** *(checked by integration test or manual testing):*
- [x] `lazy-state.py --test` shows all five skip-ahead fixtures PASS <!-- verification-only --> (hermetic `--test` `[skip-ahead]` functional block — all sub-cases green)
- [x] `parse_independent_marker` + `skip_ahead_ready` characterized in `test_lazy_core.py` (marker present/absent/`no_shared_state` alias; hard vs soft/composes dep on a gated id) <!-- verification-only --> (8 RED→GREEN characterization tests; `test_lazy_core.py` 623/623)
...
**Implementation Notes (2026-06-20, Phase 3 landed — INLINE execution under /lazy-batch, no Agent calls; part-2 plan):**
- **Predicates (WU-1):** `lazy_core.parse_independent_marker(spec_text, queue_entry)` — strict `: true` match (regex `_INDEPENDENT_MARKER_RE`, key set `_INDEPENDENT_MARKER_KEYS = {independent, no_shared_state}`); reads the SPEC leading `---` frontmatter fence (or a bare leading marker before the first heading) AND the queue entry; `: false`/absent → False. `lazy_core.skip_ahead_ready(deps, gated_ids, independent)` — PURE (deps passed in, not parsed inside): a HARD dep with `feature_id in gated_ids` → False; soft/composes ignored; else require `independent`. ⚖ policy: predicate placement (lazy_core vs lazy-state) → lazy_core with deps passed in (mechanical-internal; cleanest test surface, avoids moving `parse_dep_block`; the plan explicitly permits this). 8 RED→GREEN tests in `test_lazy_core.py`.
- **Flag (WU-2):** `--strict-research-halt` (store_true) on `lazy-state.py` argparse → appended `compute_state(... strict_research_halt=False)` kwarg (positional callers unbroken). MIRRORED on `bug-state.py` as a benign parity parse (bug pipeline has no research gate; accepted + ignored, `_ = strict_research_halt`) — `lazy_parity_audit.py` green.
- **Skip-ahead branch (WU-2):** in the `compute_state` queue-selection loop, the FINAL gate before dispatch (after completion/cloud/device/research-batch/park/budget). A `_is_gated_head(spec_path)` closure (the SAME research-pending peek the `--skip-needs-research` branch uses, PLUS BLOCKED.md) flags a gated head; under default skip-ahead it is recorded (`gated_ids` + the surfaced `_GATED_HEADS`), audit-logged, and skipped. A later candidate dispatches ONLY if `skip_ahead_ready(parse_dep_block(spec), gated_ids, parse_independent_marker(spec, entry))` — an unmarked/downstream candidate is recorded in `skip_ahead_blocked` and skipped.
- **ADDITIVE invariant (WU-3 / key design decision):** ⚖ policy: all-gated terminal vs gated-head fallback → always fall back to the gated head's per-feature terminal (additive skip-ahead). A skip is REALIZED only when an INDEPENDENT alternative actually dispatches (`current` non-None). Whenever no skip-ahead-ready alternative exists (single gated item, OR a gated head with only downstream/unmarked siblings), a `gated_head_fallback` dispatches the FIRST gated head normally so its per-feature terminal (Step 3 `blocked` / Step 5 `needs-research`) fires — exactly the pre-feature single-/lazy behavior. This keeps default-on skip-ahead STRICTLY ADDITIVE (zero regression to needs-research / BLOCKED / park-precedence; all ~50 existing `--test` fixtures stayed green) and the gated head is still surfaced (its own terminal is the SPEC's "clean terminal or equivalent", never a false completion). When the skip is NOT realized, `_GATED_HEADS` is cleared so the probe output is byte-identical to the pre-Phase-3 single-item path. The SPEC's literal `queue-blocked-on-research` all-gated terminal remains reachable via the unchanged `--skip-needs-research` batch path.
- **Probe surfacing + audit (WU-3):** `_state()` folds a `gated_heads` list into the probe JSON ONLY when a skip was realized (`_GATED_HEADS` non-empty) — same byte-identity discipline as `parked`/`budget_guard`. The skip-ahead audit `_diag` (RESEARCH_SUMMARY rich-audit leg) emits `gated_heads`, the candidate, its evaluated dep array (feature_id+kind), and the DISPATCH/SKIP verdict on every skip-ahead evaluation.
- **Files modified:** `user/scripts/lazy_core.py` (2 predicates + `_INDEPENDENT_MARKER_*` constants + `_coerce_marker_truthy`), `user/scripts/lazy-state.py` (`--strict-research-halt` arg + `compute_state` kwarg + `_is_gated_head` closure + skip-ahead branch + `gated_ids`/`skip_ahead_blocked`/`gated_head_fallback`/`_GATED_HEADS` globals + `_state` `gated_heads` surfacing + additive fallback + 3-case `[skip-ahead]` `--test` block), `user/scripts/bug-state.py` (benign parity flag + kwarg), `user/scripts/test_lazy_core.py` (8 predicate tests), `user/scripts/tests/baselines/lazy-state-test-baseline.txt` (regenerated via `_normalize_smoke_output` for the one new print line; bug-state baseline unchanged).
- **TDD:** the 8 `test_lazy_core.py` predicate tests written FIRST — confirmed 8 FAIL RED (`AttributeError: no attribute parse_independent_marker / skip_ahead_ready`), then implemented → 623/623 GREEN. The `compute_state` skip-ahead `[skip-ahead]` fixture block characterizes WU-2/WU-3 end-to-end.
- **Gates:** `lazy-state.py --test`, `bug-state.py --test`, `lazy_coord.py --test`, `test_lazy_core.py` (623/623), `lazy_parity_audit.py --repo-root .` all GREEN; `py_compile` clean.
- **Review verdict:** PASS — spec-aligned (Locked Decisions 3 + 5), additive-by-construction (zero regression), TDD RED-first, parity green. MCP gate skipped (no MCP-reachable surface — state-machine logic only).
- **Phase-4 hand-off:** the `gated_heads` probe key shape is finalized here (a list of gated-head feature ids, present only when a skip realized). Phase 4 wraps it into the end-of-run flush + documents `--strict-research-halt` and the default-on behavior in both batch wrappers' flag tables, and registers the new flags as a `lazy_parity_audit.py` mechanic set.
