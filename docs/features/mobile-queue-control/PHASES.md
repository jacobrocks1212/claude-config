...
**Work completed (all three phases, single inline `/execute-plan` cycle):**
- `user/scripts/lazy-queue-doc.py` (net-new) — pure-read generator. `render_doc(state, repo_root, *, run_active, link_mode='relative', remote_url=None, branch='main') -> str` is a pure function of the `probe_state()` aggregate + the on-disk SPEC/PHASES files items point at; `main(argv)` is the CLI (`--repo-root`/`--stdout`/`--link-mode`). Reuses `pipeline_visualizer.probe.probe_state` (no re-shelling of the state scripts) and the item-attached `curated_stage` verbatim.
- `user/scripts/test_lazy_queue_doc.py` (net-new) — 34 tests, all phases (state-fidelity, byte-stability, triage, SPEC links, phase N/M, exec-summary, relative/absolute link modes, no-op-commit gate, real-repo smoke). Authored RED-first (FileNotFoundError against the absent module), then GREEN.
- Phase 3 wiring is **prose-only** (no state-machine change): `.claude/skill-config/commit-policy.md` + `user/skills/lazy-batch/SKILL.md` per-cycle commit note document running the generator before `git add -A` so `LAZY_QUEUE.md` rides the cycle commit on `main`. `user/scripts/CLAUDE.md` + root `CLAUDE.md` scripts tables updated.

**Key design choices:**
- **Byte-stable / no wall-clock** (Integration Note option (a)): the doc body embeds no timestamp; `state["server_time"]` is intentionally ignored. Freshness = the run-active/idle (`🔒`/idle) marker from `lazy-state.py --marker-present` (shelled in `main`, passed as a pure boolean into `render_doc`) + GitHub's native last-commit time.
- **Phase-progress** — a new display-only `phase_progress(phases_md_path) -> (checked, total)`: a phase is "checked" iff it has ≥1 `- [x]` and zero `- [ ]` (code-fence-safe). No reusable `lazy_core` public helper counts *phases* (only `count_deliverables` counts boxes repo-wide), so a small local helper was added per the plan's "only add if none reusable" directive.
- **Link modes** — `relative` (default, Decision 7) + `absolute` (`github.com/<owner>/<repo>/blob/<branch>/...`); `parse_owner_repo` handles https + ssh remotes. A failed GitHub-mobile relative-link check is a one-flag flip, not a rewrite.
- **Triage byte-stability** — "Needs attention" header is omitted entirely when no item is Blocked/Needs-input (no empty header).

**Pitfalls / notes:**
- The module is dash-named (`lazy-queue-doc.py`, sibling-convention to `lazy-state.py`), so tests load it via `importlib.util` rather than a plain `import`.
- `--repo-root .` makes the header show `.` as the repo name; the pipeline passes absolute paths so it renders `claude-config` correctly. Cosmetic only.
- **Cross-repo (AlgoBooth) wiring** is documented for the operator (a one-line `--repo-root`-addressable invocation add in AlgoBooth's `/lazy-batch`(-cloud) cycle), NOT authored from this repo's tree (one-writer / repo-boundary). The generator itself needs no AlgoBooth-side change. No spin-off item was created — adoption is trivial documentation, not new feature work.

**Files modified:** `user/scripts/lazy-queue-doc.py` (new), `user/scripts/test_lazy_queue_doc.py` (new), `.claude/skill-config/commit-policy.md`, `user/skills/lazy-batch/SKILL.md`, `user/scripts/CLAUDE.md`, `CLAUDE.md`.
...
**Deliverables:**
- [x] `user/scripts/lazy-queue-doc.py` — new stdlib-only generator. Imports the sibling `pipeline_visualizer` package using the `_SCRIPTS_DIR`-on-`sys.path` pattern from `pipeline_visualizer/__main__.py` (lines 18-20), calls `probe_state(repo_root)`, and renders markdown.
- [x] Argument surface: `--repo-root` (default cwd, matching `__main__.py`), `--stdout` (print, don't write), `--test` (in-file fixture smoke harness, mirroring the `lazy-state.py --test` convention) OR a pytest sibling (see Testing Strategy — choose pytest sibling for consistency with `test_pipeline_visualizer.py`).
- [x] Renderer: Features table (`# | item | state | tier`), Bugs table (`# | item | state | sev`), grouped under `## Features (N)` / `## Bugs (M)` headers with live counts. Each item name is a markdown link to its SPEC.md (relative path `docs/{features,bugs}/<spec_dir>/SPEC.md`, resolved from the state script's `spec_path` when present, else `spec_dir`/`id`).
- [x] "Needs attention" section: enumerate items whose `curated_stage` ∈ {Blocked, Needs-input} (the `/lazy-status` triage signal), one bullet each; omit the section entirely when none (byte-stability: an empty triage section must not emit an empty header).
- [x] Freshness header: `# Lazy Queue — <repo>   (run active 🔒 | idle)`. The run-active/idle token comes from shelling `lazy-state.py --marker-present --repo-root <root>` (exit 0 → active, exit 1 → idle). **No embedded wall-clock timestamp** — RESOLVED by operator (NEEDS_INPUT.md Decision 1 → option (a), 2026-06-22): the doc body carries no generated-at line; "last updated" is read from GitHub's native last-commit time for `LAZY_QUEUE.md`. This makes the doc trivially byte-stable (an unchanged-state regen is byte-identical → no spurious commit), satisfying Phase 3's no-op-commit gate with zero special-casing. The run-active/idle marker still distinguishes a live run from idle.
- [x] Tests: pytest sibling `user/scripts/test_lazy_queue_doc.py` (see Testing Strategy).
...
- **Byte-stability vs. the freshness timestamp — RESOLVED (operator, 2026-06-22, NEEDS_INPUT.md Decision 1 → option (a)).** The freshness design point is settled: the doc embeds **NO wall-clock timestamp**. Freshness is derived from the doc's own git commit time (GitHub mobile shows the file's last-commit time natively; "last updated" reads from `git log -1 LAZY_QUEUE.md`). The run-active/idle (`🔒`/idle) marker still distinguishes a live run from idle. This makes the doc trivially byte-stable — an unchanged-state regen is byte-identical, so no spurious commit — which is exactly Phase 3's no-op-commit gate with zero special-casing. (Option (b), a byte-stability-excluded timestamp line, is NOT taken — it was the rejected alternative.) Phase 3 inherits this: no diff-exclusion logic is needed in the no-op detector because there is no per-run-varying line in the doc body.
