...
**Deliverables:**
- [x] `lazy_core.py`: new `evaluate_completion_evidence(feature_dir, repo_root) -> dict` returning `{verdict: "exempt-and-tick"|"warn-exempt"|"refuse", reason: str, pass_count: int|None, validated_commit: str|None}`. Reuses the existing sentinel parser (`parse_sentinel`) and `_current_head` — never a parallel reader.
- [x] Decision-table logic implementing every row of the SPEC's authoritative-evidence table: require the union of `VALIDATED.md` (`kind: validated`) AND `MCP_TEST_RESULTS.md` (`all-passing`, `pass==total`, `pass>0`); `validated_commit == HEAD` exact; `VALIDATED.md` present + results missing/malformed → refuse (forged-attestation); results present + `VALIDATED.md` missing → refuse; `SKIP_MCP_TEST.md` / `DEFERRED_*` present (without passing results) → refuse, no tick (fail-closed); neither → refuse.
- [x] HEAD-drift carve-out: when `validated_commit != HEAD`, inspect `git diff --name-only <validated_commit> HEAD` (via the existing subprocess pattern, `_git_diff_name_only`); all paths matching `*.md` → `warn-exempt`; any non-`.md` (source/script/config) path → `refuse` (refuse-and-revalidate / TOCTOU).
- [x] `pass>0` zero-test guard: `pass==total==0` → refuse (CI false-positive anti-pattern), distinct reason string.
- [x] Tests: register `lazy_core.py` `_TESTS` smoke fixtures covering EACH decision-table row — exempt-and-tick happy path, forged-attestation refuse, results-without-validated refuse, SKIP fail-closed, DEFERRED fail-closed, zero-test refuse, HEAD-drift docs-only warn-exempt, HEAD-drift source-file refuse-and-revalidate, neither-present refuse. Each builds a temp feature dir (and a git fixture for the HEAD-drift rows).

#### Implementation Notes (Phase 1 — 2026-06-19)

**Review verdict:** PASS (inline; cycle-subagent inline execution, test-first — 9 RED fixtures authored + confirmed RED on `AttributeError`, then GREEN).

- **Verdict-dict shape (LOCKED contract Phase 2/3 branch on):** `evaluate_completion_evidence(feature_dir, repo_root) -> {"verdict": "exempt-and-tick"|"warn-exempt"|"refuse", "reason": str, "pass_count": int|None, "validated_commit": str|None}`. Note the signature dropped the speculative `pass_count_out=` kwarg (plan said "or equivalently-named") — `pass_count`/`validated_commit` are surfaced in the RETURN dict instead, which Phase 2 reads directly.
- **Helpers added:** `_coerce_evidence_count` (mirrors the `__write_validated_from_results__` `_coerce_count` bool/int/digit-string tolerance) and `_git_diff_name_only(repo_root, base, head) -> list[str]|None` (best-effort subprocess, None on non-git/unknown-commit → caller refuses conservatively). `_FAIL_CLOSED_EVIDENCE_SENTINELS = (SKIP_MCP_TEST.md, DEFERRED_NON_CLOUD.md, DEFERRED_REQUIRES_DEVICE.md)`.
- **Freshness leniency:** `validated_commit` absent OR HEAD unresolvable (non-git) → `exempt-and-tick` with a "freshness UNVERIFIED" reason (the upstream mint-gate already required a fresh commit to write VALIDATED.md, so a field-less file is the legacy path) — matches the state scripts' permissive Step-9 skip.
- **Pure / side-effect-free:** the only I/O is `parse_sentinel` reads + one `git diff` for the drift row; no PHASES.md mutation (Phase 2) and no completion-gate wiring (Phase 3).
- **Pitfall fixed in fixtures:** the existing `_git_fixture_commit` does `add -A && commit`, which fails ("nothing to commit") on an empty/untracked tree; added `_cc_seed_and_commit` to seed a tracked README first.
...
**Deliverables:**
- [x] `lazy_core.py`: new `autotick_verification_rows(phases_path, validated_commit, pass_count) -> dict` returning `{ticked_count: int, ok: bool, reason: str|None}`. Uses the existing `_atomic_write` helper (temp-in-same-dir → `os.replace`) — never `open('r+')` / naive truncate.
- [x] Line-anchored + code-fence-safe matcher: rewrite only lines matching `^\s*-\s+\[\s+\]` (`_UNCHECKED_ROW_RE`) that ALSO carry `_VERIFICATION_ONLY_MARKER` on the SAME line or header-scope (tolerate variable whitespace `- [  ]`); skip any line inside a ``` fence. NO global `.replace('- [ ]','- [x]')`. Reuses the fence-tracking + Superseded + marker-scope pattern from `remaining_unchecked_are_verification_only`.
- [x] Audit-trail comment: append a byte-stable `<!-- auto-ticked: validated_commit=<sha> -->` to each rewritten row, so a later auditor distinguishes gate mutations from human/agent edits.
- [x] Cardinality lock: assert `ticked_count <= pass_count`; if more rows would be ticked than tests passed, abort the rewrite (return `ok: false`, write nothing) — catches marker-drift hallucination / forged evidence.
- [x] Superseded-aware: do NOT count or require unchecked boxes under phases whose Status is `Superseded` so the downstream `check-docs-consistency.ts` is satisfied without ticking superseded rows.
- [x] Tests: `_TESTS` fixtures — happy rewrite (N marker rows → all ticked, audit comments present, atomic, fenced + non-verification rows untouched), variable-whitespace `- [  ]` row IS matched, cardinality-lock abort when `ticked_count > pass_count` (file byte-unmodified), Superseded-phase unchecked box left alone, idempotent re-run (already-ticked rows unchanged, no duplicate audit comment).

#### Implementation Notes (Phase 2 — 2026-06-19)

**Review verdict:** PASS (inline, test-first — 5 RED fixtures confirmed RED on `AttributeError`, then GREEN).

- **Return shape (LOCKED for Phase 3):** `autotick_verification_rows(phases_path, validated_commit, pass_count) -> {"ticked_count": int, "ok": bool, "reason": str|None}`. Phase 3 records `ticked_count` in `COMPLETED.md`/`FIXED.md` (`auto_ticked_rows`) and the `--apply-pseudo` JSON.
- **Two-pass design:** pass 1 collects the to-tick line indices (so the cardinality count is known BEFORE any write); the cardinality abort returns `ok: False, ticked_count: 0` writing NOTHING (proven byte-identical). Pass 2 rewrites only those indices via `_atomic_write`, preserving each line's original ending and flipping ONLY the interior blank to `x`.
- **New module constants:** `_UNCHECKED_ROW_RE = re.compile(r"^(\s*-\s+\[)\s+(\]\s.*)$")` (captures the dash+bracket prefix; tolerates `- [  ]`) and `_AUTOTICK_COMMENT_PREFIX = "<!-- auto-ticked: validated_commit="` (also the idempotency sentinel — a row already carrying it is skipped, no double-tick / no duplicate comment).
- **Marker scope:** row-level marker OR enclosing subsection-header marker (header-scope), matching `remaining_unchecked_are_verification_only`. Superseded phases + fenced rows are skipped.
- **Integration ordering note for Phase 3:** the rewrite MUST run BEFORE the `_phase_completion_plan` residual-incoherence re-check (tick first → the gate then sees zero unchecked verification rows).
...
**Deliverables:**
- [x] `_phase_completion_plan` / completion handler: thread the evidence verdict so that when remaining unchecked rows are ALL verification-marked AND the verdict is exempt/warn-exempt, those rows are auto-ticked FIRST (so the re-parse sees them checked) and NOT counted as refusals; a non-verification unchecked row still appends a refusal naming the phase. Preserves the existing auto-flip-to-Complete and Superseded behavior exactly. **Note — no `_phase_completion_plan` signature change was needed:** wiring the tick-first ordering into the handler (tick → re-parse → `_phase_completion_plan`) means the gate sees the rows already `- [x]`, so `verify_ledger`'s caller is untouched (no caller migration / no `--test` fixture churn). Evidence-as-source-of-truth is fully preserved.
- [x] `--apply-pseudo __mark_complete__` / `__mark_fixed__` handler: BEFORE the residual-incoherence refusal, when the verdict authorizes, call `autotick_verification_rows`; then re-run the coherence check (now zero unchecked verification rows); then mint the receipt. Order: tick → re-check → write receipt.
- [x] `COMPLETED.md` receipt: record the auto-ticked row count (`auto_ticked_rows: <n>`) so a later auditor sees how many rows the gate mutated. Mirrored in `FIXED.md` for `__mark_fixed__` (same `write_completed_receipt(..., auto_ticked_rows=)` param).
- [x] Kill-switch env var (`LAZY_STRICT_EVIDENCE_GATE` / `LAZY_DISABLE_AUTOTICK`): when set/truthy, the auto-tick is skipped entirely and the coherence gate falls back to the legacy strict path (verification rows INCLUDED in refusals) — frictionless rollback with no code revert. Read once via `_evidence_gate_killed()` (an explicitly-falsy `""`/`0`/`false`/`no`/`off` does NOT arm it).
- [x] `--apply-pseudo` JSON result surfaces `auto_ticked_rows` alongside `flipped_phases` (orchestrator-visible), matching the existing flip surfacing pattern.
- [x] Tests: end-to-end `_TESTS` fixtures — (a) validated feature with only verification rows unchecked → `__mark_complete__` mints `COMPLETED.md`, rows ticked, `auto_ticked_rows` recorded in receipt + JSON, NO refusal; (b) feature with a real implementation `- [ ]` row → refuses, names the phase, zero writes (PHASES.md byte-unchanged, no receipt); (c) `verify_ledger` (deliverables_done exempts verification rows) and the completion gate agree on a verification-only feature; (d) kill-switch set → legacy refusal, zero PHASES.md mutation; (e) zero-test evidence refuses at the live gate; (f) `__mark_fixed__` mirror — FIXED.md minted, rows ticked, auto_ticked_rows recorded.
...
#### Implementation Notes (Phase 3 — 2026-06-19)

**Review verdict:** PASS (inline, test-first — 6 RED end-to-end fixtures; the 3 should-now-pass fixtures confirmed RED under the legacy gate, then GREEN after wiring; the 3 refusal/kill-switch fixtures stayed green as designed).

- **Wiring (LOCKED order):** in the `apply_pseudo __mark_complete__` / `__mark_fixed__` handler, BEFORE the completion-coherence gate: read the kill-switch (`_evidence_gate_killed()`), call `evaluate_completion_evidence(spec_path, repo_root)`, and on an authorizing verdict (`exempt-and-tick` / `warn-exempt`) call `autotick_verification_rows(PHASES.md, validated_commit, pass_count)` FIRST. Then the existing coherence gate re-reads PHASES.md and runs `_phase_completion_plan` over the freshly-ticked file. **`_phase_completion_plan` itself was NOT modified** — the tick-first ordering makes the verification rows already `- [x]` at re-check time, so no signature change and no caller migration (`verify_ledger` untouched). A cardinality-lock abort (`ok: False`) leaves the rows unchecked → the gate refuses, surfacing the over-tick guard at the live gate.
- **Receipt + JSON:** `write_completed_receipt` gained a keyword-only `auto_ticked_rows: int | None = None`, emitted as a frontmatter line when not None (omitted for legacy/backfill callers); the handler always passes it (default 0), and the `--apply-pseudo` result dict carries `result["auto_ticked_rows"]` alongside `flipped_phases`.
- **Kill-switch:** `_evidence_gate_killed()` reads `LAZY_STRICT_EVIDENCE_GATE` and `LAZY_DISABLE_AUTOTICK`; a value in `{"", "0", "false", "no", "off"}` (case-insensitive) is treated as NOT armed so an inherited empty export can't disable the feature.
- **No state-smoke baseline drift:** `lazy-state.py --test` + `bug-state.py --test` stayed green (the new `auto_ticked_rows` JSON key did not perturb any pinned fixture).
- **Docs:** `user/scripts/CLAUDE.md` gained the "Evidence-gated completion exemption + auto-tick" subsection under the verification-only-marker section (Coupling Rule — prose lockstep with code).
- **Pitfall:** `verify_ledger(repo_root, spec_path, plan)` takes the feature DIR as `spec_path` (it reads `<dir>/PHASES.md`), not the `SPEC.md` file — a fixture initially passed the file and false-failed `deliverables_done`.
