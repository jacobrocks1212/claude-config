...
**Deliverables:**
- [ ] `lazy_core.py` self-edit predicate: returns true iff `~/.claude/skills`, `~/.claude/scripts`, AND `~/.claude/hooks` all resolve (after symlink resolution) under the run's `git rev-parse --show-toplevel`. Semantically-correct (robust to the repo cloned elsewhere); NOT a cwd-basename match.
- [ ] `--probe` / `emit_cycle_prompt` carry `self_edit_mode: true|false` (and optionally `governing_files_touched` derived from the last commit's `git diff --name-only`).
- [ ] Orchestrator governing-file reload discipline (prose in `user/skills/lazy-batch/SKILL.md`, mirrored to bug/cloud twins in Phase 5's coupled-trio work — here author the prose pattern): when `self_edit_mode` is true, after every cycle intersect the cycle's commit with the **governing-file set** (`user/skills/lazy-batch/SKILL.md` + bug/cloud twins, `user/skills/_components/{orchestrator-voice,completeness-policy,lazy-dispatch-template}.md`); re-`Read` any hit via its `~/.claude/...` path before composing the next dispatch. Keep this set in lockstep with the compaction re-read list.
...
- [ ] Auto-refresh-boundary documentation (in `lazy_core.py` and/or orchestrator prose): `lazy_core.py`/`lazy-state.py`, `cycle-base-prompt.md`/addenda/`loop-block.md`, hook `.sh` bodies, and downstream skill prose are ALREADY live on the next probe/dispatch — they MUST NOT be flagged for reload (they were never stale).
- [ ] Tests: `pytest` on the predicate — symlink fixtures where `~/.claude/{skills,scripts,hooks}` resolve UNDER the git toplevel (true) vs. outside it (false); a normal-repo fixture (false); governing-file-set membership tests (a `lazy-batch/SKILL.md` edit IS in the set; a `lazy_core.py` / `cycle-base-prompt.md` / hook-body edit is NOT).
