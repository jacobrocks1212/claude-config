...
- Wired in `settings.json` under `PreToolUse` for the `Bash` tool (and `Agent`, to deny recursive dispatch attempts — see below; **2026-07-09 amendment:** the `Agent` matcher and the recursive-dispatch deny were REMOVED — the harness allows nested dispatch, and blanket-denying it broke mandated read-only Explore fan-outs; see `docs/bugs/adhoc-containment-denies-mandated-explore-fanout`. The hook is now wired for `Bash` and `Skill`). Fast-path `test -f` exit when `lazy-cycle-active.json` is absent (interactive sessions and orchestrator-between-cycles untouched).
- While the marker is present, parse the Bash command and **DENY** (`permissionDecision: deny`, with a corrective `permissionDecisionReason`) when it matches any of:
  ...
  - **Recursive dispatch:** an `Agent` tool call while the marker is present is DENIED (a cycle subagent has no `Agent` tool today, but the deny makes the intent explicit and future-proofs it). **(REMOVED 2026-07-09 — see the amendment in the wiring bullet above; the inline-override remains prose policy in `cycle-base-prompt.md`.)**
