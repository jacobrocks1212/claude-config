...
- State-Script CLI Contract: Registry + Shared Surface (`state-cli-contract-registry`) — P1 — generate a committed `cli-surface.json` from the live ArgumentParsers and lint every skill/component `--flag` mention against it (kills the invented/drifted-flag error class); hoist the ~72 name-identical flags + twin-duplicated helpers of `lazy-state.py`/`bug-state.py` into a parameterized shared CLI builder so twin parity becomes structural instead of regex-audited.
...
- Coupled-Pair Generation (`coupled-pair-generation`) — P1 — stop hand-mirroring the 5 coupled skill pairs (112 of 129 audited headings are manual restatements, ~300KB derived prose): generate derived SKILL.md files from the canonical + the manifest's existing token substitutions + divergence overlays; the parity audit demotes to a freshness verifier.
...
- lazy-batch Skill Deflation (`lazy-batch-skill-deflation`) — P2 — cut the 252KB orchestrator skill's ~65-85KB of driftable script-behavior narration down to verdict-routing rules, relocate incident narratives, and add a size/long-line lint ratchet (soft-dep: coupled-pair-generation).
- skill-config Schema + Reference Lint (`skill-config-schema-and-reference-lint`) — P2 — per-repo declared-files manifest with intended-absent markers, JSON-schema validation for load-bearing configs (`build-queue-ops.json`), and a lint sweeping every `.claude/skill-config/` mention; quick win: the missing algobooth `commit-policy.md` behind the corpus's #1 tool-error cluster (377 failed Reads).
