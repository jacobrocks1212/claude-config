...
**Deliverables:**
- [ ] `user/scripts/kpi-scorecard.py` — add `"repo-static-scan": frozenset({"hook-duplicated-line-count"})` to `_SOURCES` (line ~75) + a `_sel_*` selector function computing the deterministic count (static scan of `user/hooks/`, honesty ladder: absent/unrecordable → NO-DATA, never a fabricated zero).
...
- [ ] Tests: extend `user/scripts/test_kpi_scorecard.py` — cover the new source/selector registration + the counter (deterministic count on a fixture tree; NO-DATA on an unrecordable input).
