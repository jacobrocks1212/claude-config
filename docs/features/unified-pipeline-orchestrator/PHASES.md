...
**Deliverables:**
- [ ] Merged-view helper: either `lazy-state.py --next-merged` (preferred — single CLI surface, reuses the bound active repo) OR a `merged_queue.py` module. Decision recorded in Integration Notes below; default to `--next-merged` unless Phase-1 recon shows a separation reason.
- [ ] Ordering rule implemented: priority/tier descending; tie → `type == "bug"` sorts before `type == "feature"`. Stable for equal (priority, type).
- [ ] Reuses the existing queue loaders in `lazy-state.py` / `bug-state.py` (do NOT re-parse JSON by hand) and binds the active repo before reading.
- [ ] Resolves the Open-Question "ordering field source" — confirm whether feature/bug `queue.json` items carry a comparable `tier`/`priority` field or whether a normalization map is needed (see Runtime Verification spike below).
- [ ] Tests: `test_lazy_core.py` (or `lazy-state.py --test`) fixtures covering: both queues populated → correct order; bug-breaks-tie at equal priority; only-features → feature order unchanged; only-bugs → bug order unchanged; both empty → no item.
...
- [ ] Runtime Verification spike (ordering-field ground truth): inspect a REAL `docs/features/queue.json` and `docs/bugs/queue.json` and record the OBSERVED priority/tier field names + value ranges before locking the comparator. The SPEC Open Question ("do items already carry a comparable priority field?") is a runtime-coupled assumption about live queue shape — confirm against actual files, not by reading the enqueue code. Record the observed field names in this section.
- [ ] `--next-merged` over both populated queues returns items in priority order with bugs breaking ties (live two-queue fixture).
...
**Deliverables:**
- [ ] `lazy-batch/SKILL.md` cycle loop rewired: each cycle probes the merged head → dispatches via the matching state script → commits → pushes. Both state machines and gates run unchanged.
- [ ] Type-dispatch is explicit and tabulated in the skill's State Machine Summary (feature → `lazy-state.py` + `__mark_complete__`; bug → `bug-state.py` + `__mark_fixed__`).
- [ ] Cloud mirror: `repos/algobooth/.claude/skills/lazy-batch-cloud/SKILL.md` updated with the same merged-view orchestration shape; its "Differences from /lazy-batch" block updated to reflect only intended divergences (per coupled-pair rule).
- [ ] `lazy-bug-batch/SKILL.md` converged/cross-referenced: the unified driver supersedes the standalone bug loop; document the relationship (do not silently leave two drivers that drift).
- [ ] `lazy_parity_audit.py` extended: assert the merged-view dispatch branch stays consistent across feature/bug handling (a new check in `audit_state_script_parity` or `audit_all_pairs`).
- [ ] No-regression guard: with only one queue populated, the unified run behaves identically to today's per-type batch (asserted by parity audit + a single-type fixture).
- [ ] Tests: parity audit passes; `lazy-state.py --test` + `bug-state.py --test` green (driver change must not break either state machine).
