...
**Deliverables:**
- [ ] **Two-pass loop (Decision 1).** Rewrite the Per-Chunk Loop (L74–121) to:
  ...
  6. **Advance.**
- [ ] **AI-role scoping (SPEC §5).** Bake the facilitator framing into the Pass-1 and Pass-2 copy so the reviewer does not defer to the tool on business-logic correctness.
...
- [ ] **Phase 2 severity→section mapping (SPEC §4 + drift correction).** Map dispositions onto the **existing** synthesizer-v2 sections: **Blocking** → `## Critical Findings` (investigation-sourced) / `### Important` (rule/reuse/intrafile); **Important** → `### Important` subsections; **Suggestion** → the existing **`### Minor` (nit)** subsections — do **not** create a `### Suggestion` section. **Dismiss** → excluded. Update Collect Curated Content (L184–191) accordingly.
