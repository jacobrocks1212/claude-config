...
**Deliverables:**
- [x] `commands/learn-from-pr.md` (and/or `calibrate.md`): add a **buddy-disposition signal source** alongside the existing Haiku→GitHub-comment matching (`§2.5.2/§2.5.4`). Read `buddy-session.json` dispositions, recover `rule_id`/`source` by joining `finding_ref`+`source` against `processed-findings.json`, derive signal (`dismiss=0`, kept=`1`), apply the existing EMA (`new = α·signal + (1-α)·old`, `α=0.25`). Update both rule weights and the new `source_weights`.
- [x] `commands/review-pr-buddy.md` Phase 2: auto-invoke disposition recalibration at completion; print a summary of weight deltas; no prompt.
- [x] `commands/review-pr.md`: on completion (near the REVIEWED.md sentinel `:477-504`), write a `pending-calibration` marker recording the cache dir / PR so a later `/learn-from-pr` consumes it.
- [x] `/learn-from-pr`: consume and clear the `pending-calibration` marker when run.
...
**2026-06-22 — Batch 1 (WU-4a + WU-4c): disposition→EMA helper + non-buddy pending-calibration marker.**
- **WU-4a — net-new `scripts/disposition-calibration.ts` (291 lines) is the single disposition-calibration implementation.** CLI: `--session <buddy-session.json> --findings <processed-findings.json> --weights <weights.yaml> [--dry-run]`; `--weights` defaults to `knowledge/weights.yaml`. Flattens `chunks[].dispositions[]`, derives signal (`dismiss→0`, any kept severity→`1`), and applies the existing EMA (`new = α·signal + (1−α)·old`, `α = ema_alpha ?? 0.25`, rounded `toFixed(4)`).
- **Recovery join is tolerant by design** — real `finding_ref` values are `"<basename>:<line> (parenthetical)"` while `processed-findings.json` carries FULL paths. The join parses the leading `file:line` (`^(.*?):(\d+)`), then matches a processed finding by `source` + `line` + basename (case-insensitive, `\`→`/` normalized, also `endsWith("/"+base)`). sweep+`rule_id` → updates `rule_weights[rule_id].weight` + increments `data_points`; non-sweep (investigation/intrafile/reuse) → updates the scalar `source_weights[source]`. `source:"reviewer"` and unmatched dispositions are skipped (unmatched logged to stderr — no fabricated signal).
- **Comment-preserving surgical write (critical):** the helper `yaml.load`s only to READ current values, then mutates the raw file TEXT line-by-line (`replaceSourceScalar` / `replaceRuleWeight`) — never a `yaml.dump` round-trip (which would destroy weights.yaml's `# …` section comments). A zero-change run (no usable dispositions or `--dry-run`) does NOT write the file → byte-for-byte identical.
- **Tests (TDD):** `scripts/disposition-calibration.test.ts` (6 tests, node:test black-box via `execSync`, mirrors `post-process.test.ts`) + fixtures under `scripts/test-fixtures/disp-*`. Asserts: sweep dismiss `0.7→0.525` w/ `data_points 0→1`; kept investigation `0.9→0.925`; reuse dismiss updates `source_weights.reuse→0.525` (no new rule key); reviewer disposition skipped; empty session byte-identical no-op; `# api-design rules` comment preserved after a real edit. Each test runs against a throwaway temp copy of the fixture weights — never the real file. 6/6 green; `post-process.test.ts` still 9/9; `tsc --noEmit` clean.
- **WU-4a prose:** `learn-from-pr.md` §2.5.7 added — invokes the helper on any persisted `buddy-session.json` and consumes+clears `pending-calibration.json` (then `rm -f`); the §2.5.2/§2.5.4 Haiku/GitHub-comment path stays intact as the non-buddy signal (additive/asymmetric per R2). `calibrate.md` Notes points bulk/replay disposition calibration at the same helper (no second EMA path).
- **WU-4c — `review-pr.md` Step 12.7** writes `{cacheDir}/pending-calibration.json` (`{pr, cache_dir, date}`) on non-buddy PR-mode completion. **Buddy-safe by construction:** buddy delegates only Steps 1–8 and runs its own Phase 2, never reaching Step 12 — so the marker is written only on the non-buddy path (the intended asymmetry). PR-mode only; non-fatal on write failure.
- **Review verdict:** PASS (ground-truth verified: yes — orchestrator independently re-ran `wc -l`/`grep`/both test suites/`tsc`; counts and 6/6 + 9/9 + clean matched the subagent reports). One trivial PASS-WITH-FIXES applied in-session: removed two dead `Map` declarations (`ruleAccum`/`sourceAccum`) the impl agent left unused.
- **MCP integration test:** N/A — no MCP surface (deterministic `tsx` + command-prompt change).
- Files created: `scripts/disposition-calibration.ts`, `scripts/disposition-calibration.test.ts`, `scripts/test-fixtures/disp-{weights.yaml,processed-findings.json,buddy-session.json,buddy-session-empty.json}`. Files modified: `commands/learn-from-pr.md`, `commands/calibrate.md`, `commands/review-pr.md`.

**2026-06-22 — Batch 2 (WU-4b): buddy auto-recalibrates inline at Phase 2.**
- `commands/review-pr-buddy.md` Phase 2 gained `### Auto-Recalibrate from Dispositions`, placed after `### REVIEWED.md Sentinel` and before `### Cleanup and Report`. It **silently** (no `AskUserQuestion`) invokes the shared `scripts/disposition-calibration.ts` helper against this session's `buddy-session.json` + `processed-findings.json`, then prints the per-rule/per-source delta summary (the operator-validated decision: buddy auto-recalibrates with a summary, no opt-in prompt). Zero-disposition sessions no-op with a "weights unchanged" message; helper failure is non-fatal (WARN + continue). Documents the R2 asymmetry inline (buddy recalibrates here; non-buddy `review-pr.md` defers via the `pending-calibration` marker) and that this is the SAME single helper `/learn-from-pr` uses.
- **Review verdict:** PASS (ground-truth verified: yes — `wc -l` 356 + grep ordering REVIEWED.md(272)→Auto-Recalibrate(297)→Cleanup(316) re-run by orchestrator and matched; last committed touch of the file pre-dates this session).

**2026-06-22 — Follow-up fix (WU-4a helper, caught by Batch 2's quality gate): CRLF robustness in `disposition-calibration.ts`.** Batch 1 passed 6/6 on the LF working copy; after commit, git's `autocrlf` normalized `test-fixtures/disp-weights.yaml` to CRLF and the `sweep dismiss → 0.525` test went red (`got 0.7`). Root cause: `replaceRuleWeight` located the rule via an exact line-equality scan (`lines[i] !== "  appropriate-http-methods:"`) that fails against the CRLF line `"…:\r"`, so the rule weight was never replaced (the source-scalar path used a value-only regex and was unaffected). Fix: strip a trailing `\r` before the header/indentation comparisons and replace only the numeric token in place via regex (`/(^\s*weight:\s*)[\d.]+/`) so each line's existing ending is preserved — the helper is now robust to both LF and CRLF and never introduces mixed endings into the CRLF `knowledge/weights.yaml`. Re-verified: disposition test 6/6 (3 consecutive runs, non-flaky), `post-process.test.ts` 9/9, `tsc --noEmit` clean. (The atomic gate+commit chain aborted the Phase-4 post-phase commit before it ran — the `&&` caught the red test — which is the gate working as designed.)

**2026-06-22 — Phase 4 Integration Verification (PASS).** Ran the helper in `--dry-run` against a REAL captured pair (PR 16693, `…/56963-fix/56941-is-ui-minor-fixes/.pr-review/pr-cache/16693/`). All 7 non-reviewer dispositions joined cleanly to real findings (no `unmatched` stderr — the tolerant `finding_ref` parse handles the real `"<basename>:<line> (parenthetical)"` shape vs full-path processed findings); the lone `source:"reviewer"` disposition was correctly skipped; sequential EMA matched severities (dismiss→down, kept→up) ending `investigation 0.9→0.5417`, `intrafile 0.7→0.8313`; real `weights.yaml` left untouched (`--dry-run`). Confirms the full loop: buddy dismissal → `disposition-calibration.ts` → EMA-updated weights → (Phase 3) effective without rebuild; and non-buddy `review-pr` writes `pending-calibration`, `/learn-from-pr` consumes+clears it. No MCP surface — runtime-verification rows stay for a live buddy/review run to tick.
