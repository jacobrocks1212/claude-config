...
The shared protocol is embedded below (build-time embed — no runtime Read needed).
...
This scaffold defines your cache/local-codebase access model, tree-sitter usage, verdict taxonomy, and the exact output schema. Apply it in full. It is embedded below (build-time embed — no runtime Read needed).
