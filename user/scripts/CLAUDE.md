...
python3 lazy-state.py --ensure-runtime               # unified-pipeline-orchestrator Phase 5: ensure the dev runtime + MCP server are up AND current; print JSON {status: ready|booted|stale-rebuilt, mcp_tools_present, health_code}. Collapses the lazy-batch Step-1d.0 runtime-ensure dance (probe /health → stale-binary check → dev:restart bg → curl-until-200 → assert MCP tool present). AlgoBooth specifics (TCP 3333, npm run dev:restart, src-tauri/crates globs, asserted MCP tool) are PARAMETERIZED in lazy_core._ENSURE_RUNTIME_DEFAULT_CONFIG (caller-overridable dict) — NOT hard-coded into the shared harness flow. lazy_core.ensure_runtime takes injected probe/restart/stale_check callables so --test is hermetic; production uses a real urllib probe + background dev:restart + the stale_binary predicate.
