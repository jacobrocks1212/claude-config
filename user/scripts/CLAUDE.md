...
| `lazy_core/` | Package of submodules behind a byte-compatible PEP 562 lazy facade (`__init__.py`); the body currently lives in `_monolith.py` with the shared mutable kernel in `_ctx.py` (further seams are extracted in later phases). Shared, domain-agnostic helpers (sentinel/plan parsing, deliverable counting, receipt writers, diagnostics infra) imported by both `lazy-state.py` and `bug-state.py`. Owns the per-invocation `_DIAGNOSTICS` list. `write_completed_receipt(..., kind=)`/`has_completion_receipt(..., filename=)` are parameterized so the bug pipeline can write `FIXED.md` (`kind: fixed`). |
