...
| `benchmark_lazy_core_import.py` | **Phase-0 measurement harness** for `lazy-core-package-decomposition` — pure-read, stdlib. Records the SPEC's three KPI proxies in one command: cold `import lazy_core` warm wall-ms (best-of-N subprocess imports), pytest collect-only test count + wall-s for the `lazy_core` suite, and a largest-module LoC census (asserts "no post-split module >4K LoC" once the decomposition lands). `--repo-root` / `--iters` / `--json` / `--no-collect`; exit 0 always (a benchmark never gates) except a malformed `--repo-root` (exit 2). Not on any state-script path. |
