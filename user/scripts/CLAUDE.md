...
| `lazy_core/` | Package of 12 seam submodules behind a byte-compatible PEP 562 lazy facade (`__init__.py`, explicit-total name→submodule map — no fallback module; `_monolith.py` deleted at Phase-5): `_ctx` (shared mutable kernel: `_DIAGNOSTICS`/`_diag`/`_atomic_write`/`_die`), `statedir` (hook-touched state-dir surface), `docmodel` (sentinel/plan/PHASES parsing + sentinel lifecycle), `depdag` (queue deps + merged ordering + queue mutation ops), `hostcaps`, `notifyplane`, `gates` (completion evidence/receipts/archive-on-fix/gate-coverage), `ledgers` (deny/friction/telemetry/provenance/interventions + feature-dir state ledgers), `dispatch` (cycle/dispatch prompts + prompt registry), `runtimeplane` (ensure_runtime/spawn/transient-build + git helpers), `markers` (run/cycle markers, ownership, refusals, checkpoints, budget counters), `pseudo` (`apply_pseudo`). Every submodule ≤4K LoC (benchmark census asserts). Shared, domain-agnostic helpers imported by both `lazy-state.py` and `bug-state.py`. Owns the per-invocation `_DIAGNOSTICS` list. `write_completed_receipt(..., kind=)`/`has_completion_receipt(..., filename=)` are parameterized so the bug pipeline can write `FIXED.md` (`kind: fixed`). |
| `bug-state.py` | Bug-lifecycle state machine over `docs/bugs/`. Same JSON contract as `lazy-state.py`; research/Gemini/stub steps dropped; terminal action is **archive-on-fix** (`__mark_fixed__`). Hybrid ordering (`queue.json` overrides, then severity + Discovered date). In-file `--test` smoke harness. Imports `lazy_core`. **`--fsck`** (fixed-bugs-unarchived-fsck): read-only lint asserting the archive-on-fix invariants — `unarchived-fixed` (Fixed+receipted dir outside `_archive/`), `fixed-without-receipt` (Fixed with no valid receipt), `stale-queue-entry` (a queue.json row pointing at a Fixed/archived dir). Exit 0 clean / 1 named violations; never mutates. See `docs/bugs/CLAUDE.md` → "Fixing a bug OUT-OF-PIPELINE". **`--ack-deny <selector> --resolution <text>`** (meta-dispatch-not-by-reference-and-ack-overpriced): cheap per-entry deny-ledger ack (no full hardening dispatch) with same-cause dedup (`lazy_core.ack_deny_by_selector`); orchestrator-only (`refuse_if_cycle_active`). Coupled-pair mirror in `lazy-state.py`. |
| `lazy_coord.py` | **Concurrency plane** (Phase 4; generalized by `parallel-worktree-batch-execution`) — net-new, **kept separate from `lazy_core.py`** (stdlib-only; MUST NOT import `lazy_core` — its lanes.json writer carries its own temp+`os.replace` atomic write, a documented justified duplication of `_atomic_write`). `os.mkdir` global lock (**stale-holder reclamation, `coord-lock-no-stale-reclaim`:** a successful `mkdir` writes `owner.json` `{pid, kernel_start_time, acquired_at}` into the lock dir via write-temp-then-`os.replace` — the `_write_leases` pattern; a losing `mkdir` reclaims via atomic rename-then-`shutil.rmtree` ONLY on a CONFIRMED-dead holder (`_confirmed_dead_owner`: the recorded pid no longer exists, OR is alive but its live `kernel_start_time` mismatches the recorded one — pid reuse), mirroring the build-queue `active.lock` precedent (`Get-ActiveLockStatusFromText`/`Test-ShouldReclaimLock`: an unreadable pid is `'unknown'`, never `'dead'`). A metadata-less lock (crashed between `mkdir` and the metadata write) reclaims only past a bounded grace age (dir mtime, default 2.0s) — inside the grace window it is ambiguous, not stale. `kernel_start_time` is DUPLICATED from `lazy_core.py` (same policy as `_current_head`'s documented duplication) since this module must not import `lazy_core`. A genuinely live/ambiguous lock still raises `TimeoutError` within the unchanged timeout/backoff budget — no caller signature changes.) + fencing-token leases (`leases.json`, with a sibling `lease-token-watermarks.json` preserving per-item `term_token` monotonicity ACROSS reclamation/release — the zombie-lane fix), heartbeat, expiry reclamation, worktree-pool provisioning + scrub-to-clean (now repo-agnostic: `repo_root` param, `branch_template=`/`detach_target=` kwargs with byte-identical Cognito defaults), plus the **parallel-lane layer**: `claim_shardable` (conservative D3 predicate over caller-computed dep-ready/independent booleans + the live-lease rail), the `lanes.json` lane ledger (`ledger_record_claim/_lane_complete/_merge/_demotion/_park`, `read_lanes`), `merge_order` (deterministic queue-order), `merge_lane_branch` (abort-and-demote, lane branch preserved), `merge_back_lanes` (`concurrent-worktree-agent-coordination` Phase 5: the merge-back orchestration wiring — a THIN composition over `merge_order`+`merge_lane_branch`+`ledger_record_merge`/`_demotion` that merges lane-complete lanes back in QUEUE order, abort-and-demote on conflict; no new merge engine), `flush_summary`, `effective_lanes`/`lane_budget_slice` (D6 arithmetic), `lane_branch`/`lane_pool_dir` (lane conventions: `lane/<item-id>`, sibling `<repo_root>-lanes/wt-NN`). Consumed by the `lazy-worker` and `/lazy-batch-parallel` skills. In-file `--test` smoke harness (28 fixtures — fixtures 23–26 cover the per-queue-item FIFO lock: same-key serialize, different-key no-block, confirmed-dead reclaim, live-holder no-false-reclaim; fixtures 27–28 cover the Phase-5 merge-back-in-queue-order + conflict-aborts-and-demotes). See "Concurrency plane — sanctioned parallel worktree lanes" below. Per-queue-item FIFO lock (`concurrent-worktree-agent-coordination` Phase 3): `acquire_item_lock`/`release_item_lock`/`ItemLockTimeout` — a polling mutex over `acquire_lease`/`verify_fencing`/`release_lease` keyed by `wi_id` (NO new substrate; injected `now`/`clock`/`sleep` for hermetic `--test`), the stdlib-Python plane of the two-implementation lock contract whose PowerShell workstation plane is `concurrent-lock.ps1`. |
| `concurrent-lock.ps1` | **PowerShell workstation plane of the per-queue-item FIFO file lock** (`concurrent-worktree-agent-coordination` Phase 3) — net-new, self-contained (does NOT import build-queue). The workstation implementation of the ONE documented lock grammar (`user/skills/_components/concurrent-lock-contract.md`), conforming INDEPENDENTLY of the stdlib-Python plane (`lazy_coord.py`'s `acquire_item_lock`/`release_item_lock`) — a *documented grammar, not shared code* (runner-outcome two-implementation pattern). An `active.lock`-style per-item lock file keyed by queue-item id (per-item grain, Locked Decision 1): acquire = wait-for-unlock FIFO (bounded backoff, never steals a live holder); fencing-token release; CONFIRMED-dead reclaim only (dead/reused pid via `Get-LockHolderStatus`, mirroring `build-queue.ps1`'s `Get-ActiveLockStatusFromText`/`Test-ShouldReclaimLock` — ambiguous/live never reclaimed); authoritative `concurrent-lock:` LAST-stdout-line banner (runner-outcome-contract.md); exit 0/124/1/2. Silently inert off-Windows. Pure functions dot-sourceable (dot-source guard on `$MyInvocation.InvocationName`). Tests: `concurrent-lock.Tests.ps1` (Pester, 8 tests). |
...
| `cli_surface.py` | **Shared CLI-surface introspection lib** (state-cli-contract-registry Phase 1/3) — stdlib, imported by both state scripts + the other five roster scripts. `add_dump_cli_surface_flag(parser)` adds a self-describing `--dump-cli-surface` flag; `dump_parser_surface(parser)` introspects a LIVE `ArgumentParser._actions` into the schema-v1 flags projection (name/aliases/action/nargs/required/choices/metavar/help_head/group/positional/default_kind — never a default VALUE, which may be env-dependent); `maybe_handle_dump_cli_surface(args, parser, script_name)` is the one-line wiring called immediately after `parser.parse_args(...)` in every roster script's `main()`. Also carries `DidYouMeanArgumentParser` (Phase 3, D4-A) — an `ArgumentParser` subclass wired into `lazy-state.py`/`bug-state.py`'s `build_parser()` that appends a `difflib`-suggested near-miss flag on an "unrecognized arguments" error while keeping the leading `error:` line + exit code byte-identical (additive epilogue only). Tests: `test_cli_surface_gen.py`. |
| `cli_surface_gen.py` | **CLI-surface registry aggregator + freshness gate** (state-cli-contract-registry Phase 1, D1/D3) — stdlib. `ROSTER` is the closed v1 scope (`lazy-state.py`, `bug-state.py`, `surface_resolver.py`, `lazy_parity_audit.py`, `kpi-scorecard.py`, `lint-skills.py`, `doc-drift-lint.py`); `generate_registry` shells each roster script's own `--dump-cli-surface` subprocess and merges the results (never re-implements argparse semantics — a projection can never drift from the parser it introspects). Default mode regenerates + writes the committed, key-sorted, byte-stable `docs/cli/cli-surface.json`; `--check` regenerates to memory and diffs against the committed file, exiting 1 naming any added/removed/changed flag per script (the freshness gate — a flag change forces the registry to update in the same commit). `--repo-root`; exit 0 clean/regenerated, 1 drift (`--check` only), 2 a roster script's dump subprocess failed. NOT on the state-script compute path — an author-time/CI check. Tests: `test_cli_surface_gen.py` (incl. a live self-check that the real repo's committed registry is fresh — the regression net against a roster script's argparse changing without a regen). |
| `cli-surface-lint.py` | **Prose/fence lint of `--flag` mentions against the registry** (state-cli-contract-registry Phase 2, D2-A) — stdlib, sibling of `doc-drift-lint.py` (same committed-claims-vs-reality family; reuses its `<!-- marker -->` exemption precedent as `<!-- cli-surface: historical -->`). Scans `user/skills/**/SKILL.md`, `user/skills/_components/*.md`, `repos/*/.claude/skills/**/*.md`, `repos/*/.claude/skill-config/**/*.md`, and `user/scripts/CLAUDE.md`. Attribution rule: a `--flag` token is checked against a roster script's registry entry only when EXACTLY ONE roster script is named in the same attribution unit — a logical line (backslash-continuations joined) further split on `.`/`;` boundaries so a single markdown-table-cell line packing multiple scripts' descriptions doesn't cross-attribute a flag from one clause to an unrelated script mentioned elsewhere in the same physical line; zero or >1 mentioned scripts ⇒ ignored (false-positive control). An unknown flag is an ERROR naming file:line, the attributed script, the flag, and the nearest registered flag (`difflib`, cutoff 0.3 — a best-effort hint, not a confidence gate). Reachable standalone (`python3 cli-surface-lint.py --repo-root .`) or via `lint-skills.py --check-cli-surface`. Exit 0 clean / 1 findings / 2 malformed (registry missing/unreadable — run `cli_surface_gen.py` first). **Known v1 imprecision:** this file's own script-table rows (giant single-line table cells discussing several scripts per row) occasionally still cross-attribute a flag across clauses with no `.`/`;` boundary between them (e.g. a flag genuinely owned by a non-roster tool, mentioned near a roster script's name later in the same clause) — verified false positives, not stale docs; left unmarked rather than mass-adding exemption comments across a 900+-line doc for a v1 heuristic. A tighter clause-boundary grammar is a documented vN follow-up. Tests: `test_cli_surface_lint.py`. |
| `harness-gate.py` | **Anti-overfit / gate-weakening design-gate checker** (anti-overfit-design-gate) — stdlib, **READ-ONLY over git** (`git diff` / `--name-only`; never writes; not on any state-script path; does not import `lazy_core`). Inspects a diff against the committed control-surface manifest `docs/gate/control-surfaces.json` (D1); IN SCOPE iff ≥1 changed path matches a manifest glob (else `in_scope: false`, exit 0 — ordinary work + non-claude-config repos byte-identical). Four STRUCTURAL detectors (D2 — keyed on diff SHAPES, never incident literals, so the checker passes its OWN overfit check; its files are on the manifest's `gate_own` block): **overfit** (matcher-alternation/list/set/allow-list literal append + incident-shaped `docs/{features,bugs}/<slug>`/dated/session literal → `flag`), **gate_weakening** (`def test_*` deletion, numeric-literal-only change on a gate line, exemption/sanction-set membership add, `*_BYPASS` env-var, `permissionDecision: deny`/`refuse_*`/`exit 3` removal → `hit`, routes to operator sign-off — NEVER judgment-passable), **tautology** (`## Intervention Hypothesis` presence + `signal_independence` substance), **complexity** (`retires:` declaration required in scope). A flag is NOT a verdict — the adversarial half (`_components/harness-change-gate.md`) records judgment in `GATE_VERDICT.md`; blocking authority lives ONLY at the completion-gate ship seam (`lazy_core.gate_verdict_ok` — **WIRED + live-blocking**; the "SEAM-DEFERRED" note is retired), whose authoring seam is the `gate-verdict` completion-time dispatch class (GAP 1). `--repo-root` / `--range A..B` / `--staged` / `--feature-dir` / `--json`; exit 0 pass/out-of-scope, 1 verdict-required, 2 malformed. Named regression fixtures: `_VERIFICATION_SECTION_RE` phrase-append + GAP-2 exemption-add. `/harden-harness` Step 3 delegates smell detection here. Tests: `test_harness_gate.py` (22 fixtures). |
| `skill-size-ratchet.py` | **Per-file skill-size ratchet lint** (lazy-batch-skill-deflation D3) — stdlib, sibling of `lint-skills.py` (also reachable via its `--check-skill-size` flag). Committed opt-in baseline `skill-size-baseline.json` (byte ceiling + >500-char long-line-count ceiling per file). `check(repo_root, baseline)` fails naming any listed file over either ceiling; `--lock-in <path>` LOWERS a ceiling to the file's current size on improvement only (`min(current, existing)` — never raises, so a transient deletion can't set an unreachable bar); `--lock-in --new <path>` seeds a not-yet-listed file. A file absent from the baseline carries no ceremony. Writes go through `lazy_core._atomic_write`. Tests: `test_skill_size_ratchet.py` (incl. a live self-check that the real baseline + repo tree are clean). |
| `generate-coupled-skills.py` | **Coupled-pair SKILL.md generator** (coupled-pair-generation) — derives a coupled/derived skill from its canonical + a per-pair overlay (`coupled-overlays/<pair>.overlay.json`): each overlay directive is either `"canonical"` (restated — `apply_tokens(canonical_block)`, imported from `lazy_parity_audit`, never re-implemented) or `"verbatim"` (stored divergent/inserted block, byte-exact); a canonical block absent from the overlay is deleted by omission. `--check` (default, drift gate — byte-diffs regenerated vs. committed), `--write`, `--extract` (rebuild overlays from committed canonical+derived), `--report` (per-pair block accounting); `--pair <name>` / `--repo-root`. Exit 0 ok / 1 drift-or-failure (`--check`/`--write`) / 2 malformed input. **PROVISIONAL** — a 2026-07-12 extraction found most blocks store `verbatim` even on the zero-token-sub cloud axis (measured line-diff ratios 0.3–0.8); byte-faithful by construction regardless. See the feature's `NEEDS_INPUT_PROVISIONAL.md`. |
| `lint-skill-config.py` | **Schema + reference lint for `repos/*/.claude/skill-config/`** (skill-config-schema-and-reference-lint) — stdlib. Three checks: (1) each repo's `MANIFEST.json` `provides`/`intended_absent` bidirectional validation (declared-but-missing and present-but-undeclared both error); (2) structural JSON-schema check for registered configs (today: `build-queue-ops.json`); (3) a reference sweep of every `.claude/skill-config/<file>` mention across user-level and repo-scoped skill sources, flagging a dangling/undeclared reference (an `intended_absent` reference needs a fallback form to be rescued — a bare pointer is not enough). A `SUPPRESSIONS` allowlist (reason-required) covers known-dead references this feature doesn't own the fix for. `--repo-root`; exit 0 clean (warnings allowed) / 1 errors / 2 malformed CLI. Stdlib, pure read. |
| `benchmark_lazy_core_import.py` | **Phase-0 measurement harness** for `lazy-core-package-decomposition` — pure-read, stdlib. Records the SPEC's three KPI proxies in one command: cold `import lazy_core` warm wall-ms (best-of-N subprocess imports), pytest collect-only test count + wall-s for the `lazy_core` suite, and a largest-module LoC census (asserts "no post-split module >4K LoC" once the decomposition lands). `--hook-surface` (Phase 2 WU-5): best-of-N fresh-subprocess `import lazy_core` + touching the 3 hook-touched names, recording whether `_monolith` loaded (always 0 post-Phase-5 deletion — structural, not merely observed). **`--function-sizes`** (Phase 6, the D7 compute_state follow-up baseline): `ast`-based top-level function LoC census of `lazy-state.py` / `bug-state.py` (NOT `lazy_core/`, already decomposed) via `measure_function_sizes` — reports every top-level function per file sorted desc by LoC, flags each file's `compute_state` explicitly (2026-07-13 baseline: `lazy-state.py::compute_state` 2,144 ln / `bug-state.py::compute_state` 1,239 ln); measurement ONLY, neither monolith is refactored by this feature. `--repo-root` / `--iters` / `--json` / `--no-collect`; exit 0 always (a benchmark never gates) except a malformed `--repo-root` (exit 2). Not on any state-script path. Tests: `tests/test_lazy_core/test_misc.py::test_benchmark_function_sizes_reports_compute_state_monoliths`. |
...
| `gate-battery.py` | **Stdlib-only cross-platform gate-battery runner** (generalized-build-test-runner-skills Phase 1) — the cloud-capable Python instantiation of the runner-outcome contract (`user/skills/_components/runner-outcome-contract.md`, the SSOT for the banner grammar + await semantics). Runs the battery declared in the invoking repo's `.claude/skill-config/gate-battery.json` (`{version: 1, gates: [{id, cmd, cwd?}]}` — claude-config seeds the invariant battery AS COMMANDS, SPEC L5) sequentially (a failing gate never stops the battery; first failing id recorded), streams gate output, and prints the authoritative LAST-stdout-line banner `gate-battery: run=<id> op=battery RESULT=<PASS\|FAIL> cmds=<n> failed=<k> (elapsed=<s>s) [-> first failing gate: <id>]` (try/finally-guaranteed, even on internal error). Results at `~/.claude/state/gate-battery/<repo-key>/results/<run-id>.json` (private `_repo_key` copy of `lazy_core/statedir.py::repo_key`, keep-in-sync comment — the runner NEVER imports `lazy_core`, SPEC L4). `--await <run-id>` re-emits the recorded banner as ITS last line + exits with the run's own exit code; **124** = not-yet (NEVER success), **125** = malformed result (mirrors `build-queue-await.ps1`); `--timeout-seconds` bounded poll (default 0 = single-shot); await never needs the manifest. Manifest-less repo → clean exit-2 refusal, zero state; state-root-unwritable degrades gracefully (banner notes `await unavailable`). Exit vocabulary: 0 pass / 1 fail / 2 no-manifest / 124-125 await-reserved. CLI-surface roster member (`--dump-cli-surface`, `DidYouMeanArgumentParser`). Tests: `tests/test_gate_battery.py` (18, mostly hermetic tmp state roots — one reads the committed repo manifest). |

## Runner-outcome contract (the banner/await/turn-end grammar)

The authoritative last-stdout-line banner grammar, the followable-await 124/125 semantics, the
never-pipe-through-tail rule, and the turn-end gate (by reference) that any conforming build/test
runner in this workspace obeys are documented **once** in
`user/skills/_components/runner-outcome-contract.md`. This is the cross-repo seam — a *documented
grammar, not shared code*: the PowerShell build-queue plane (`build-queue*.ps1`, workstation-only)
and the stdlib-Python gate battery (`gate-battery.py`, cloud-capable) each conform independently.
The gate-battery runner's own script-table row is below (added with the script in Phase 1); this
paragraph is the prose pointer to the contract component, which is a skill component — not a
`user/scripts/` file — and so is deliberately NOT a script-table row.
...
- **Loop-detection state is run-scoped, not machine-wide.** The `repeat_count` streak signature and
  `pending_hardening()`'s mandatory-debt read are both stamped/compared against the LIVE run
  marker's `run_started_at` — a crashed run's leftover streak or undrained deny-ledger entry no
  longer leaks into the next run's first probe on the same `(feature_id, current_step)` tuple
  (leftovers surface informationally via `prior_run_pending_hardening()`, never silently block).
  The debounce oracle also filters `consumed_emission_count` to `cls="cycle"` so a mid-step
  hardening/investigation/recovery dispatch never inflates the streak
  (`loop-detector-false-positives-probes-and-cross-run-state`).
- **Hermetic subprocess tests MUST pass explicit `--repo-root` to a temp fixture repo — never rely
  on `os.getcwd()`.** Isolating `LAZY_STATE_DIR` alone is not enough: an un-isolated `--repo-root`
  binds the subprocess to the REAL checkout, so a test can silently validate the wrong gate against
  real on-disk state (e.g. a real `docs/interventions/` read) instead of the one it names and pins
  (`adhoc-run-end-tests-leak-real-repo-state`). Any helper that reads repo-relative state (e.g. an
  efficacy-breadcrumb seeder) must accept an explicit fixture-root override rather than defaulting
  to the marker's/caller's `repo_root`.
...
- **Advisory, not enforcing.** Exit code is informational — this gate is NOT wired into the
  invariant battery, any hook, or CI. Flipping to enforcing is a later, separate
...
python3 lazy-state.py --park-blocked        # batch park mode companion: skip (park) a feature/bug-local BLOCKED.md into parked[] (sentinel_kind: blocked) instead of halting on terminal_reason=blocked; the /lazy-batch skill's `--park` convenience flag passes BOTH `--park-needs-input` and `--park-blocked`. Global/env terminals (cloud/device/research/scoped-id) still halt. Output byte-identical without either flag. Same two flags on bug-state.py.
...
python3 lazy-state.py --next-merged          # unified-pipeline-orchestrator Phase 1: print the head of the MERGED feature+bug work-list as JSON {item_id, type, repo_root} (or null when both queues empty). Read-only ORDERING ONLY — reuses load_queue (features) + bug-state.load_bug_queue (bugs, via importlib) and the lazy_core ordering helper; NEVER re-infers per-item state (the unified driver still calls --probe/--emit-prompt per item). Normalizes the two queues' divergent ordering fields (feature `tier` / bug `severity` P0..Low) to one effective-priority scale (lower = higher priority); equal priority → bug before feature; stable within each queue. Binds the active repo before reading. Shared impl: lazy_core.merged_priority/merged_worklist/next_merged. **Feature `tier` NAMED ENUMS** (feature-tier-strings-fall-to-merged-priority-default) map onto the SAME integer scale as bug severity, so a feature and a bug are directly comparable — SSOT `lazy_core._FEATURE_TIER_ENUM`: `pre-release=1` (operator-LOCKED, == P1 — load-bearing: P0 bug 0 < pre-release feature 1 < P2 bug 2), `commercialization=2`, `milestone=3`, `major-initiative=4`, `4a=4`, `4b=5`, `follow-up=6`, `non-audio=7` (the seven legacy-string values are a provisional recommendation pending operator ratification — see `docs/bugs/_archive/feature-tier-strings-fall-to-merged-priority-default/`). A `tier` may be a single enum name, a bare int / numeric string (backward-compat, passed through), OR a **list** of those — a multi-enum feature whose effective priority is the **MIN** (highest-priority enum wins). Null / missing / unrecognized → `MERGED_PRIORITY_DEFAULT` (99, sorts last), byte-identical to pre-unification. `--set-tier` accepts enum names (single or comma-separated).
  ...
  *different* repo resolves to a different subdir → absent → the hook is a no-op. Fail-OPEN: a
  ...
  - **Run-start owner bind + owner-side detect/re-arm (`single-slot-marker-ownership-race-disarms-owning-run`,
    ...
    - **Owner detect + re-arm backstop (Phase 2 — for the legacy/un-threaded paths).**
      ...
      `lazy_parity_audit.py::audit_state_script_parity`, which checks that both `lazy-state.py`
      and `bug-state.py` still expose `--reassert-owner`). See
...
--feature-id <id> --current-step <step>` on `lazy-state.py` (mirrored as `--bug-id` on `bug-state.py`)
calls `record_resolution_signal`. `/lazy-batch` and `/lazy-bug-batch` Step 1g
...
- **Gate:** `python lazy_coord.py --test` (28 fixtures). Because the scoping flags touch both state
