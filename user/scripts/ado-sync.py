...
def install_task(repo_root: Path) -> None:
    ...
    DASHBOARD.md periodically, with no console window.

    Generates a machine-specific Python wrapper (absolute python + repo paths,
    so it is gitignored, not committed) that runs `ado-sync.py --once` then
    `work-status.py --markdown`. The task launches the wrapper via pythonw.exe
    (the windowless interpreter) and the wrapper spawns each step with
    CREATE_NO_WINDOW, so nothing flashes a console on the 10-min cadence.

    Registered via PowerShell's ScheduledTasks module rather than schtasks:
    schtasks cannot clear the default battery start-conditions, which leave the
    task stuck "Queued" on a laptop. -StartWhenAvailable catches up a missed run
    after the machine wakes from sleep; -ExecutionTimeLimit caps a hung instance
    so IgnoreNew never wedges the schedule. Interval is derived from config
    pool.heartbeat_interval_seconds (default 600s = 10 min).
    """
    ...
    # Prefer the windowless interpreter (pythonw.exe) for the task action so the
    # wrapper itself owns no console; fall back to python.exe if absent.
    pythonw_exe = python_exe
    if python_exe.lower().endswith("python.exe"):
        candidate = Path(python_exe).with_name("pythonw.exe")
        if candidate.exists():
            pythonw_exe = str(candidate)
    ...
    # Python wrapper chains sync -> render. Children spawn with CREATE_NO_WINDOW
    # (no console) and PYTHONUTF8=1 so Unicode in ADO titles can't crash cp1252.
    wrapper_path = scripts_dir / "_ado-mirror-refresh.py"
    wrapper_py = f'''"""Auto-generated by ado-sync.py --install-task. Machine-specific
(absolute paths); gitignored, not committed. Run headless by the AdoMirrorSync
scheduled task via pythonw.exe."""
import os
import subprocess
...
REPO = r"{repo_root}"
SYNC = r"{script_path}"
RENDER = r"{render_script}"

_env = dict(os.environ, PYTHONUTF8="1")
_NO_WINDOW = 0x08000000  # CREATE_NO_WINDOW — suppress child consoles

for _cmd in (
    [PYTHON, SYNC, "--once", "--repo-root", REPO],
    [PYTHON, RENDER, "--markdown", "--repo-root", REPO],
):
    subprocess.run(_cmd, env=_env, creationflags=_NO_WINDOW, check=False)
'''
    wrapper_path.write_text(wrapper_py, encoding="utf-8")

    task_name = "AdoMirrorSync"
    ps_script = f"""$ErrorActionPreference = 'Stop'
$action   = New-ScheduledTaskAction -Execute '{pythonw_exe}' -Argument '"{wrapper_path}"'
$trigger  = New-ScheduledTaskTrigger -Once -At (Get-Date) -RepetitionInterval (New-TimeSpan -Minutes {interval_minutes})
$settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -MultipleInstances IgnoreNew -Hidden -ExecutionTimeLimit (New-TimeSpan -Minutes 5)
