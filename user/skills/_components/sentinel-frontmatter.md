...
- `stub_origin: true` — **stub-origin marker (stub-origin-provisional-exclusion).** Set by the PRODUCER when this sentinel's decisions shaped a BASELINE the operator has never seen: `/spec`'s "Phase 1 under `--batch`" gating-decision halt on a stub/pre-baseline SPEC, and `/spec-bug`'s pre-conclusion halt (`**Status:** Investigating`). The Step 1d.5 input-audit backstops the marker (adds it when the audited cycle was a stub-spec baseline round and the producer omitted it). Effect: the sentinel is NEVER provisional-eligible (`lazy_core.provisional_eligibility` refuses on any present value that is not an explicit `false` — fail-closed on malformed values); it parks for the operator like any product-class decision. Inert outside `--park-provisional`. Post-research Phase-3 sentinels (operator-locked baseline) do NOT carry it. **Exception — the claude-config auto-generated carve-out** (see `auto_generated` below): a `stub_origin: true` sentinel that ALSO carries `auto_generated: true` + a recognized `auto_generated_origin:` in the claude-config checkout IS provisional-eligible (a machine-generated triage stub is not an operator-shaped baseline).
- `auto_generated: true` + `auto_generated_origin: canary-revert | incident-capture` — **harness-auto-generated-stub marker (park-provisional-parks-claude-config-auto-generated-stubs).** Stamped by the two harness generation sites onto their seeded bug-dir capsule frontmatter (`efficacy-eval.py --canary` → `EVIDENCE.md` `kind: canary-evidence` → `auto_generated_origin: canary-revert`; `incident-scan.py` → `INCIDENT.md` `kind: incident-capture` → `auto_generated_origin: incident-capture`), then PROPAGATED by `/spec-bug`'s pre-conclusion halt (and backstopped by the Step 1d.5 input-audit) onto the `NEEDS_INPUT.md` it writes, ALONGSIDE `stub_origin: true`. Effect: `lazy_core.provisional_eligibility` bypasses the `stub_origin` exclusion — auto-accepting the recommended option under `--park-provisional` — ONLY when `auto_generated: true` AND `auto_generated_origin` ∈ {`canary-revert`, `incident-capture`} AND the sentinel lives under the claude-config checkout (repo root carrying both `manifest.psd1` and `user/settings.json`). A machine-generated triage stub is not an operator baseline, so its recommended option (typically close-as-noise) should not demand an operator decision. NEVER an id-prefix match (`canary-revert-*`) — a structural origin tag. Fail-closed: any missing/malformed piece ⇒ the exclusion stands and the stub parks. Inert outside `--park-provisional` and outside claude-config.
...
- `conflict_kind: semantic` — **semantic-conflict marker (concurrent-worktree-agent-coordination Phase 4; `class: product`).** Set by the conflict router when a concurrent-writer conflict that slipped past the FIFO file-lock is classified SEMANTIC by `lazy_core.classify_conflict` — an un-auto-resolvable conflict on the SAME logical artifact (same function / Locked-Decision row / sentinel), OR an ambiguous/undeterminable conflict (which fails safe to semantic). A semantic-conflict `NEEDS_INPUT.md` is ALWAYS `class: product` (reconciling two agents' divergent intent needs human judgment) and is **NEVER provisional-eligible** — `lazy_core.provisional_eligibility` refuses on `conflict_kind: semantic` via a fail-closed carve-out placed with the spike / `stub_origin` exclusions, so it parks for the operator even with otherwise-eligible `isolated/isolated` divergence grades. By contrast a WRITE conflict (git auto-merges, or the conflicting hunks touch disjoint surfaces) is **non-halting** and writes NO sentinel — it is retried/queued through the FIFO lock and the run continues. Absent ⇒ not a semantic-conflict halt (byte-identical to every existing `NEEDS_INPUT.md`). **Schema-lockstep note:** the coupled AlgoBooth mirror `scripts/check-docs-consistency.ts` `SENTINEL_SCHEMAS` must register this optional field when that sibling checkout is present (it is absent from the claude-config checkout — tracked as a cross-repo follow-up in the Phase-4 plan/Implementation Notes; `conflict_kind` is an OPTIONAL addition to the pre-existing `needs-input` schema, so an un-updated mirror keeps parsing existing sentinels).

**Provisional-acceptance rule (park-provisional-acceptance — enforced by `lazy_core.provisional_eligibility`, `--park --park-provisional` only):** a `NEEDS_INPUT.md` MAY be PROVISIONALLY accepted (recommended option taken, `## Resolution` appended `resolved_by: auto-provisional` with the HEAD `decision_commit`, file RENAMED to `NEEDS_INPUT_PROVISIONAL.md` — never neutralized) ONLY when ALL hold: (1) both park flags active; (2) `divergence` AND `audit_divergence` both ∈ {isolated, contained}; (3) every decision carries a `**Recommendation:**` block (recommendation-first options); (4) the file is NOT two-key mechanical (`class: mechanical` + `audit_concurs: true` → the flush auto-accept is the stronger path — full resolution, no ratification debt); (5) `written_by` is not `completion-integrity-gate`; (6) `stub_origin` is absent or explicitly false (stub-origin-provisional-exclusion — baseline-shaping decisions always park) — UNLESS the claude-config auto-generated carve-out applies (`auto_generated: true` + recognized `auto_generated_origin:` in the claude-config checkout; park-provisional-parks-claude-config-auto-generated-stubs), in which case the `stub_origin` exclusion is bypassed; (7) `conflict_kind` is not `semantic` (concurrent-worktree-agent-coordination Phase 4 — a semantic concurrent-writer conflict is never auto-accepted; both callers — the park-mode routing peek and `--provisionalize-sentinel` — re-run `provisional_eligibility`, so the carve-out is airtight). The renamed `NEEDS_INPUT_PROVISIONAL.md` keeps `kind: needs-input` (the FILENAME is the state carrier, same convention as `_RESOLVED_`): park-mode probes treat it as workable (the feature keeps implementing; surfaced via the park-mode-only `provisional[]` probe key), a NON-park probe halts on the `needs-ratification` terminal, and the completion pseudo-skills (`__mark_complete__` / `__mark_fixed__`) MECHANICALLY refuse while it exists. Ratification (the `provisional-ratification.md` affordance) appends a `## Ratification` block (`outcome: ratified | redirected`) and neutralizes via the standard rename (`NEEDS_INPUT_PROVISIONAL_RESOLVED_<date>.md`); a redirect additionally propagates the changed choice and authors a `**Phase kind:** corrective` phase scoped by `git diff <decision_commit>..HEAD`. Decision-tier summary: **scope-class → D7 standing policy (both modes, no ask)** · **mechanical two-key → flush auto-accept (`--park`)** · **product-class, low divergence two-key + recommendation → provisional accept (`--park --park-provisional`, ratify-or-redirect before completion)** · **stub-origin baseline decisions (`stub_origin: true`), and everything else → park / ask the operator.**
