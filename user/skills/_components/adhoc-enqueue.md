...
- **Bug (`--type bug`):** lands at the top of `docs/bugs/queue.json` (via the **existing** `bug-state.py` enqueue — not a reimplementation), with a `docs/bugs/<bug_id>/` dir, an `ADHOC_BRIEF.md`, and a minimal **gate-compliant stub `SPEC.md`** (`**Status:** Investigating`, a canonical `**Severity:**` token, `**Discovered:**`) so the dir resolves as an open bug directory from the first probe — `/spec-bug` then overwrites the stub with the full investigation SPEC. From there it flows through the bug pipeline (`/spec-bug` investigation → `/plan-bug` → `/execute-plan` → MCP → mark-fixed). Use this for a harden-harness spin-off or any "this is a defect, not a feature" item.
  - The queue entry carries a `severity` field **only** when an explicit override is supplied — a bare auto-file/ad-hoc enqueue OMITS it (never `severity: null`), leaving the stub SPEC's `**Severity:**` as the source of truth. Both are required to keep the target repo's `qg:bugs-consistency` gate green (queue-schema + queue-dangling-id + bug-status/severity-canonical).
