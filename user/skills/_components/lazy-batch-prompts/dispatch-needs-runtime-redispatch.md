...
You are running a NEEDS-RUNTIME re-dispatch of an mcp-test cycle for the autonomous pipeline. A prior mcp-test run returned NEEDS_RUNTIME — it discovered an MCP-testable surface that the plan's "MCP runtime: not-required" declaration missed. The orchestrator has now booted the Tauri dev runtime. Your job is to run the mcp-test cycle properly against the live runtime.
...
You are running a NEEDS-RUNTIME re-dispatch of an mcp-test cycle for the autonomous pipeline. A prior mcp-test run returned NEEDS_RUNTIME — it discovered an MCP-testable surface that the plan's "MCP runtime: not-required" declaration missed. Cloud runs cannot boot the Tauri runtime.
...
RUNTIME IS ALREADY UP — the orchestrator pre-booted tauri:dev before this dispatch. The MCP HTTP server is ready. Proceed with the /mcp-test skill exactly as if the runtime had always been required: invoke /mcp-test via the Skill tool in batch mode, connect to the live MCP server, execute the test scenarios from the {spec_path}/mcp-tests/ directory, write MCP_TEST_RESULTS.md with the results, and return the skill's structured summary.
...
2. Write MCP_TEST_RESULTS.md to {spec_path}/ per the sentinel schema (result, pass_count, total_count, validated_commit: HEAD, scenarios).
3. Commit: test({item_id}): mcp-test results (runtime re-dispatch). WORK-BRANCH-ONLY.
4. Push the work branch.

<!-- @section cloud-job pipelines=feature,bug modes=cloud -->
CLOUD RUN — the Tauri dev runtime is not available in this cloud session. Record this NEEDS_RUNTIME trigger (one line in the cycle log and in {spec_path}/BLOCKED.md or notes) and DEFER the mcp-test dispatch to a workstation run. Write a brief note explaining: the plan declared MCP runtime as not-required but the test cycle found a testable surface; a workstation run is needed to complete validation.
