...
2. Run the mechanical checker over the item's OWN shipped commits — the SAME item-commit derivation the completion ship seam (`gate_verdict_ok`) uses, so scope AGREES even when the fix is ALREADY MERGED to `origin/main` (an `origin/main..HEAD` range would be EMPTY and falsely report out-of-scope — `gate-verdict-dispatch-derives-scope-from-empty-range-for-merged-item`): `python3 ~/.claude/scripts/{state_script} --repo-root . --gate-verdict-check <the spec path shown above>`. It prints the same JSON shape harness-gate.py does (`in_scope`, `scope_hit`, per-check `checks`, `verdict_required`, `gate_weakening_hit`) plus `item_commits` (the shas the diff was built over). Read its `in_scope`, `scope_hit`, and per-check findings. If `in_scope: false` the completion refusal was spurious — report that and STOP (author nothing); otherwise proceed.
