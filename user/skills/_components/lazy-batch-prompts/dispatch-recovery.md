...
2. Reconcile per the failing_check:
   ...
   - deliverables_done: tick a verification box ONLY when there is on-disk evidence that verification actually ran (VALIDATED.md or MCP_TEST_RESULTS.md present in {spec_path}/ and covering that row). If verification boxes are unticked AND no such evidence exists, DO NOT tick them — instead write {spec_path}/NEEDS_INPUT.md (written_by: recovery, describing the gap) and commit it. Surface the issue; do not silently tick unverified boxes.

3. After reconciling, re-run the verify-ledger check to confirm ok. Report its output in your return summary.

   When {failure_summary} names a specific plan file (e.g. "plan_complete failing for
...
Re-run verify-ledger (feature pipeline):
  # Standard form (feature-level — use when no plan file is named in {failure_summary}):
   python3 ~/.claude/scripts/lazy-state.py --repo-root {cwd} --verify-ledger {spec_path}
  # Plan-scoped form (use when {failure_summary} names a plan file <plan_file>):
...
Re-run verify-ledger (bug pipeline):
  # Standard form (bug-level — use when no plan file is named in {failure_summary}):
