...
Long / Tier-2 / typegen builds run with `run_in_background: true` from the orchestrator session; while the build runs, dispatch the next **independent** agent (or next disjoint batch) instead of blocking the turn on the build. Background the heavy builds — full-solution `/msbuild` (no `-Project`, also the server-typegen trigger), `/msbuild -Test`, the typegen `Cognito.Services` build, `/nxbuild -All`, and fan-out Nx library builds; do NOT background the fast in-loop ones (`/msbuild -Project`, single-project `/nxbuild`, `--no-build` filtered `/mstest`/`/nxtest`). **Constraint guard:** an agent dispatched alongside a backgrounded build MUST be file-disjoint from and seam-independent of whatever that build verifies — a dependent (Sequenced) agent awaits the build's authoritative result first (`~/.claude/scripts/build-queue-await.ps1 -Seq <N>` to the `RESULT=` banner; its exit `124` = still running, never success — do not hand-read `results/<seq>.json`). Full policy + the OQ4 signature set live in the contract's **Parallelism & background builds** section (`~/.claude/skills/_components/execution-contract.md`).

### Turn-End Gate (MANDATORY — before ending any turn)

Backgrounding and dispatching buy overlap WITHIN your turn — never a license to end it "awaiting". A backgrounded job you have not read the terminal result of, an inner `Agent` dispatch whose returned report you have not consumed (dispatch-and-AWAIT — a child cannot re-invoke you after your turn ends), or a bare `build-queue: enqueued as seq=N` line is in-flight work, NOT an outcome. Canonical statement (the contract injects it; re-read if unsure): `~/.claude/skills/_components/turn-end-gate.md`.
