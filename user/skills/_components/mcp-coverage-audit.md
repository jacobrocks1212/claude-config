...
2. **Enumerate the MCP scenarios that own this feature.** List `{spec_path}/mcp-tests/*.md` (the per-feature scenario symlinks `/lazy` maintains). For each, read the file content. If the directory is empty AND `{spec_path}/SKIP_MCP_TEST.md` exists, apply the SAME provenance rule as `lazy_core.skip_waiver_refusal()` (keep this table in lockstep with `sentinel-frontmatter.md`):
   - `granted_by: operator`, OR `granted_by: mcp-test` with a non-empty `spec_class` citation, OR `granted_by` absent on a file whose `skipped_by` is NOT a pipeline author (`lazy` / `lazy-cloud` / `pipeline`): treat the skip as a legitimate test-exempt grant (audit passes vacuously — the waiver carries trusted provenance).
   - `granted_by: pipeline`, `granted_by: mcp-test` WITHOUT `spec_class`, or `granted_by` absent on a pipeline-authored file (`skipped_by: lazy` / `lazy-cloud` / `pipeline`): **do NOT treat as a valid coverage waiver.** A skip without trusted provenance is not a confirmed decision. Treat it exactly as if no `SKIP_MCP_TEST.md` existed — ALL decisions are uncovered and route to the Step 4 D7 outcome (author real coverage — the most complete path — unless a decision is documented MCP-untestable, in which case the test-exempt acknowledgement supersedes the unconfirmed skip for that decision).
