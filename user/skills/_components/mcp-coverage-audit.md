...
2. **Enumerate the MCP scenarios that own this feature.** List `{spec_path}/mcp-tests/*.md` (the per-feature scenario symlinks `/lazy` maintains). For each, read the file content. If the directory is empty AND `{spec_path}/SKIP_MCP_TEST.md` exists, check its `granted_by` field:
   ...
   - `granted_by: pipeline`: **do NOT treat as a valid coverage waiver.** A pipeline-self-granted skip is not an operator-confirmed decision. Surface it exactly as if no `SKIP_MCP_TEST.md` existed — treat ALL decisions as uncovered and write `NEEDS_INPUT.md`. The `NEEDS_INPUT.md` should note that `SKIP_MCP_TEST.md` exists but carries `granted_by: pipeline` and must be confirmed by an operator (update `granted_by` to `operator`) before the skip can waive audit coverage.
   If the directory is empty AND no `SKIP_MCP_TEST.md`, treat ALL decisions as uncovered (the audit will write `NEEDS_INPUT.md` proposing the default consolidated-scenario option per Step 4).
