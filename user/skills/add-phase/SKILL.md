...
**Verification rows carry the canonical marker (HARD — routes the phase to the MCP gate, not write-plan):** a corrective validation phase exists to add "run to PASS via `/mcp-test`" rows, and every Runtime Verification / MCP-validation `- [ ]` row this phase authors MUST be marker-exempt via the canonical `<!-- verification-only -->` marker (SSOT `lazy_core:_VERIFICATION_ONLY_MARKER`). Emit it on the `**Runtime Verification**` / `**MCP Integration Test Assertions:**` SUBSECTION HEADER line (header-scope — it exempts every row beneath, is authored ONCE on the fixed header string, and SURVIVES the freehand row authoring that drops per-row HTML comments — the live gap in `docs/bugs/verification-only-marker-dropped-on-freehand-rows`), AND on each row where practical. Do NOT rely on the header FREE TEXT alone (that unmarked form is the deprecated `_VERIFICATION_SECTION_RE` shim, which merely warns). `lazy-state.py`'s `remaining_unchecked_are_verification_only()` is MARKER-FIRST: an *unmarked* verification row is classified as an outstanding **implementation** item, so a phase whose only remaining unchecked rows are unmarked verification rows routes to `write-plan`/`execute-plan` instead of the Step-9 `/mcp-test` gate — the exact loop observed 2026-07-05 (recording-panel Phase 7: a `- [ ] Tests: … run to PASS via /mcp-test` row lacked the marker and routed to write-plan, not mcp-test, until the marker was hand-added). This is the same placement discipline `/write-plan` and `/spec-phases` already mandate; a corrective phase authored via `/add-phase` is the one authoring path that previously did not restate it. See `~/.claude/skills/_components/phases-runtime-verification.md`.
