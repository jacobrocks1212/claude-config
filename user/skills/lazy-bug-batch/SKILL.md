...
- **Post-`/execute-plan` and `/mcp-test` ledger-consistency guard (guardrail D):** see
  ...
  **Pause-vs-terminal discriminator gate (Gap 2 —
  ...
  re-invocation` and fall through to the next state probe (Step 1a). `verdict ==
  "wedge-candidate"` (marker present + plan not `Complete` + marker mtime STALE past the
  live-suite window — the dead-lineage stall
