...
#     including a pseudo-skill like __mark_fixed__ for a completion-gate meta cycle):
python3 ~/.claude/scripts/bug-state.py --cycle-begin --bug-id {bug_id} --nonce {dispatch_nonce} --kind {real|meta} --sub-skill {sub_skill}
