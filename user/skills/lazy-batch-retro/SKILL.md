...
- `docs/features/<area>/<feat>/PHASES.md` — capture every phase's `Status:` field AND every Implementation Notes block (apply sibling-then-embedded: check `docs/features/<area>/<feat>/IMPLEMENTATION_NOTES.md` first; fall back to embedded `## Implementation Notes` blocks in PHASES.md). These notes are the durable record of QG runs, test counts, design deviations.
...
| **R-O-2** Cycle count | `cycle_count ≤ --max-cycles` (default 10 unless `$ARGUMENTS` overrode). Read the orchestrator's start banner from the first assistant message in the parent jsonl to recover `max_cycles` — the T1 banner's `budget fwd {max_cycles} · meta …` line on post-contract runs (the `meta` term is `no cap` as of 2026-06-14; on older runs it was `{2*max_cycles}` — read `max_cycles` from the `fwd` term either way), or the legacy bookend's `**Max cycles:**` line on pre-contract runs. **R-O-2 caps only forward cycles** — meta_cycles is uncapped and is NOT subject to this rule. |
...
| **R-O-4** Prompt template compliance | Compare each cycle's dispatched `prompt` against the lazy-batch skill's base template (capture verbatim). Produce a unified diff. Flag missing load-bearing clauses: "Operating mode: batch", the mode-appropriate sub-subagent dispatch-policy block — `"WORKSTATION DISPATCH — LOAD-BEARING"` (workstation, post-2026-07-09) / `"INLINE OVERRIDE — LOAD-BEARING"` (historical workstation runs) / `"CLOUD OVERRIDE — LOAD-BEARING"` (cloud) — and the per-skill execution sections. (The legacy "follow the skill's internal subagent-vs-orchestrator rules" wording was replaced by the inline-override block — grade against the CURRENT template, captured verbatim, not a remembered older phrasing.) |
...
| **R-O-8** Stop-authorization (HARD — non-sanctioned early stop is a FAIL) | **(Phase 7 / lazy-validation-readiness. Incident reference: 2026-06-14.)** Verify the orchestrator did NOT end the run via a non-sanctioned stop. A non-sanctioned early stop is defined as: (a) a `--run-end --reason checkpoint` on an attended run (run marker `attended: true` or field absent) WITHOUT a preceding `AskUserQuestion` budget-and-queue-guard question AND without `--operator-authorized`, OR (b) a `--run-end --reason terminal --terminal-reason <reason>` (or bare `--run-end`) on a `terminal_reason` NOT in the sanctioned set {`all-features-complete`, `all-bugs-fixed`, `max-cycles`, `cloud-queue-exhausted`, `device-queue-exhausted`, `queue-missing`, `blocked-halt-for-manual`, `needs-research`, `queue-blocked-on-research`}. **Detection:** (1) read the run marker's `attended` field from the `--run-start` output in the parent jsonl; (2) locate the `--run-end` Bash tool_use; (3) verify `--reason terminal --terminal-reason <reason>` is present and `<reason>` is in the sanctioned set, OR `--reason checkpoint` is accompanied by `--operator-authorized` (attended) or the run's `attended` field is `false` (unattended checkpoint is sanctioned when a reliability trigger holds — ≥2 guard denials visible in the deny ledger, or an operator pause message). **Grade: `fail` (HARD) if a non-sanctioned stop is found** — this is not `partial`. A run stopped mid-budget without a sanctioned reason IS the 2026-06-14 incident class. A bare `--run-end` without `--reason` is `partial` (back-compat form; deprecated; not a hard fail unless budget clearly remained and no sanctioned terminal was probed). Cite the `--run-end` Bash tool_use jsonl line and the final state probe's `terminal_reason`. |
| **R-O-9** Single-cycle containment (HARD — a runaway dispatch is a FAIL + force-cap) | **(lazy-cycle-containment / SPEC §C6. Incident reference: 2026-06-16 AlgoBooth overview runaway — one dispatch ran 40 min / 14 commits across multiple features.)** The git+jsonl-keyed detection layer — **always available even when `/tmp` transcripts are reclaimed**. For EACH cycle dispatch (one `--cycle-begin`/`--cycle-end` bracket = one Agent dispatch in the parent jsonl), compute the **commits-per-dispatch** and **features-per-dispatch** metrics from two always-available sources: (1) `git log <dispatch-window>` `--name-only` ∩ `docs/{features,bugs}/<id>/` → the set of distinct feature dirs the dispatch's commits touched; (2) the parent jsonl's Bash tool_uses inside the dispatch's timestamp window → any run-lifecycle command issued (`--run-end`/`--run-start`/`--apply-pseudo`/`--enqueue-adhoc`/`dev:kill`/`dev:restart`). A dispatch is a **runaway** iff `features-per-dispatch > 1` OR it issued ANY run-lifecycle command. **Detection:** assemble the per-dispatch list `{nonce, feature_id, commit_features, lifecycle_calls}` and run the helper `python3 ~/.claude/scripts/retro_ro9.py` (or import `retro_ro9.grade_ro9`) — its verdict names the offending dispatch(es) + the metric. **Grade: `fail` (HARD) + force-cap (§5c) if any runaway is found.** **R-O-9 COMPLEMENTS, does NOT replace, R-EP-1/R-EP-2:** under the historical workstation inline-override branch (and in cloud) R-EP-1 inverts and R-EP-2 → `n/a`, so neither can see a runaway there — and even on post-lift workstation runs R-O-9 remains the backstop whose evidence (git + parent jsonl) survives subagent-transcript reclaim. Cite the offending dispatch's `--cycle-begin` jsonl line + the git commit SHAs spanning >1 feature (or the lifecycle Bash tool_use line). |
...
| Rule | What to check |
|------|---------------|
| **R-V-1** Mechanics-silent | No procedural narration in orchestrator turns. Detection heuristics (case-insensitive match over `orchestrator_text`, EXCLUDING text inside sanctioned T6 rich zones and the T7 final report): `"Per the"`, `"I must"`, `"I'll start by"`, `"Let me read"` / `"Now I'll read"`, `"Now I will"` / `"Now composing"` / `"Composing the"`, `"Entering the"`, `"no loop-guard"`, `"Preflight passed"`, `"Sync clean"`, `"Real work returned"`, tool-call announcements (a sentence whose content is what the next/previous tool call already shows, e.g. "Running the preflight check block", "Now the cycle base prompt template:"), and sentences restating probe-JSON fields in prose (a sentence outside a cycle block that repeats ≥2 of the probe's `feature_id` / `sub_skill` / `current_step` / `terminal_reason` values). The zero-text rule means ANY non-template text between tool calls outside T6/T7 counts, not just these phrases — the list is illustrative. **Fail on ≥3 distinct instances; `partial` (warn) on 1–2; `pass` on 0.** Cite each instance's jsonl line + the matched phrase. |
| **R-V-2** Cycle blocks | Every dispatched cycle — each real-skill Agent dispatch AND each inline pseudo-skill action from the Step 3 ledger — has a **T2 dispatch block** (canonical step heading `### {Step name} — {work summary} [x/y]` (forward) / `[meta x/y]` (meta) + `disp` line) and a **T3 return block** (`done` / `ledger` / `next`; `audit` where the orchestrator skill requires it — **REQUIRED on every `/spec` and `plan-feature` cycle**, carrying the Step 1d.5 input-audit's NEEDS_INPUT disposition in BOTH the surfaced case (`audit  {N} … surfaced → NEEDS_INPUT.md`) AND the skipped case (`audit  needs-input skipped — {N} reviewed, all {…}; {justification}`). A `/spec` / `plan-feature` cycle whose T3 carries NO `audit` line is the silent-skip regression this rule closes — fail that cycle, per `sentinel-frontmatter.md` Producer responsibilities #7), or a **T4 block** (`act` / `gates` / `done` / `next`) for inline pseudo-skill / gate cycles. A cycle missing its canonical heading, missing its dispatch block, or reported via a retired shape (the `### Cycle fwd N/M · meta K/L` split-counter heading — accepted as conforming ONLY for runs predating 2026-06-11 — the `**Result:**`/`**Commit:**` bullet block, a `▶ … (dispatched)` line, a multi-line cycle summary) fails for that cycle. Roll up: `pass` (all cycles conform) / `partial` (1–2 nonconforming cycles) / `fail` (≥3). Cite the jsonl line of each nonconforming cycle. |
| **R-V-3** Rich-zone containment + glyph discipline | Verbose output (multi-paragraph freeform prose) appears ONLY at sanctioned T6/T7 points: resolution briefings (Step 1g / 1h / 1i and parked-flush — including the verbatim sentinel re-prints HARD CONSTRAINT 6 mandates), research-halt announcements, errors / deviations / refusals / recoveries (`⚠` blocks), runtime-reboot / blocking-foreground-wait status zones (`⟳` blocks), standing-directive echo-back, and the T7 final report. Any multi-paragraph prose turn outside those points (narrated preflight, re-explained commit policy, restated probe output, dispatch commentary) is a violation. **Glyph discipline (F3):** `⏸` is reserved EXCLUSIVELY for park (T5). A `⟳` status zone for a runtime-reboot / blocking-foreground-wait is the SANCTIONED form — recognize it as a legitimate T6 zone, NOT a deviation. The (minor) deviation is the inverse: `⏸` used for a NON-park wait (e.g. `⏸ runtime rebooting…`) — count each such `⏸`-overload as a distinct violation. `fail` on ≥3 distinct violations; `partial` on 1–2; `pass` on 0. NOTE: long Step 1g/1h/1i briefings are REQUIRED rich output — never count a verbatim re-print briefing as a violation; likewise never count a `⟳` runtime-reboot status zone as a violation. |
...
| **R-EP-5** PHASES.md update (Step B.3) | Implementation Notes block added per phase, dated, citing actual test counts + commands. Verify in the feature artifact snapshot from 2d (check sibling `IMPLEMENTATION_NOTES.md` first; fall back to embedded notes in PHASES.md — sibling-then-embedded). |
...
2. **Workstation inline-override branch (HISTORICAL — runs predating workstation-recursive-subagent-dispatch, 2026-07-09)** — iff the dispatched cycle prompt contains the RETIRED phrase `"INLINE OVERRIDE — LOAD-BEARING"`. Detection is marker-based, never date-based: the marker was unconditional in the workstation cycle template until the lift, so any captured prompt carrying it was dispatched under the old inline contract and MUST be graded against it — inline Edit/Write is EXPECTED (not a violation of R-EP-1), and zero sub-subagents is EXPECTED (R-EP-2 → `n/a (workstation-inline-override)`). Do NOT falsely flag such historical cycles as "missing sub-subagent dispatches". Post-lift workstation prompts instead carry `"WORKSTATION DISPATCH — LOAD-BEARING"` and are graded under branch 3 (workstation standard, full R-EP contract — the skill's own sub-subagent orchestration model is authoritative again; sanctioned inline execution of a small mechanical batch is disclosed on the cycle's T3 `audit` line and is not an R-EP-2 fail when the audit line justifies it).
...
#### `/execute-plan` cycles (token-efficiency)
- **R-EP-EFF-1** Cost-aware partitioning (efficiency — Phase 9, lazy-validation-readiness). For each `/execute-plan` cycle, read the executed plan part's frontmatter (`complexity:`) and its WU count. Two non-force-cap efficiency notes (grade `partial`, NEVER a hard `fail`, NO new force-cap):
  - **Soft-target overshoot:** a part that materially exceeded the ~3–4 WU soft target (e.g. a single part packed near the 8-WU hard cap) **without** a complexity split, when `/write-plan` could have partitioned it into smaller fresh-context parts. Cite the part path + WU count + the measured cycle token/time cost (the d7-multi-timbral baseline: one part hit 399k tok / 106 min in a single inline context — that is the shape this rule flags).
  - **Mis-tiered part:** a part tagged `mechanical` whose WUs include a novel design decision / algorithm-DSP / cross-boundary wiring (the tag should have been `complex` — it dispatched on Sonnet when it warranted Opus), OR a part tagged `complex` that is provably all-mechanical (a missed Sonnet-downgrade opportunity). The `mechanical` mis-tag is the higher-severity of the two (it risks quality); cite the offending WU + the tag. A correctly-tagged `mechanical` part that ran on Sonnet is the WIN this phase intended — confirm the savings, do not flag it.
...
- **R-MT-EFF-1** Metric-not-buffer assertions (efficiency — Phase 9, lazy-validation-readiness). An `/mcp-test` cycle that pulled raw interleaved `get_audio_buffer` `samples` arrays, or full unfiltered `get_console_logs` / `get_session_events` dumps, into context **when the assertions only needed the scalar metrics** (`rms` / `max_discontinuity` / `dc_offset` / `scheduler_playing`, the harmonic/spectral scalar fields, pass/fail counts) or filtered logs (`evt_filter` / `since_line` / head-tail-grep) is an efficiency note. Grade `partial` (NOT a hard `fail`, NO new force-cap): a raw buffer/full-dump pull is justified ONLY after a scalar metric already FAILED and a deeper look was needed — flag only the unjustified pulls. Confirm the savings when a cycle certified from scalars + filtered logs alone (the d7-multi-timbral baseline: ~1.34M tok / 9 cycles, bloated by exactly these raw pulls). Cite the tool call + whether a scalar metric had already failed.
...
**Force-cap rule (R-O-9 single-cycle containment — HARD, lazy-cycle-containment / SPEC §C6):** if `R-O-9` (§4a) graded a **`fail`** — i.e. the git+jsonl evidence shows ANY single dispatch touching `>1 feature` OR calling a run-lifecycle command (`--run-end`/`--run-start`/`--apply-pseudo`/`--enqueue-adhoc`/`dev:kill`/`dev:restart`) — **force-cap the run's headline grade at `fail`** regardless of arithmetic, and cite the offending dispatch (nonce + feature dirs / lifecycle command + jsonl line). This is the runaway-containment cap: a single dispatch that escapes its one-cycle scope is the 2026-06-16 incident class (40 min / 14 commits / multiple features in one dispatch). It fires **independently of R-EP-1/R-EP-2** — and is the cap that survives transcript reclamation, because R-O-9's evidence (git log window + parent jsonl) is always available even when every R-EP grade is `unverifiable`. R-O-9 **complements, does not replace** the R-EP force-cap above; both can fire on the same run.
