...
5. **Tick the plan-part WU checkbox + update PHASES.md (BLOCKING GATE).**
   - **First, tick this batch's `- [ ] WU-N` rows → `- [x]` in the PLAN PART's `## Work Units` checklist (machine source of truth — HARD).** As of the 2026-06-15 d8-effect-chains review, `--verify-ledger --plan <plan_part>` reads `deliverables_done` from **these** rows, not from PHASES.md. Leaving a landed WU's plan-body checkbox unticked false-fails the completion gate and triggers a recovery cycle. (A `- [ ] WU-N` under a `**Runtime Verification**` / `## MCP Integration Test` subsection stays unticked — the Step-9 `/mcp-test` gate owns it.)
   - **Then update PHASES.md** — Read `~/.claude/skills/_components/phases-update.md` and follow it: write the phase **Status** + **Implementation Notes** (the durable human record) and best-effort tick the PHASES.md per-deliverable checkboxes. PHASES.md deliverable ticks are now **human-readable documentation, not the gate** — keep them as accurate as you reasonably can, but the machine completion verdict comes from the plan-WU checkboxes above. Re-read PHASES.md to verify the Status + Notes write landed. Do NOT proceed until both the plan-WU ticks and the PHASES.md Status/Notes are verified on disk.
...
> **Bug-fix plans (SEAM B) — the completion bar is symptom reproduction, not unit-green.** If the plan is a bug fix (`docs/bugs/**`, `kind: fix-plan`/`implementation-plan` on a bug), the "work units complete" judgment must apply the sharpened "Bug fixed" rule in `~/.claude/skills/verification-before-completion/SKILL.md` and the evidence ladder in `~/.claude/skills/_components/symptom-reproduction-gate.md`: a serving-path regression test (red→green on the symptom's *actual serving path*, not the fix's internal target) or a runtime/manual repro is the REQUIRED evidence. A batch that closed on a unit test asserting the fix's internal target has NOT reproduced the symptom — treat the serving-path regression test as an unmet deliverable (route rework), not a completable WU. This is distinct from the runtime/MCP gate carve-out below: a serving-path *unit* test needs no live runtime and IS `/execute-plan`'s to close.
...
> **State-machine backstop (Phase 8 — lazy-validation-readiness).** If this flip is nonetheless missed (an older `/execute-plan` revision ran, or the agent ticked every implementation row but left the plan `Ready`/`In-progress` with only verification rows open), `lazy-state.py` now self-corrects: a plan whose only in-scope unchecked rows are verification-only is recognized as *finalize-stale* and routed to the inline `__flip_plan_complete_stale__` pseudo-action — NOT a redundant full `/execute-plan` re-dispatch (which previously burned a whole cycle just to flip the status — d7-multi-timbral Phase 11, cycle 34). This is the robust backstop; still do the flip here so the backstop never has to fire.
