...
| `host-capability-saturated` | The host-axis generalization of `device-queue-exhausted` (`host-capability-declaration-for-gated-features`): the remaining feature(s) declare a `requires_host:` capability (binary toolchain, GPU, etc.) ABSENT on this host, deferred via `DEFERRED_REQUIRES_HOST.md` (carrying the missing capability ids). The `notify_message` names each feature + its missing id(s). Tell the user to re-run `/lazy` on a **capability-bearing host** to re-open and certify them — the features are deferred, not dropped. |
| `queue-exhausted-budget-deferred` | Budget guard: all remaining queue items are budget-deferred/evicted to the queue tail (no independent successor to skip-ahead to). Not `all-features-complete` — the roadmap is not finished; features were over-budget. Re-run `/lazy` to continue; deferred features reappear at the queue tail with fresh cycle counts. |
...
| `RETRO_DONE.md` | DORMANT — /retro is unwired (2026-06); never written for new features | (formerly the retro completion gate) | Still deleted on feature completion if a stale one exists |
...
The script mirrors the state machine documented in earlier revisions of this file (Step 3 blocker, Step 4 SPEC, Step 4.5 stub, Step 4.6 realign, Step 5 research gate, Step 6 phases, Step 7 plan, **Step 9 MCP** (Step 8 retro UNWIRED — see below), Step 10 mark-complete). When the state machine needs to change, update `lazy-state.py` — and keep this skill's wrapper logic + the paired `/lazy-cloud` skill (per CLAUDE.md coupling rule) in sync.
...
**Research is a PRE-planning gate — skipped when `PHASES.md` already shows implementation.** Step 5 only routes a feature to research when it has NOT yet been planned. A pre-Step-5 guard in `lazy-state.py` (`compute_state`) checks, when no `RESEARCH*.md` is present, whether `PHASES.md` already exists with implementation evidence — any phase `Complete`/`In-progress`, ≥1 checked deliverable, or Implementation Notes present (the `lazy_core.phases_show_implementation` predicate). The predicate uses the sibling-then-embedded read order: it checks a sibling `IMPLEMENTATION_NOTES.md` first (via `_sibling_impl_notes_present()` — looking for `#{2,4} Implementation Notes` heading content), then falls back to an `## Implementation Notes` block embedded in PHASES.md itself. If evidence is found it emits a `_diag("Step 5 research gate skipped: …")` and falls through to Step 6, because a feature with implemented phases is past the pre-planning research stage — re-running Gemini there is wasted work. An empty-stub `PHASES.md` (zero parsed phases) does NOT count as evidence, so a stub never suppresses legitimate research. A bare-scaffold sibling (no content headings) likewise does NOT count. The predicate + diagnostic live in `lazy-state.py`/`lazy_core.py`; this wrapper carries no logic.
...
**Step 8 retro — UNWIRED (operator decision, 2026-06).** The `/retro` step has been removed from the pipeline. Once `PHASES.md` is all-phases-Complete (and no open BLOCKED/NEEDS_INPUT), `lazy-state.py` routes DIRECTLY to Step 9 (the MCP gate) — it never emits `retro-feature` and never waits on `RETRO_DONE.md`. Rationale: the retro gate proved low-yield (a docs/design-alignment audit that structurally can't see the runtime defects `/mcp-test` catches) while costing a full Opus dispatch per feature. The `/retro-feature` skill remains in the catalog and the now-inert `retro_staleness()`/`phase_kind` parsing is left dormant in `lazy_core.py`/`lazy-state.py`/`bug-state.py` — git history is the restore path. Cloud Step 9 writes `DEFERRED_NON_CLOUD.md`; workstation Step 9 runs `/mcp-test`.

**Device axis (orthogonal to cloud).** A third environment dimension governs audio MCP assertions that can only be CERTIFIED with a real output device. On a no-real-device host, `/mcp-test` writes `DEFERRED_REQUIRES_DEVICE.md` (scoped to specific scenario IDs) instead of a permanent `SKIP_MCP_TEST.md`; `lazy-state.py` then treats that sentinel + no `VALIDATED.md` (on a feature past implementation) as **device-saturated** (Step 2 skips it, terminal `device-queue-exhausted`) — the feature does NOT complete here (no receipt is written). On a real-device host, Step 9 RE-OPENS the feature: it dispatches `/mcp-test` scoped to the deferred scenario IDs, which certifies them, deletes the sentinel, and writes `VALIDATED.md` — only then does the feature proceed to `__mark_complete__`. This is the inverse the framework previously lacked, and it mirrors the cloud→workstation deferral on the device axis. The decision was: device-deferral BLOCKS completion (stays In-progress) rather than completing-with-a-distinct-receipt, so `Complete` always means fully validated.

**Host-capability axis (generalizes the device axis to N named capabilities — `host-capability-declaration-for-gated-features`).** A feature whose runtime validation needs an arbitrary named host capability (a binary toolchain, GPU, a specific CLI) declares it on-disk in a `requires_host:` set (SPEC frontmatter or its `queue.json` entry; ids drawn from a **closed registry** in `lazy_core`). Before dispatching the feature's runtime-verification step, `lazy-state.py` probes the host (active invocation — runs the tool + checks the exit code, NEVER `which()`/`exists()`, guarding the Windows `\WindowsApps` zero-byte alias false-positive) and computes `missing = requires_host − host.present` (flat AND-set). On a non-empty `missing` it writes a re-openable `DEFERRED_REQUIRES_HOST.md` (carrying the missing ids) and skips — terminal `host-capability-saturated` when only capability-gated features remain (see Step 2b). On a **capability-bearing host** the feature re-opens (an empty `missing` is the no-special-case re-open) and proceeds to `__mark_complete__`. An **unregistered** `requires_host:` id (a typo with no probe) is a loud, immediate `BLOCKED.md` (`blocker_kind: unknown-host-capability`) naming the offending id + the registry — never a silent defer-forever (which would starve the queue). The `requires_host:` parse + this fail-fast mirror into `bug-state.py` for parity; the capability-miss defer is feature-pipeline-shaped. An ungated feature (no `requires_host:`) is byte-identical to today.

**Budget guard + skip-ahead (feature-budget-guard-and-skip-ahead).** When `lazy-state.py` is passed `--per-feature-cycle-cap <N>`, the budget guard caps each feature at N cycles; an over-budget feature is deferred to the queue tail (`action: defer|evict`, surfaced in the `budget_guard` probe field). When all remaining items are budget-deferred and no independent successor exists, the terminal is `queue-exhausted-budget-deferred` (see Step 2b above). The default-on dependency-aware skip-ahead: when the queue head is research-gated or BLOCKED, `lazy-state.py` automatically advances to the next `independent: true`-marked queue item (if one exists) — the gated head appears in the `gated_heads` probe key. Pass `--strict-research-halt` to restore the legacy halt-on-first-gated-head behavior. `/lazy` (the single-step wrapper) does NOT pass these flags by default — they are batch-runner flags threaded from `/lazy-batch` / `/lazy-batch-cloud` through every state probe.
