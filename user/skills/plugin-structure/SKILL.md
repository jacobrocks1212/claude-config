...
description: Use when creating or scaffolding a Claude Code plugin — organizing commands/agents/skills/hooks, plugin.json, CLAUDE_PLUGIN_ROOT, auto-discovery, and layout best practices.
version: 0.1.0
---
...
## Auto-Invocation and Reactive Behavior

When a plugin needs auto-invocation or reactive behavior (e.g., running a check after edits, enforcing a workflow at session end), include a `hooks/hooks.json` declaration from the start. Behavioral guidelines in CLAUDE.md alone are insufficient — Claude forgets passive instructions. Use Stop hooks with block decisions for end-of-session enforcement and PostToolUse hooks for tracking work events.
