...
1. **Ensure the runtime is up AND current — ONE `--ensure-runtime` subcommand call (unified-pipeline-orchestrator Phase 5).** The probe → stale-binary check → `dev:restart` (bg) → curl-until-200 → MCP-tool assertion dance is now the `lazy-state.py --ensure-runtime` subcommand (`lazy_core.ensure_runtime`). Run it ONCE instead of hand-running steps 1–3:

   ```bash
   python3 ~/.claude/scripts/lazy-state.py --ensure-runtime --repo-root "{cwd}"
   # → JSON {status: ready|booted|stale-rebuilt, mcp_tools_present, health_code}
   ```

   - `status: ready` — runtime was up and current; nothing rebooted.
   - `status: booted` — runtime was down; the subcommand started it (background `dev:restart`, orchestrator-owned) and waited for health=200.
   - `status: stale-rebuilt` — runtime was up but the native binary was stale (a newer `src-tauri`/`crates` commit exists since boot, via the same `stale_binary` predicate); the subcommand forced a `dev:restart` so the mcp-test cycle sees the current tool registry. (Spec reference: F7 in `docs/specs/lazy-validation-readiness/SPEC.md`.)
   - `mcp_tools_present: false` (or health never reached 200, `status: booted` with `health_code != 200`) → the runtime could not be brought up MCP-ready → surface a `BLOCKED.md` (`blocker_kind: mcp-runtime-unready`) rather than dispatching a subagent against a dead runtime — a subagent cannot recover a runtime the orchestrator failed to boot.

   The AlgoBooth specifics (TCP 3333 health endpoint, `npm run dev:restart`, the `src-tauri`/`crates` native globs, the asserted MCP tool) are **parameterized in `lazy_core`'s default config dict** — not hand-typed here. The subcommand owns the orchestrator-owned background process exactly as the old hand-run steps did (the runtime survives the upcoming subagent's turn boundary).
