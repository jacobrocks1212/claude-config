...
8. **TWO session-global monotonic counters replace the single `cycle` counter.** Both are initialized once in Step 0 and NEITHER is ever reset on feature transitions.
   - **THE SCRIPT is the sole budget-counting authority** (cycle-budget-counters-double-count-on-probes-and-inject-hook). The orchestrator NO LONGER hand-counts `forward_cycles` / `meta_cycles`: `lazy-state.py` advances them at the `--cycle-end` bracket (keyed on the closed cycle marker's `--kind`) and at `--apply-pseudo`. The `--repeat-count` probe and the per-turn inject hook are BUDGET-NEUTRAL (they advance only the loop-detection streaks). The instructions below that say "increment `forward_cycles` / `meta_cycles`" are now DESCRIPTIVE of what the script does at that step, not an orchestrator action.
   - **`forward_cycles`** — counts pipeline-advancing work. Ceiling: `max_cycles`. Advanced by the SCRIPT at: (a) the `--cycle-end` bracket when the closed cycle marker's `--kind` is `real` (every real-skill Agent dispatch is wrapped `--cycle-begin --kind real … / --cycle-end`), and (b) `--apply-pseudo` for a pipeline-advancing pseudo-skill (`__mark_complete__`, `__mark_fixed__`, `__write_deferred_non_cloud__` (cloud variant only — workstation `lazy-state.py` never emits this), `__write_validated_from_results__`, `__write_validated_from_skip__`, `__grant_skip_no_mcp_surface__`, `__flip_plan_complete_cloud_saturated__`). **Capped at Step 1c** (`if forward_cycles >= max_cycles` → the existing max-cycles halt).
   - **`meta_cycles`** — counts resolution / recovery / audit / cleanup work. **NO ceiling — uncapped by design (operator decision 2026-06-14).** Advanced by the SCRIPT at the `--cycle-end` bracket when the closed cycle marker's `--kind` is `meta` — every resolution / recovery / audit dispatch is bracketed `--kind meta` (Step 1g decision-resume, Step 1h blocked-resolution, Step 1i operator-directed halt-resolution, LOOP-DETECTED / Step 1e.4a recovery dispatches, the input-audit cycle at Step 1d.5). The stale-plan flip pseudo-skill `__flip_plan_complete_stale__` classifies to meta whenever it advances the budget. The meta loop is NOT bounded by a meta cap; the run's only hard stop is the `forward_cycles >= max_cycles` cap at Step 1c. `meta_cycles` is still tracked and displayed (as a bare count), but there is NO `if meta_cycles >= …` halt anywhere — Step 1g/1h/1i have no meta-cap check.
...
11. **Concurrent-writer awareness — no monsters-in-the-closet serialization.** Other agents may be committing to this same worktree/branch concurrently — a parallel `/lazy-batch-parallel` lane, a background harden dispatch, or a second interactive/scheduled session. An unexpected incoming commit / moved HEAD is EXPECTED, not a defect to panic on or halt for. Genuine write contention is resolved by the coordination layer (git safety + the FIFO file-lock + conflict-routing) — not by the orchestrator pre-serializing dispatches on the mere possibility of a collision.

**Cycle-subagent execution model (workstation: sub-subagent dispatch PERMITTED with guardrails — `workstation-recursive-subagent-dispatch`, 2026-07-09).** **ORCHESTRATOR SCOPE FIRST — read this before the capability below, because it is the CYCLE SUBAGENT's, not yours.** The orchestrator dispatches **EXACTLY ONE `Agent` per cycle: the SINGLE emitted `cycle_prompt`, verbatim, to ONE subagent.** The sub-subagent split described below (test-agent, impl-agent, per-work-unit worker, research subagents A–G, Explore fan-out) belongs to the SUBAGENT and happens INSIDE it once it is running — **the orchestrator NEVER composes or dispatches the cycle skill's internal worker prompts itself.** The enticing examples in the next sentence (`/execute-plan → test-agent + impl-agent`, `/retro → subagents A–G`) describe what the dispatched SUBAGENT does; they are NOT a cue for you to set that split up. Improvising a worker prompt at the orchestrator level under a freshly-armed `subagent_model` cycle whose own emission is not yet consumed (no worker in flight) is exactly what the dispatch guard's self-announcing deny catches and routes to hardening (`lazy_guard._subagent_model_improvisation_deny_reason`; hardening Rounds 112/114 — the WU-1 impl split and the test-agent split were both denied this way). Dispatch the one `cycle_prompt` and let the subagent split internally; do not make the guard say no. The cycle subagent dispatched at Step 1d MAY use the `Agent` tool: when the dispatched skill's SKILL.md defines a sub-subagent orchestration model (e.g. `/execute-plan` → Sonnet test-agent + impl-agent, `/retro` → research subagents A–G, read-only Explore fan-outs), the skill's own contract is authoritative and SHOULD be followed — the emitted cycle prompt's "Sub-subagent dispatch policy (WORKSTATION DISPATCH — LOAD-BEARING)" section carries the binding guardrails (terminal-stop ban restated in every sub-subagent prompt; single-writer discipline with the cycle subagent as the sole integrator owning the turn-end verify/commit gates; scope containment — no wholesale re-dispatch of the cycle). *History:* the former "INLINE OVERRIDE" ban dated from a 2026-06-14 harness limitation (the `Agent` tool was genuinely unavailable inside a dispatched subagent); the harness has exposed it since (verified live 2026-07-09), the containment hook's recursive-dispatch deny was removed the same day (`docs/bugs/adhoc-containment-denies-mandated-explore-fanout`), and this feature retired the policy remnant on workstation. **The orchestrator's own constraints are unchanged** — it still dispatches exactly one `Agent` per cycle, and HARD CONSTRAINT 1 still scopes its `Write`/`Edit` to sentinels. **`/lazy-batch-cloud` KEEPS the inline override** (cloud cycles have no persistent state and are reclaim-prone; zero sub-subagents remains the cloud steady state) — the execution models now deliberately DIVERGE between the coupled pair; the divergence is tabulated in `/lazy-batch-cloud`'s "Differences from /lazy-batch" block. **Concurrent-writer awareness:** other agents may be working this same worktree/branch concurrently — an unexpected commit / moved HEAD is expected, not a defect. Genuine write contention is resolved by the coordination layer (git safety + the FIFO file-lock + conflict-routing) — not by halting.

> **Restored guarantee — structural TDD agent-separation (workstation).** With dispatch lifted, `/execute-plan`'s test-agent→impl-agent split is structurally enforceable again on workstation: a separate agent writes the failing tests before a separate agent implements, and `/lazy-batch-retro` grades `R-EP-2`/`R-EP-3` from dispatch evidence under the full workstation-standard branch (historical runs whose captured prompts carry the retired `INLINE OVERRIDE — LOAD-BEARING` marker keep grading under the old inline branch — the retro's detection is marker-based, never date-based). Inline execution of a small mechanical batch remains sanctioned; when executing inline, keep tests-before-impl manually. Cloud cycles keep the documented inline tradeoff (`n/a (cloud-override)` grading) unchanged.
...
**Operator-directed queue mutations (in-place priority / dependency changes).** When the operator directs a queue-item priority or dependency change ("bump this bug to P1", "make feature X tier 0", "gate this on Y"), NEVER hand-edit `queue.json` (HARD CONSTRAINT) and do NOT try to fake it with `--reorder-queue` (that moves position but leaves `merged_priority` — the effective/merged priority — unchanged). Use the deterministic mutators; each is `refuse_if_cycle_active` (exit 3 for a cycle subagent) and requires `--operator-authorized`. A priority change ATOMICALLY re-sorts the queue's listed order to match the new merged priority in the same write (so effective priority and work order stay coherent — no stale reorder):

- **Change a bug's severity:** `python3 ~/.claude/scripts/bug-state.py --set-severity <id> <P0|P1|P2|Low> --operator-authorized` (the promote/demote sibling of `--pin`; clears any active pin). Feature analog: `lazy-state.py --set-tier <id> <int>`.
- **Un-pin a bug:** `python3 ~/.claude/scripts/bug-state.py --unpin <id> --operator-authorized` (inverse of `--pin` — restores the SPEC severity + re-positions).
- **Add / remove queue dependencies post-hoc:** `--add-deps <id> --deps a,b --operator-authorized` / `--remove-deps <id> --deps a,b --operator-authorized` (on either state script; the arbitrary-edit sibling of `--sync-deps`, cycle-guarded).
- **Unsure which command performs an op?** `python3 ~/.claude/scripts/bug-state.py --search-ops "<natural-language op>"` (e.g. `"set bug severity"`, `"add dependency"`) returns the ranked matching commands + usage; `--list-ops` dumps the full catalog. Use this instead of guessing or hand-editing.
...
2. **halt** (both modes) — fired on every terminal/halt: `NEEDS_INPUT` halt, `BLOCKED` halt-for-manual, `needs-research` strict halt, `queue-blocked-on-research`, `queue-missing`, `all-features-complete`, `queue-exhausted-all-parked` (`--park` mode — after the flush), `queue-exhausted-budget-deferred` (budget-guard — all items deferred to queue tail), `max-cycles`, `device-queue-exhausted`, `host-capability-saturated`, script-error, and any future obstacle terminal. Most of these already carry per-terminal `PushNotification` calls above — this point names the policy explicitly so no terminal can be added without a notification.
   ...
   **Intervention-efficacy evaluation runs ONCE per run at the same end-of-run flush, BEFORE `--run-end` (intervention-efficacy-tracking D10-A).** Immediately alongside the incident scan above (same first-terminal trigger, same once-per-run rule), run `python3 ~/.claude/scripts/efficacy-eval.py --repo-root . --json` in the orchestrator session — the standalone evaluator (read-only over the telemetry ledger; sole post-capture writer of `docs/interventions/<id>.md` records) accrues each open hypothesis's post-ship run window and writes any DUE verdicts (CONFIRMED / REFUTED / INCONCLUSIVE per the D5 bands) into the records. A REFUTED verdict auto-enqueues `reconsider-<id>` via the sanctioned `--enqueue-adhoc --type bug` path (two-layer recurrence-guarded — at most once EVER per intervention); announce it exactly like any ad-hoc enqueue. Relay the JSON summary's verdict lines + any `needs_triage` escalations verbatim (an escalated INCONCLUSIVE hypothesis is information for the operator, never a halt). Record updates are committed docs — stage + commit them in the run-end sequence per the **§1c.6 flush-commit staging contract** below (`docs(interventions): efficacy verdicts — <run terminal>`; push per project policy) so verdicts ride `main`. NON-BLOCKING: a non-zero exit prints one `⚠ efficacy-eval failed` warning and the run-end sequence continues unchanged — the evaluator must never wedge a terminal path. Zero records / nothing due → clean no-op output, nothing to commit. On-demand reruns any time: `python3 ~/.claude/scripts/efficacy-eval.py --repo-root . [--dry-run]`.

   **Harness-change canary watch runs ONCE per run at the same end-of-run flush, BEFORE `--run-end` (harness-change-canary-rollback D6-A).** Immediately alongside the efficacy evaluation above (same first-terminal trigger, same once-per-run rule), run `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root . --json` in the orchestrator session — the same evaluator in canary mode (read-only over the telemetry ledger + fresh incident sources; the SOLE writer of `canary.*` record fields + the trip-time `EVIDENCE.md`) accrues each OPEN canary window and, on a trip (D2 band regression or ≥2 attributable fresh incidents on the change's surface), flags-and-enqueues an evidence-bearing `canary-revert-<id>` bug stub via the sanctioned `--enqueue-adhoc --type bug` path (once EVER per canary — never a silent revert). Surface the JSON's `notify` line (`⚠ canary tripped: <id>`) exactly like any ad-hoc enqueue. Record updates + the seeded evidence are committed docs — stage + commit them per the **§1c.6 flush-commit staging contract** below (rides the same commit as the efficacy verdicts) so trips ride `main`. NON-BLOCKING: a non-zero exit prints one `⚠ efficacy-eval --canary failed` warning and the run-end sequence continues unchanged — the watcher must never wedge a terminal path (it is fail-open by construction). Zero open canaries → clean no-op. On-demand reruns any time: `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root . [--dry-run]`.

   **TWO-SCOPE flush — the interventions-bearing scope MUST be covered (interventions-telemetry-repo-scope-split-brain, D2).** The efficacy review + canary invocations above run `--repo-root .` (this run's TARGET repo). Intervention records live ONLY in the **claude-config** checkout (they follow the FIX), while the telemetry that grades them accrues in the TARGET repo's `repo_key`-keyed state dir (it follows the RUN) — so a target-only flush sees the telemetry but NO records and can never produce a verdict (the zero-verdicts-ever symptom this fix closes). Therefore, immediately after the `--repo-root .` pair, run the SAME two evaluators a SECOND time rooted at the claude-config checkout (`~/source/repos/claude-config` on the workstation): `python3 ~/.claude/scripts/efficacy-eval.py --repo-root ~/source/repos/claude-config --json` then `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root ~/source/repos/claude-config --json`. This claude-config-rooted evaluation sees the originating target repo's telemetry via the **Phase-1 merged read** (`read_intervention_telemetry` additionally merges the live run marker's target-repo keyed ledger), so records and the telemetry that grades them finally meet at ONE `repo_root`. **Both scopes are attested in the Phase-2 coverage breadcrumb** (`interventions_covered`) — which is exactly what discharges the tightened `--run-end` gate below; a target-only flush no longer satisfies it. **When this run's OWN repo IS claude-config**, the `--repo-root .` pair already covered the interventions scope, so the second invocation resolves to the same path and is an idempotent no-op (safe to run; nothing new is written). Doc-write/commit ownership is UNCHANGED: the second scope's `docs/interventions/` (and any `docs/bugs/reconsider-*` / `canary-revert-*` seed) land in the claude-config checkout tree and are staged + committed there exactly as the `--repo-root .` pair's records are — on the workstation this is the same tree; for a non-claude-config target repo the record-updates + commit happen in the claude-config checkout, not the target repo. The NON-BLOCKING `⚠ efficacy-eval failed` / `⚠ efficacy-eval --canary failed` posture is preserved for the second scope too (a non-zero exit warns and the run-end sequence continues unchanged).

   **KPI scorecard regen rides the SAME claude-config-rooted scope (efficacy-signal-integrity D4 — chosen option A).** The per-cycle `kpi-scorecard.py` regen documented below is gated on `<run's repo_root>/docs/kpi/registry.json` existing, which is TRUE only when the run happens IN claude-config — so for every other repo (AlgoBooth today) it never fires, and `docs/kpi/SCORECARD.md` goes stale between operator-triggered runs. Immediately after the claude-config-rooted efficacy/canary pair above, in the SAME claude-config-rooted scope, run `python user/scripts/kpi-scorecard.py --repo-root ~/source/repos/claude-config` — this is registry-gated correctly BY CONSTRUCTION (the registry lives in claude-config, so this invocation is never a no-op there) regardless of which repo the run actually happened in. Stage + commit the regenerated `docs/kpi/SCORECARD.md` alongside the intervention records per the **§1c.6 flush-commit staging contract** below (same commit, same NON-BLOCKING fail-open posture — a regen failure prints `⚠ kpi-scorecard (claude-config-scope) failed` and never blocks `--run-end`). The per-cycle per-repo-registry-gated regen below is UNCHANGED (correctly a no-op in a repo with no registry) — this is an ADDITIONAL invocation, not a replacement.

   **§1c.6 flush-commit staging contract (one-writer coordination — `end-of-run-flush-commit-absorbs-concurrent-writer-staged-files`).** The efficacy/canary/KPI commit above runs in the **claude-config** checkout even when this run TARGETS a different repo (AlgoBooth), where a `/harden-harness` agent may be concurrently writing claude-config. Stage + commit ONLY the specific artifacts THIS flush generated, **by explicit path**, via a **pathspec-scoped commit** — NEVER a broad `git add -A docs/*` / directory add (a blanket add absorbs a concurrent writer's staged, unrelated files into the flush commit — observed live: `115a991a "docs(interventions): efficacy verdicts"` swallowed a concurrent harden agent's `docs/bugs/*/SPEC.md`). The exact paths are: the `docs/interventions/<id>.md` records the evaluators wrote this run (the `verdicts[].id` from the efficacy/canary JSON), the regenerated `docs/kpi/SCORECARD.md`, and any `docs/bugs/reconsider-<id>/` / `docs/bugs/canary-revert-<id>/` seed enqueued this run. Commit them with an explicit pathspec so only those paths are captured regardless of what else is staged:

   ```bash
   git -C ~/source/repos/claude-config commit -m "docs(interventions): efficacy verdicts — <run terminal>" -- \
     docs/interventions/<id-1>.md docs/interventions/<id-2>.md docs/kpi/SCORECARD.md docs/bugs/reconsider-<id>/
   ```

   The trailing `-- <paths>` pathspec commits ONLY the named paths from the working tree; a concurrently-staged foreign file (a harden agent's spec) is left untouched, never absorbed. The sanctioned mechanism is `lazy_core.flush_commit_artifacts(repo_root, artifact_paths, message)` (the tested embodiment of this `git commit -- <pathspec>` discipline). If a foreign staged/dirty claude-config state is present, do NOT blanket-add to "clean it up" — commit only this flush's explicit paths and leave the foreign state for its owner.

   **Intervention-coverage lint runs ONCE per run at the same end-of-run flush, BEFORE the orchestrator's run-end step (hardening-intervention-records-unmeasurable-or-missing, D2 / Fix-Scope #3).** Alongside the trio above (same first-terminal trigger, same once-per-run rule), run `python3 ~/.claude/scripts/doc-drift-lint.py --repo-root ~/source/repos/claude-config` in the orchestrator session — its `check_intervention_coverage` asserts every post-contract `Mechanical fix applied:` round in the current month's hardening-log has a matching `docs/interventions/harden-<YYYY-MM>-rN.md` (or the round's explicit `**Intervention record:** none` exemption marker), so an undisciplined round that ships a fix with NO record is caught mechanically instead of via prose self-attestation. Rooted at the **claude-config checkout** because the hardening-log + intervention records live ONLY there (they follow the FIX — exactly like the two-scope efficacy flush above); when this run's OWN repo IS claude-config, `--repo-root .` is equivalent (idempotent). FAIL-OPEN and NON-BLOCKING: a non-zero exit (a coverage hole, or any other doc-drift finding) prints one `⚠ intervention-coverage lint failed` warning and the run-end sequence continues unchanged — the lint must NEVER wedge a terminal path or block `--run-end` (house posture; the lint is OFF the state-script compute path). Surface the coverage findings verbatim on the T7 report path so a missing-record round is visible for the NEXT run. On-demand reruns any time: `python3 ~/.claude/scripts/doc-drift-lint.py --repo-root .`.

   **`--run-end` MECHANICALLY ENFORCES the flush (efficacy-future-check-unenforced-orchestrator-prose D1).** The three scripts above each drop a run-scoped "efficacy flush ran this run" breadcrumb when they run (even on a clean no-op). `--run-end` REFUSES (exit 1, marker + registry LEFT IN PLACE) unless that breadcrumb is present — MIRRORING the unacked-hardening gate — so skipping the flush is impossible-by-accident (it was skipped at a real checkpoint `--run-end` before this gate existed, starving the loop). The gate applies to `--reason checkpoint` run-ends too (a checkpoint is a run boundary; the loop must flush there as well). For the genuine "no interventions/incidents exist, deliberately skip" case, pass `--efficacy-skip-authorized` (operator-authorization ONLY, parallel to `--ack-unhardened`; it prints into the run-end message under `efficacy_skip` for retro grading) — never passed autonomously. The trio + the commit stay orchestrator-owned (the gate does not move the doc-writes/enqueues/commit into `--run-end`, preserving terminal-commit ordering).
...
Dispatch `dispatch_prompt` VERBATIM using `dispatch_model`. The `@requires` keys for `--emit-dispatch coherence-recovery` are: `item_name`, `spec_path`, `gate_output`, `item_id`, `cwd`. The subagent reconciles PHASES.md honestly (tick-with-evidence or re-scope, never blind-tick) then returns to Step 1a. Exactly as a Gate-1 halt routes. **Honest-stuck terminal (no oscillation):** if the coherence-recovery subagent reports `ESCALATED` — it could reconcile nothing because the only remaining blockers are verification rows that genuinely never ran on this host (0 genuine implementation deliverables) — it will have written `NEEDS_INPUT.md` (`written_by: completion-integrity-gate`, surfacing `turn-routing-enforcement` decisions #2/#5/#6). Do NOT re-dispatch coherence-recovery; return to Step 1a and the next probe returns `terminal_reason: needs-input` (Step 1g handles it). This is the deterministic replacement for the prior manual-NEEDS_INPUT improvisation the oscillation tripwire used to force (see `dispatch-coherence-recovery.md` step 3a). The ROADMAP strikethrough is NO LONGER an orchestrator step — `--apply-pseudo __mark_complete__` strikes the `docs/features/ROADMAP.md` row itself (unified-pipeline-orchestrator Phase 5; returns `roadmap_struck`). Then commit + push per project policy. See the component files for the full gate algorithms. **Both gates are docs-only** (read SPEC.md / PHASES.md / `mcp-tests/*.md` / sentinels, no Tauri / no MCP server) — they run identically in workstation and cloud. **Harness-change design-gate refusal (`gate-verdict`, distinct from coherence — GAP 1):** when the `--apply-pseudo __mark_complete__` refusal reason NAMES the **harness-change design gate** (`lazy_core.gate_verdict_ok`: `harness-change design gate: … GATE_VERDICT.md`) — an in-scope change whose shipped commits touch a `docs/gate/control-surfaces.json` control surface with a MISSING / failing / unsigned-gate-weakening `GATE_VERDICT.md` — do NOT route coherence-recovery (PHASES.md is coherent; the block is a missing VERDICT). Authoring the verdict is JUDGMENT work (`_components/harness-change-gate.md` adversarial questions) the orchestrator must NOT improvise (HARD CONSTRAINT 1) — dispatch the completion-time authoring seam: `python3 ~/.claude/scripts/lazy-state.py --emit-dispatch gate-verdict --context item_name="{feature_name}" --context spec_path="{spec_path}" --context gate_output="<the refusal reason string>" --context item_id="{feature_id}" --context cwd="{cwd}"` and dispatch `dispatch_prompt` VERBATIM (opus; `@requires`: `item_name`, `spec_path`, `gate_output`, `item_id`, `cwd`). The subagent runs `harness-gate.py` over the shipped diff, works the adversarial questions, writes `GATE_VERDICT.md`, and returns to Step 1a to re-attempt `__mark_complete__`. **A gate-weakening `hit` is NEVER self-approved** — the subagent reports `ESCALATED` + writes `NEEDS_INPUT.md` (`written_by: harness-change-gate`) for operator sign-off; do NOT re-dispatch (next probe returns `needs-input`, Step 1g handles it).
...
`compute_state` routes `sub_skill == "spike"` from two on-disk signals (Phase 2): **(a)** a `**Spike:** required` PHASES.md phase-declaration line with no PASS `SPIKE_VERDICT.md` on disk (Step 9.5, BEFORE the mark-complete gate), and **(b)** a `BLOCKED.md` carrying `blocker_kind: runtime-spike-verdict-pending` (Step 3, the ad-hoc/blocked resolver). Both hand you a `spike` cycle; the handling below is identical for either.

**Step A — Runtime pre-boot (WORKSTATION ONLY — reuse Step 1d.0's `--ensure-runtime`).** Runtime is **ORCHESTRATOR-OWNED**, exactly like `/mcp-test`: the spike subagent DRIVES the live runtime but never boots a background runtime of its own (it would not survive the subagent's turn boundary — the failure 1d.0's orchestrator-owned runtime fixed). A spike cycle is runtime-owning identically to mcp-test. Before dispatching, boot/own the dev runtime via the SAME readiness subcommand 1d.0 uses:

```bash
python3 ~/.claude/scripts/lazy-state.py --ensure-runtime --repo-root "{cwd}"
```

Route on the FULL M4 verdict JSON exactly as 1d.0 does (read the whole object, never `state` alone), and **proceed to dispatch ONLY when `state == READY AND health_code == 200 AND mcp_tools_present`** — the existing conjunction, do NOT invent a new one. A `state: READY` with `ownership_verified: false` (the soft owned-unverified READY) still proceeds; `STALE`/`DEAD` that recovered to READY proceed. `state ∈ {HIJACKED, BLOCKED}` (or a residual `mcp_tools_present: false`) surfaces the same `BLOCKED.md` (`blocker_kind: mcp-runtime-unready`) with the verdict's `terminal_blocker` text VERBATIM and does NOT dispatch — a subagent cannot recover a runtime the orchestrator failed to boot. This sub-step is `Bash` only (owns a background `dev:restart` + health poll) and performs ZERO file edits — **HARD CONSTRAINT 1 holds** (same rationale as 1d.0). (Cloud DEFERS the runtime spike — see the coupled `/lazy-batch-cloud` mirror: a cloud spike records the trigger and returns PENDING, never boots a runtime.)
...
A spike is pipeline-advancing: increment `forward_cycles`, append the `cycle_log` row, update `prev_cycle_signature = (feature_id, sub_skill, sub_skill_args, current_step)`, and return to Step 1a — **DO NOT fall through to §1d.**
...
1. **Ensure the runtime is up, current, AND owned — ONE `--ensure-runtime` subcommand call consuming the M4 verdict (long-build-and-runtime-ownership Phase 5, atop unified-pipeline-orchestrator Phase 5).** The probe → stale-binary check → `dev:restart` (bg) → curl-until-200 → MCP-tool assertion dance is the `lazy-state.py --ensure-runtime` subcommand (`lazy_core.ensure_runtime`), now the M4 liveness/recovery state machine. Run it ONCE instead of any hand-composed rebuild→health-poll→inspect until-loop — there is NO hand-rolled poll loop in the cycle prompt:
   ...
   **Guard-takeover — the orchestrator owns long builds (long-build-and-runtime-ownership Phase 3/4).** The `long-build-ownership-guard.sh` PreToolUse(Bash) hook (registered in `user/settings.json`) fail-OPEN denies any subagent attempt to run an exact long-build signature (`tauri build` / `cargo build --release` / `npm run build`) OR a heavy AlgoBooth quality gate (`npm run qg -- {rust,ts,sidecar}`, and the `quality-gate` alias — the fast `arch`/`docs`/`lint` groups are NOT redirected), bubbling the literal takeover token **`LONG-BUILD-OWNERSHIP-TAKEOVER`** in its deny reason. When that token surfaces from a cycle subagent's denied build, the **orchestrator session** (long-lived, persists across subagent turns) takes over the spawn instead of the subagent: run the build/gate under the Transient Build contract — `lazy_core.run_transient_build(cmd, cwd=...)` (spawns detached via `spawn_detached` so it survives a subagent tear, then synchronously AWAITS conclusion, capturing `exit_code`/`stdout` for telemetry) — and on its return call `lazy_core.promote_artifact_atomically(staging_dir, final_dir, exit_code=result["exit_code"])` so the artifact is `os.replace`'d into production ONLY on `exit_code == 0` (a torn/failed build leaves production untouched). The Transient Build contract is DISTINCT from the Persistent Service `--ensure-runtime` above (LD5: one spawn primitive, two contracts) — it does NOT write `.runtime.lock.json` and does NOT leave the process for a future cycle. This is `Bash`/`lazy_core`-driven process ownership, NOT a file edit — HARD CONSTRAINT 1 holds (same rationale as the runtime boot).

4. **No prompt amendment by hand — the script owns the runtime variant.** The mcp-test prompt variant (RUNTIME IS ALREADY UP vs RUNTIME NOT PRE-BOOTED) is selected by `emit_cycle_prompt` from the PHASES.md `**MCP runtime:**` line (step 0 above), so the probe's `cycle_prompt` already carries the right block — the orchestrator does NOT splice in a runtime paragraph. The orchestrator's ONLY job in this sub-step is the BOOT decision (step 0's `not-required` skip + the step 1 `--ensure-runtime` call): skip the boot on `not-required`, otherwise call `--ensure-runtime` so the orchestrator-owned runtime is MCP-ready when the subagent connects. (NEEDS_RUNTIME re-dispatch handling is above in step 0.)

**HARD CONSTRAINT 1 is NOT relaxed by this.** Step 1d.0 is `Bash` only — the `--ensure-runtime` subcommand owns a `run_in_background`-style `dev:restart` process plus its own health-readiness poll. It performs ZERO `Write`/`Edit` on any file (the orchestrator's sentinel-only edit scope is untouched). Owning a background process and polling a health endpoint are not file edits, exactly as Step 0.4's git reconciliation (also `Bash`-only) does not expand the edit scope.
...
**Provenance-lookup rider (code-doc-provenance-linkage D6-A — emitter-owned).** The emitted cycle prompt now carries a `provenance-lookup` section for edit-heavy cycles (`execute-plan`, `retro-feature`): the subagent consults `--provenance-lookup <file>` (a pure read over the committed `docs/provenance-index.json`) before its FIRST edit to each source/script file, and opens the cited `IMPLEMENTED.md` only when the decision ids are unfamiliar. The step lives in `cycle-base-prompt.md` (script-assembled) — do NOT restate, strip, or hand-append it here; where no index exists the step is a no-op.
...
**Completeness — route from the FULL probe JSON, never a field-extracted subset.** The atomicity rule above governs WHERE the prompt came from; the freshness rule governs WHEN; this rule governs HOW COMPLETELY the probe output is consumed. A routing/dispatch decision MUST be made against the **complete** probe JSON — never pipe it through a field-extractor (jq-style / `python3 -c "...print(d['terminal_reason'])"`) and route on that subset. Any signal outside the extracted subset is then invisible to the decision: `diagnostics`, `git_guards`, `self_edit_mode`, `governing_files_touched`, `route_overridden_by`, `cycle_prompt_refused`, `device_deferred_features`, `host_deferred_features`, `repeat_count`, etc. The `pending_hardening` section above (which withholds `cycle_prompt`/`cycle_model` on debt) is a point-harden for ONE key — this rule is the general contract covering all keys. See `~/.claude/skills/_components/lazy-dispatch-template.md` § "Full-probe-JSON read before routing" for the canonical statement.
...
The `Agent` cycle dispatch runs in the BACKGROUND: the tool call returns immediately with the child's `agentId`, and the orchestrator's turn ends. The cycle's result arrives LATER as a `<task-notification>` you are re-invoked on — it is NOT a synchronous return of the `Agent` call. **Act on ONLY the notification for the child THIS cycle dispatched** — the one whose `<task-id>` equals the `agentId` the `Agent` tool returned for this dispatch (empirically the task-id IS that agentId — treat the match as a POSITIVE "this is my cycle" signal; on any non-matching or ambiguous notification the default is to WAIT, do nothing). That matching notification is your cycle returning: run `--cycle-end` (§C1) and proceed to §1d.5 / §1e.

**Grandchild (sub-sub-agent) notifications bubble up to your session — IGNORE them.** A cycle subagent MAY dispatch its own sub-subagents (the workstation sub-subagent model above: `/execute-plan`'s test/impl agents, read-only Explore fan-outs, `/retro` research agents). When one of THOSE grandchildren stops, its `<task-notification>` surfaces to the TOP orchestrator session (you), NOT only to the cycle subagent that dispatched it. A grandchild notification carries a `<task-id>` you never dispatched and a `<summary>` naming the grandchild's sub-task (e.g. "Ground concurrency-plane systems"), NOT your cycle's `description` ("lazy-batch cycle N: {sub_skill} …"). These are the CYCLE subagent's to handle (it re-dispatches, proceeds, or halts) — they are NORMAL and are **NOT signals for the orchestrator**:
- Do NOT run `--cycle-end`, do NOT probe, do NOT route, do NOT print a cycle block on a grandchild notification.
- **NEVER `TaskStop` your cycle child (or any descendant) on a grandchild's BLOCKED / failure / error notification.** A grandchild failing is EXPECTED — a cycle subagent commonly fans out redundant Explore agents and proceeds on whichever succeed. Killing the cycle on a grandchild failure destroys a productive cycle mid-work (the 2026-07-18 incident: two Explore grandchildren returned BLOCKED on a tool wedge while the direct spec cycle had already completed grounding and was mid-draft; the orchestrator killed it twice before understanding — `grandchild-notification-misroute`).
...
**Design note (undocumented platform behavior — do not over-depend).** Grandchild-notification bubbling and the task-id↔agentId correlation are UNDOCUMENTED Claude Code behavior (confirmed via `claude-code-guide`, 2026-07-18). The rule above is robust to that: it uses the task-id match as a POSITIVE signal and its DEFAULT on any non-matching / ambiguous notification is to WAIT — the safe direction, since the incident was caused by OVER-reacting. Never add logic that DEPENDS on a grandchild notification (e.g. routing off its content); a wedged grandchild is at most an observed-friction *signal* (§1d.1 Trigger 5, tool-wedge carve-out), never a mid-cycle intervention trigger.

**F2a dispatch-by-reference (PREFERRED when available).** When the probe emits `cycle_prompt_ref` (a `@@lazy-ref nonce=<hex>` token), use it as the `prompt:` field instead of the full `cycle_prompt` text. The PreToolUse guard resolves the token → registered bytes and rewrites the tool input before the subagent runs — the subagent receives the full prompt unchanged. This eliminates the transcription-slip class: the guard rejects mismatched hashes, so an orchestrator that accidentally paraphrases the prompt gets denied, not silently dispatched. Fall back to `cycle_prompt` verbatim ONLY when `cycle_prompt_ref` is absent or null. **A by-reference dispatch is dispatchable ONLY on the SAME turn its emission was registered — this binds the LAZY-ROUTE banner's `nonce=`/`@@lazy-ref` too (2026-07-11 banner-ref divergence).** The banner-consumption rule above (§Step 1d "consume it directly") is valid because the inject hook registered that emission on THIS turn's UserPromptSubmit — so the banner's `cycle_prompt` (full text) and its ref are same-turn-fresh and dispatchable *this turn*. But the instant ANY turn boundary or compaction intervenes, the banner (and its `nonce=`) is a PRIOR-turn emission and is STALE per the Freshness rule below — do NOT dispatch by-reference from a banner carried over from an earlier turn: by then the nonce is stale/consumed/superseded, the guard's F2a cannot resolve it, and a near-miss copy (the token WITH the banner's surrounding text) can slip a literal `@@lazy-ref` line through to the subagent, which then takes 0 tool uses and returns "no task attached." On any stale-banner turn, RE-PROBE with `--emit-prompt` in-turn and dispatch THAT fresh ref (or its `cycle_prompt` verbatim). When in doubt about freshness, dispatching the banner's full `cycle_prompt` verbatim is always safe — the ref is only a transcription-saving convenience.
...
- **Concurrency:** a backgrounded harden edits **claude-config** while cycle subagents edit the **target repo** — usually different trees. **When they overlap (`self_edit_mode` — the harden touches files this run is itself editing), the harden STILL runs concurrently on the shared tree** — no pre-serialization, no monsters-in-the-closet foreground/await. The **coordination layer owns conflict correctness**: the FIFO file-lock serializes genuine write contention and conflict-routing halts only on a true semantic conflict (`concurrent-worktree-agent-coordination` Phases 2–5). The **separate** governing-file RELOAD discipline (self-edit C8, §1a) is unaffected — a self-edit commit still staleifies the in-context copy, so re-`Read` any touched governing file at the cycle boundary before composing the next dispatch. If implementation reveals a race the coordination layer does not cover, the harden surfaces it as a NEEDS_INPUT rather than baking a workaround.
...
**Sub-sub-agent tool-wedge (observed-friction species — with a transient carve-out; `grandchild-notification-misroute`).** A dispatched agent — usually a cycle subagent's grandchild (§1d-await) — that reports a TOTAL tool-execution WEDGE (every tool call erroring BEFORE executing: e.g. the Claude Code `No tools needed for summary` gating message, or an equivalent all-tools-dead session) is a species of observed friction. Handle it **POST-HOC only** — NEVER as a mid-cycle intervention (§1d-await forbids acting on a grandchild notification) — and ONLY when it is NOT transient:
- **Transient → do NOT harden.** A SINGLE wedged agent whose siblings / re-dispatch succeed, or a wedge attributable to a platform/API blip (an API 5xx/429, a one-off session-init failure like the undocumented `No tools needed for summary` message). A one-off blip must not spawn a spurious harden round — note it and continue.
- **Non-transient → DO harden.** The wedge is REPRODUCIBLE (recurs on re-dispatch) OR RECURS ≥2× in a run AND is not attributable to a platform blip — i.e. it points at a claude-config surface (a hook, an MCP registration, a dispatch pattern). Emit an observed-friction harden (above) with `friction_summary="sub-sub-agent tool-wedge"` AFTER the current cycle resolves.

When uncertain whether a wedge is transient, treat a SINGLE occurrence as transient (ignore) and a RECURRENCE as harden-worthy. (This carve-out and §1d-await agree: the orchestrator never intervenes in a live cycle over a grandchild notification; a wedge is at most deferred, post-hoc friction.)

**All triggers → hardening dispatch:**  
Parse the `--emit-dispatch hardening` output JSON (`dispatch_prompt`, `dispatch_model`, `dispatch_class`) and dispatch `dispatch_prompt` VERBATIM as an `Agent` call using `dispatch_model` (always `"opus"`). The prompt was registered at emit time; the guard will ALLOW it. Reference: `~/.claude/skills/_components/hardening-dispatch.md` for the full required `--context` keys. **Bracket the hardening dispatch `--cycle-begin --feature-id {feature_id} --nonce {nonce} --kind meta --sub-skill hardening`** (`--sub-skill hardening` verbatim — the `dispatch_class`), `--cycle-end` after, on every return path (§C1). The `--sub-skill hardening` value is load-bearing: it is the cycle marker's identity signal that lets the dispatched harden's own `lazy-state.py --record-intervention` pass containment (the `--record-intervention` capture is exempted for a hardening-class cycle subagent ONLY — `dispatched-harden-record-intervention-refused-by-containment`; every genuinely-dangerous lifecycle op stays refused). Omitting or mis-naming `--sub-skill` on the harden bracket re-breaks the dispatched harden's mandated intervention capture.

**Honor the harden return's `reconcile:` handback (docs/bugs OUT-OF-PIPELINE contract).** After the hardening subagent returns (and after its `--cycle-end`), if its structured summary carries a `reconcile:` **orchestrator handback** — a `docs/bugs/<slug>/` spec whose fix the round shipped out-of-pipeline but could not archive itself (a dispatched harden is a cycle subagent; `--archive-fixed`/`--link-provenance` are `refuse_if_cycle_active`-refused for it) — run the two named script-owned ops now, from the claude-config checkout: `python3 ~/.claude/scripts/bug-state.py --repo-root <claude-config> --archive-fixed docs/bugs/<slug>` then `python3 ~/.claude/scripts/lazy-state.py --repo-root <claude-config> --link-provenance --id <slug> --commits <fix-sha>`, then commit + push. You are AUTHORIZED for these — they are the same script-owned archive-on-fix ops as the normal bug-completion path (§Gate-1 bug completion). This closes the `docs/bugs/CLAUDE.md` reconciliation contract so the harden-authored spec leaves the open bug queue instead of re-entering via `/plan-bug` (the three-burned-cycles class, `docs/bugs/harden-rounds-skip-docs-bugs-reconciliation-contract`). A `reconcile:` value of `done` / `deferred (...)` / `none` needs no orchestrator action.
...
2. Emit the T3 cycle-return block (Step 3 / orchestrator-voice.md) under the cycle's T2 heading: a `done` line (ONE line — duration + the load-bearing outcome); on an `/execute-plan` cycle an `audit` line — REQUIRED, this is the execution-model audit signal `/lazy-batch-retro` grades (workstation-recursive-subagent-dispatch): confirm HOW the batch executed — the skill's test-agent→impl-agent dispatch model (e.g. `audit  RED→GREEN 33/33 · gates qg:ts green · dispatched test+impl agents`) or sanctioned inline execution with manual test-first discipline (e.g. `audit  RED→GREEN 33/33 · gates qg:ts green · inline (small mechanical batch)`) — plus the gate outcome; a `ledger` line (the step 4/4a guard outcome — `clean · pushed` when healthy); and a `next` line (the fresh probe's routing, or `terminal: <reason>`). The retired 3–5-bullet cycle summary must NOT reappear — details live in the commit and the docs the subagent wrote.
...
4a. **Post-`/execute-plan` (and `/mcp-test`) ledger-consistency guard (guardrail D — codifies the previously-ad-hoc operator check).** When the cycle that just returned was `/execute-plan` or `/mcp-test`, run a SINGLE-TURN consistency check BEFORE the next state probe (Step 1a). This is one scripted check fired on the cycle's completion notification — NOT polling, so HARD CONSTRAINT 7 (no active waiting) holds; these are `Bash` reads, so HARD CONSTRAINT 1 holds too. The cycle subagent is supposed to leave a clean, consistent ledger via the atomic gate+commit (Step 1d `/execute-plan` override), but it empirically loses its turn between gates and commit — this guard catches the residue deterministically instead of relying on operator memory.
   ...
   - `clean_tree` or `head_matches_origin` failing → **pause-vs-terminal discriminator gate FIRST (Gap 2, `docs/bugs/adhoc-orchestrator-redundant-recovery-on-background-suite-reinvoke`; `/execute-plan` cycles only).** BEFORE routing to recovery, if the cycle that just returned was `/execute-plan`, run the deterministic discriminator (a dirty tree under a paused cycle is work-in-progress, not residue):
     ```bash
     python3 ~/.claude/scripts/lazy-state.py --repo-root <repo_root> --execute-plan-liveness --plan {plan_file}
     ```
     It reads the execute-plan run marker (`~/.claude/state/execute-plan/<md5(repo_root)[:12]>.json` — present iff the `/execute-plan` cycle is IN FLIGHT, the authoritative signal per `dispatched-agent-liveness.md`) + the plan `status:` + the marker's FILE mtime, NON-destructively, and prints `{"marker_present", "plan_status", "verdict"}` (always exit 0).
       - `verdict == "paused"` (marker present + plan not `Complete` + marker mtime FRESH — younger than the wedge-staleness bound) → the returned cycle is a LIVE PAUSE — a backgrounded suite about to be harness-re-invoked, NOT a resultless return. Do **NOT** dispatch recovery (it would collide one-writer with the re-invoked agent). Emit a T6 line `⚠ execute-plan cycle paused (backgrounded suite) — recovery suppressed, awaiting harness re-invocation` and fall through to the next state probe (Step 1a) instead of the recovery emit below.
       - `verdict == "wedge-candidate"` (marker present + plan not `Complete` + marker mtime STALE past the live-suite window — the dead-lineage stall in `docs/bugs/execute-plan-liveness-blind-to-dead-lineage-stall`) → do **NOT** keep suppressing indefinitely (that is the "wait forever on a dead lineage" stall this closes). CONFIRM live-vs-dead via the **genuine-wedge fallback** below (the `TaskList` lineage probe, `dispatched-agent-liveness.md` §57–62): lineage **DEAD** → route the recovery dispatch below (a confirmed wedge); still **LIVE** → treat as `paused` (safe to keep waiting). **`wedge-candidate` NEVER tears down the marker** (that authority is HARD-PARKED as decision #12) — it only routes confirm-then-recover.
       - `verdict == "commit-pending"` (marker present + plan `Complete` — the OPPOSITE-direction companion, `docs/bugs/execute-plan-liveness-false-terminal-on-live-mid-commit-cycle`) → plan-`Complete` is set at `/execute-plan`'s FINALIZE step, BEFORE the gate + atomic commit + marker removal, so `{marker present + Complete}` is the finalize→commit window: the cycle may be **ALIVE mid-commit** (must WAIT — a blind recovery here collides one-writer with the still-live cycle, the live incident) or **DEAD** (recovery appropriate). Do **NOT** route recovery directly. CONFIRM live-vs-dead via the SAME **genuine-wedge fallback** below (the `TaskList` lineage probe, `dispatched-agent-liveness.md` §57–62): lineage **DEAD** → route the recovery dispatch below; still **LIVE** → treat as `paused` (safe to keep waiting — it is mid-commit). **`commit-pending` NEVER tears down the marker** (decision #12) — confirm-then-recover only.
       - `verdict == "terminal"` (marker absent, OR any read error — fail-safe to the legacy behavior) → the cycle is genuinely done/gone (it cleaned up its own marker at completion, or the signal is unreadable) → proceed to the recovery dispatch emitted below, unchanged.
     For a `/mcp-test` cycle (no `--plan`) skip the discriminator entirely and route to recovery as before — the gate is scoped to `/execute-plan` returns. **Genuine-wedge fallback (now verdict-routed — the discriminator's `wedge-candidate` OR `commit-pending` triggers it):** if the marker persists with NO live descendant after a bounded wait (`TaskList` shows the lineage dead — `dispatched-agent-liveness.md` §57–62), that is a genuine wedge, not a pause, and recovery IS appropriate (do not loop forever on a dead lineage). On the `terminal` (or confirmed-wedge) path, route to the **recovery dispatch emitted below** (`--emit-dispatch recovery`, NEVER hand-composed — see the reconciling note before the emit block). It runs as a recovery cycle (NOT a numbered cycle, does NOT increment `forward_cycles`); its contract — owned by `dispatch-recovery.md` — is to stage + commit + push any uncommitted/unpushed residue, then re-run `--verify-ledger` until ok.
   ...
   **Do NOT hand-compose the recovery subagent.** The `failing_check` bullets above describe the recovery dispatch's *contract* (the job the emitted agent performs — owned by `dispatch-recovery.md`), NOT a prompt for the orchestrator to author. The ONLY sanctioned dispatch is the `--emit-dispatch recovery` emission below, dispatched VERBATIM by-reference (prefer `dispatch_prompt_ref`). A hand-composed "reconcile-and-commit recovery agent" prompt is not a registered emission, so the validate-deny guard denies it (2026-07-11 incident: a hand-composed reconcile+commit prompt for the `managed-llm-credits` part-3 residue was denied, then re-issued correctly via `--emit-dispatch recovery`). This mirrors the input-audit rule at §1d.5 ("the component … is the subagent's contract, not the orchestrator's composition source") and the blanket rule under §Notes ("hand-composed dispatch prompts are no longer sanctioned for any class").
...
5. The SCRIPT has already advanced `forward_cycles` for this real-skill cycle at its `--cycle-end` bracket (the orchestrator no longer hand-counts). Return to Step 1a. **Both counters are monotonic across feature transitions (HARD CONSTRAINT 8).** If the next state-script call returns a different `feature_id` — e.g. because this cycle's `__mark_complete__` finished the prior feature, or the queue rolled forward to the next ready feature for any other reason — the counters continue from where they were (the script never resets either counter on the boundary; the run marker persists across the transition).
...
**Cycle accounting at resume — two classes.** Resume counter semantics depend on *how* the prior run ended (the discriminator is the checkpoint's `operator_authorized` flag, set at `--run-end --reason checkpoint` time):

- **Operator-authorized resume** (a deliberate `/lazy-batch <N>` re-invoke after an operator-authorized checkpoint stop) gets a **fresh `0/0` budget** — the previous session's cycle count is intentionally dropped. Each `/lazy-batch <N>` run the operator authorizes is a bounded budget that starts clean. (`lazy_core.restore_checkpoint_counters` no-ops for an operator-authorized checkpoint, leaving the marker's by-design `0/0`.)
- **Automatic reliability resume** (an *unattended* mid-run pause — e.g. ≥2 guard denials — that wrote a checkpoint WITHOUT `--operator-authorized`) **carries the counters forward monotonically**, so an auto-resume cannot silently exceed the authorized `max_cycles` (HARD CONSTRAINT 8). A pre-fix checkpoint file lacking the flag takes this carry-forward path too.
...
Per that component's exclusion list, these terminals are NOT routed here and keep their existing behavior: `max-cycles` (cost bound — hard stop), `all-features-complete` (genuine success), `cloud-queue-exhausted` / `device-queue-exhausted` / `host-capability-saturated` (environment / host-capability — re-run on the right host), and `queue-missing` (no queue to continue). The research-pending terminals (`needs-research` / `queue-blocked-on-research`) keep their specialized Step 4 / Step 1f handling, which already lets the operator continue (in-session chat upload or re-invoke) rather than dead-ending; the component's "defer this research-pending feature & continue" option is available there as an enhancement when the queue has independent downstream work.
...
- Cycle accounting across the halt follows the two-class rule (Step 1f). A deliberate operator-authorized `/lazy-batch <N>` re-invoke gets a fresh `0/0` budget — an independent bounded run the operator authorizes. An *automatic* reliability pause (unattended checkpoint written WITHOUT `--operator-authorized`) instead carries the counters forward monotonically so the auto-resume cannot exceed the authorized `max_cycles`.
...
- **Checkpoint-resume counter semantics (two classes).** A checkpoint records `operator_authorized` (set at `--run-end --reason checkpoint` time). An **operator-authorized** resume (deliberate `/lazy-batch <N>` re-invoke) starts a fresh `0/0` budget — `lazy_core.restore_checkpoint_counters` no-ops, leaving the marker's by-design start. An **automatic reliability** resume (unattended pause written WITHOUT `--operator-authorized`, and pre-fix checkpoint files) carries the counters forward monotonically so an auto-resume cannot exceed the authorized `max_cycles` (HARD CONSTRAINT 8). Mirrored in `/lazy-batch-cloud` per the coupled-pair rule.
- **Runtime-readiness routing (Step 1d.0 — long-build-and-runtime-ownership Phase 5).** For `mcp-test` cycles the orchestrator calls `lazy-state.py --ensure-runtime` ONCE and routes on the M4 verdict's `state ∈ {READY, STALE, HIJACKED, DEAD, BLOCKED}` (no hand-rolled poll loop): READY / STALE→READY / DEAD→READY proceed; **`state ∈ {HIJACKED, BLOCKED}`** (foreign port-holder never SIGKILLed; or bounded-recovery exhausted / unrecoverable `mcp_tools_present: false`) is a terminal — write `BLOCKED.md` `blocker_kind: mcp-runtime-unready` (verdict `terminal_blocker` as the body) and dispatch NO subagent against the dead/hijacked runtime. A guard-redirected long build (`long-build-ownership-guard.sh` bubbles `LONG-BUILD-OWNERSHIP-TAKEOVER`) is taken over by the orchestrator via `run_transient_build` + `promote_artifact_atomically` (Transient Build contract — distinct from the Persistent Service runtime, LD5). Mirrored into `/lazy-batch-cloud` as a recorded divergence (cloud Step 9 returns `__write_deferred_non_cloud__` and never reaches Step 1d.0 — the verdict-consumption + guard-takeover are workstation-only).
- **Spike cycle routing (Step 1c.7 — spike-pipeline-role).** `compute_state` emits `sub_skill == "spike"` on two on-disk signals — a `**Spike:** required` PHASES.md line with no PASS `SPIKE_VERDICT.md` (Step 9.5, before mark-complete) or a `BLOCKED.md` `blocker_kind: runtime-spike-verdict-pending` (Step 3, blocked resolver). `spike` is a registered dispatch CLASS (always Opus), not a `cycle_prompt` skill: the orchestrator pre-boots the orchestrator-owned runtime via `--ensure-runtime` (same conjunction as Step 1d.0), then emits `--emit-dispatch spike` and dispatches the returned prompt VERBATIM. **PASS** → the spike ticked its gated deliverable (gate-boundary-safe reconcile; no top-status flip / no receipt) and the pipeline continues to the prescribed next cycle; **FAIL** → the spike wrote `NEEDS_INPUT.md` (`written_by: spike`, `spike_verdict: fail`) and halts — **NEVER provisionally auto-accepted even under `--park --park-provisional`** (the `lazy_core.provisional_eligibility` Spike-FAIL carve-out is fail-closed). Cloud DEFERS the runtime spike to workstation (recorded divergence — like `/investigate` / `/mcp-test`). Mirrored into `/lazy-bug-batch` + `/lazy-batch-cloud` via the overlay generator (coupled pair).
...
- The orchestrator is single-session by design. State lives in the filesystem sentinels; restart is free. The one run-scoped exception is the **checkpoint** (`lazy-run-checkpoint.json`): a sanctioned `--run-end --reason checkpoint` persists the next route + counters across a single resume — but resume counter semantics follow the two-class rule (Step 1f): an operator-authorized checkpoint resumes with a fresh `0/0` budget (counts dropped), while an automatic reliability pause carries the counters forward monotonically.
...
<!-- feature-budget-guard-and-skip-ahead Phase 4 (2026-06-20) — coupled-pair mirror note:
       ...
       "NO divergence" rows. -->
<!-- host-capability-declaration-for-gated-features Phase 6 (2026-06-20) — coupled-pair mirror note:
       - WU-2: the `host-capability-saturated` terminal (host-axis generalization of
         device-queue-exhausted) was added to HARD CONSTRAINT 10's sanctioned-stop set, the
         terminal-handling bullets (Step 1b), the §1c.6 halt notification list, and the
         `--terminal-reason` sanctioned set — mirrored into lazy-batch-cloud.
       - WU-2: the full-probe-JSON read rule's enumerated keys gained `host_deferred_features`
         (the always-present probe key listing each capability-deferred feature_id for end-of-run
         flush) — mirrored into lazy-batch-cloud.
       NO cloud divergence — the host-capability gate is workstation-axis (cloud already defers ALL
       MCP/device work via DEFERRED_NON_CLOUD.md, so cloud rarely reaches the host probe), but the
       terminal + probe-key surface is identical prose. The `/lazy` ↔ `/lazy-cloud` pair gained the
       same `host-capability-saturated` State Machine Summary terminal row. See lazy-batch-cloud
       "Differences from /lazy-batch" — these appear as "NO divergence" rows. -->
<!-- lazy_core SANCTIONED_STOP_TERMINAL gained "host-capability-saturated" (clean stop, re-opens on a
     capability-bearing host) so the orchestrator may end a run on it without --operator-authorized,
     exactly like device-queue-exhausted. test_lazy_core asserts its membership. -->
