...
* This is similar to the Auto-Create Entries' existing `SUBMISSION_ATTEMPT_LIMIT = 1000`.
    // Constant: `LINKED_LOOKUP_SYNCED_ENTRIES_LIMIT = 100`.

***

### Per-Field Overflow Behavior
* When the 101st entry attempts to link to a field already at the 100-entry limit, the user-defined removal strategy will be applied. If the user does not define a strategy, the following strategy will be used:
...
* If the user has defined a removal strategy, that strategy will be used instead of the above default strategy.
...
* `CreateEntriesFromActionsService` - Handles auto-create entries logic.
...
FieldLookup {  
   ...
   string LinkedLookupRemovalStrategy  
}
```
  ...
  {  
      ...
      {  
          ...
          if (hasLinkedLookup && !behavior.FastHasFlag(SubmissionBehavior.SuppressCreateEntriesFromActions) && validPrefillConfigs.Any())  
              CreateEntriesFromActionsService.CreateEntries(formEntry, validPrefillConfigs, userId, linkedLookupSyncContext);  
          else  
...
private void SyncAllEntries(Form sourceForm, FieldLookup newlyLinkedLookup)  
  {  
      ...
      {  
          ...
          // Enforce field entry limit (100 entries max) with user-defined removal strategy  
