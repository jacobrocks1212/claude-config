...
6. **The orchestrator MUST print a Zero-Context Operator Briefing AND re-print the rich `## Decision Context` to chat BEFORE calling `AskUserQuestion`.** The operator may have been away for hours and retains NO session context. The briefing (Step 1g step 2, mirroring `/lazy-batch` step 2a) catches them up from zero — what's being worked, why we halted, every option with pros/cons and fit against the original requirements, and a recommendation; the verbatim re-print preserves the full sentinel context. `AskUserQuestion` truncates option descriptions in its UI (especially on mobile); the chat text is the load-bearing context, and the `AskUserQuestion` option set MUST exactly match the options presented in chat (same labels, 1:1). Never call `AskUserQuestion` against a malformed `NEEDS_INPUT.md` — surface the malformation as a quality issue and halt instead (see Step 1g.1).
...
2. Print the **Zero-Context Operator Briefing** (per `/lazy-batch` Step 1g step 2a — assume the operator has been away for hours with NO session context: where we are, why we halted, the original requirement at stake, every option with pros/cons/fit and a recommendation), THEN re-print the rich body to chat VERBATIM (step 2b).
3. `AskUserQuestion` per decision (label, header, options) — **option set MUST exactly match the briefing's options 1:1** (same labels; recommendation marked `(Recommended)`, listed first; never introduce a UI option not explained in chat).
