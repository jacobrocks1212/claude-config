...
**Cloud nuance (background dispatch).** A cloud cycle subagent may be dispatched to run in the background (HARD CONSTRAINT 9 references in-flight background cycle agents). When it is, the ONLY output permitted before the result block is a SINGLE terse line — `▶ Cycle fwd {forward_cycles}/{max_cycles} · meta {meta_cycles}/{2*max_cycles} · {feature_name} · {sub_skill} (dispatched)` — with no following prose. Specifically do NOT narrate "running in the background", "waiting on the completion notification", or any commit-race reasoning while it runs (this is exactly the noise the discipline removes). When the cycle completes, emit the canonical result block.
