...
| Intervention-efficacy end-of-run evaluation (intervention-efficacy-tracking D10-A) | **MIRRORED (shared)** — the same once-per-run `python3 ~/.claude/scripts/efficacy-eval.py --repo-root . --json` invocation at the §1c.6 end-of-run flush (alongside the incident scan, BEFORE `--run-end`); record updates under `docs/interventions/` are committed docs in both environments; capture itself is script-owned inside `--apply-pseudo __mark_complete__`/`__mark_fixed__` (shared lazy_core — the skills add nothing). | same invocation; the record-update commit rides the run's FINAL cloud push to `main` (the telemetry-segment transport above) so verdicts survive container reclaim, and the evaluator's run-window accrual counts cloud runs via the committed `docs/telemetry/cloud/*.jsonl` segments merged with the state-dir ledger. |
