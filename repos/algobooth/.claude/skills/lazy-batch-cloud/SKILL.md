...
- `--per-feature-cycle-cap <N>` (optional) → pass `--per-feature-cycle-cap <N>` to every `lazy-state.py --cloud` probe in Step 1a, overriding the dynamically-computed per-feature ceiling `L_task`. Default (flag absent): `lazy-state.py` computes `L_task`. **NO cloud divergence** — identical semantics to `/lazy-batch`; see `/lazy-batch` Step 0 for the full rationale.
