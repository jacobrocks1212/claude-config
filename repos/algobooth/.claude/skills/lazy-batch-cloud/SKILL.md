...
8. **TWO session-global monotonic counters replace the single `cycle` counter.** Identical model to `/lazy-batch` HARD CONSTRAINT 8 — both initialized once in Step 0 and NEITHER reset on feature transitions.
   - **THE SCRIPT is the sole budget-counting authority** (cycle-budget-counters-double-count-on-probes-and-inject-hook). The orchestrator NO LONGER hand-counts `forward_cycles` / `meta_cycles`: `lazy-state.py --cloud` advances them at the `--cycle-end` bracket (keyed on the closed cycle marker's `--kind`) and at `--apply-pseudo`. The `--repeat-count` probe and the per-turn inject hook are BUDGET-NEUTRAL (they advance only the loop-detection streaks). The "increment `forward_cycles`/`meta_cycles`" instructions below are DESCRIPTIVE of what the script does at that step, not an orchestrator action.
   - **`forward_cycles`** — counts pipeline-advancing work. Ceiling: `max_cycles`. Advanced by the SCRIPT at: (a) the `--cycle-end` bracket when the closed cycle marker's `--kind` is `real` (every real-skill Agent dispatch is bracketed `--cycle-begin --kind real … / --cycle-end`), and (b) `--apply-pseudo` for a pipeline-advancing pseudo-skill (`__mark_complete__`, `__write_deferred_non_cloud__`, `__write_validated_from_results__`, `__write_validated_from_skip__`, `__flip_plan_complete_cloud_saturated__`). **Capped at Step 1c** (`if forward_cycles >= max_cycles` → the existing max-cycles halt).
   - **`meta_cycles`** — counts resolution/recovery/cleanup work. **NO ceiling — uncapped by design (operator decision 2026-06-14).** Advanced by the SCRIPT at the `--cycle-end` bracket when the closed cycle marker's `--kind` is `meta` — every resolution/recovery dispatch is bracketed `--kind meta` (Step 1g decision-resume, Step 1h blocked-resolution, Step 1i operator-directed halt-resolution, LOOP-DETECTED / recovery dispatches). The stale-plan flip pseudo-skill `__flip_plan_complete_stale__` classifies to meta whenever it advances the budget. The meta loop is NOT bounded by a meta cap; the run's only hard stop is the `forward_cycles >= max_cycles` cap at Step 1c. `meta_cycles` is still tracked and displayed (as a bare count), but there is NO `if meta_cycles >= …` halt — Step 1g/1h/1i have no meta-cap check.
...
12. **Concurrent-writer awareness — no monsters-in-the-closet serialization.** Other agents may be committing to this same worktree/branch concurrently — a parallel lane, a background harden dispatch, or a second interactive/scheduled session. An unexpected incoming commit / moved HEAD is EXPECTED, not a defect to panic on or halt for. Genuine write contention is resolved by the coordination layer (git safety + the FIFO file-lock + conflict-routing) — not by the orchestrator pre-serializing dispatches on the mere possibility of a collision.

**Cloud-specific:** the cycle subagent operates under the same cloud-environment limitations documented in `/lazy-cloud` — no Tauri runtime, no MCP HTTP server, no audio device, no Windows-only tooling. **Additionally, the cloud cycle subagent MUST NOT use the `Agent` tool — the inline override is deliberate CLOUD policy (workstation-recursive-subagent-dispatch, 2026-07-09, lifted the ban on WORKSTATION only: cloud containers have no persistent state and are reclaim-prone, so zero sub-subagents remains the cloud steady state).** This forces a load-bearing override of any dispatched skill's sub-subagent contract: skills that nominally dispatch sub-subagents (e.g. `/execute-plan` → Sonnet test-agent + impl-agent fanout, `/retro` → research subagents A–G) are performed INLINE inside the cycle subagent itself using `Edit`/`Write`/`Read` directly. The cycle subagent's prompt (Step 1d below) makes both limitations explicit and enumerates the per-skill inline overrides. **This override applies only at the cycle-subagent level** — the orchestrator still dispatches exactly one `Agent` per cycle, identical to `/lazy-batch`. The override never expands the orchestrator's `Write`/`Edit` scope (HARD CONSTRAINT 1 still holds — the orchestrator edits only sentinels). **Concurrent-writer awareness:** other agents may be working this same worktree/branch concurrently — an unexpected commit / moved HEAD is expected, not a defect. Genuine write contention is resolved by the coordination layer (git safety + the FIFO file-lock + conflict-routing) — not by halting.
...
- `--park-provisional` (optional; park-provisional-acceptance) → sets `park_provisional_mode = true` (requires `--park`; alone it is an argument error). Identical semantics to `/lazy-batch`: Step 1a appends `--park-provisional` to every probe; eligible parked-class sentinels route `__provisional_accept__` (`lazy-state.py --provisionalize-sentinel` + a `resolution_kind="provisional"` apply dispatch — docs-only, runs identically in cloud; the apply subagent's commit is pushed IMMEDIATELY per the cloud durability rule); an unratified `NEEDS_INPUT_PROVISIONAL.md` blocks completion mechanically, parks for ratification once validated, and halts non-park probes on `needs-ratification` (Step 1g-ratify — bind `provisional-ratification.md` with the cloud push rule). The T7 report carries the `### Provisionally accepted decisions (--park-provisional)` digest table. Provisional acceptance is environment-agnostic (no cloud divergence beyond the push rule).
...
> `/lazy-batch-cloud`: unrecognized argument `{token}`. Usage: `/lazy-batch-cloud <N> [--allow-research-skip] [--adhoc "<task>"] [--park] [--park-provisional] [--per-feature-cycle-cap <N>] [--strict-research-halt]`.
...
**Park-mode arm (`{park_flags}`) — load-bearing.** Substitute `{park_flags}` with `--park` when `park_mode == true` (the operator passed `/lazy-batch-cloud --park`), and ALSO `--park-provisional` when `park_provisional_mode == true`; otherwise `{park_flags}` is empty (byte-identical to a non-park cloud run's run-start). `--park` is the umbrella flag that arms BOTH `park_needs_input` AND `park_blocked` in the marker (the same expansion as `--set-park on`), so park mode is live from cycle 1's probe. **Do not omit this:** park is a RUN_FRESH_FIELD re-supplied at run-start from the invocation flags — a run-start that drops `{park_flags}` writes a park-OFF marker EVEN under a `--park` invocation, so an unattended cloud `--park` run would halt on the first blocker / needs-input until a mid-run `--set-park` toggle (the `lazy-run-marker-park-arm-and-forward-cycle-inflation` friction, 2026-07-17). Park mode is environment-agnostic — cloud runs park exactly like workstation runs.
...
- `park_provisional_mode = <parsed>` — `true` if `--park-provisional` (requires `park_mode`); `provisional_accepted = []` — digest rows per `/lazy-batch`.
...
- Re-probing when a banner is present advances the loop-detection STREAKS twice for one logical cycle — a protocol violation the retro grades. (The BUDGET counters `forward_cycles`/`meta_cycles` are no longer probe-driven — the SCRIPT advances them at the `--cycle-end` bracket / `--apply-pseudo`, so a double-probe cannot inflate the budget.)
...
**Park-mode probe flag (`--park` only).** The skill's own `--park` invocation flag arms park mode. When `park_mode == true`, append BOTH `--park-needs-input` AND `--park-blocked` to EVERY `lazy-state.py --cloud` probe invocation in this step (base or enriched form alike). With these flags, the script skips features carrying an unresolved `NEEDS_INPUT.md` (instead of halting on `needs-input`) OR a feature-local `BLOCKED.md` (instead of halting on `blocked`), and reports them in a `parked[]` array on the JSON output — each entry tagged `sentinel_kind` (`needs-input` | `blocked`) — the input to the Step 1g park path, the Step 1g-flush, and the §1c.6 park notifications. When every remaining feature is parked, the script returns the distinct `queue-exhausted-all-parked` terminal (handled in Step 1b). When `park_mode == false`, call the script plain (NEITHER flag) — existing behavior, byte-for-byte; the `parked[]` key never appears, and a feature-local `BLOCKED.md` still halts on `blocked` (Step 1h). Park is environment-agnostic — this is identical to `/lazy-batch` (no cloud divergence). When `park_provisional_mode == true`, ALSO append `--park-provisional` — identical to `/lazy-batch` Step 1a (eligible sentinels route `__provisional_accept__`; the park-mode-only `provisional[]` key surfaces pending ratifications).
...
2. **halt** (both modes) — fired on every terminal/halt: `NEEDS_INPUT` halt, `BLOCKED` halt-for-manual, `needs-research` strict halt, `queue-blocked-on-research`, `cloud-queue-exhausted`, `device-queue-exhausted`, `host-capability-saturated`, `queue-missing`, `all-features-complete`, `queue-exhausted-all-parked` (`--park` mode — after the flush), `queue-exhausted-budget-deferred` (budget-guard — all items deferred to queue tail), `max-cycles`, script-error, and any future obstacle terminal. Most of these already carry per-terminal `PushNotification` calls above — this point names the policy explicitly so no terminal can be added without a notification. **MANDATORY: run `python3 ~/.claude/scripts/lazy-state.py --run-end` on EVERY terminal/halt path, BEFORE firing the PushNotification.** `--run-end` deletes the run marker AND the prompt registry. Missed deletion is self-healing (24h staleness + session-id mismatch on re-run) but is a protocol violation the retro grades. **Telemetry-segment commit (harness-telemetry-ledger D5-B, cloud-only):** on a cloud run, `--run-end` also flushes the run's telemetry-ledger segment to `docs/telemetry/cloud/<run_id colon-stripped>.jsonl` (script-owned, fail-open) and surfaces `telemetry_flushed: {path, events}` in its JSON. When that key is present, the orchestrator MUST `git add docs/telemetry/cloud/ && git commit -m "docs(telemetry): flush cloud run telemetry segment" && git push` (work-branch push rules unchanged) BEFORE the PushNotification, so the segment survives container reclaim — otherwise the run's telemetry dies with the container and the workstation trends aggregator never sees it. Key absent (no events / flush failed) → nothing to commit, proceed normally.
   ...
   **Incident scan runs ONCE per run at the end-of-run flush, BEFORE `--run-end` (incident-auto-capture D6-A; identical to `/lazy-batch` §1c.6 — NO cloud divergence: the collector is docs+state-dir-local and scans THIS environment's keyed state dir).** On the FIRST terminal/halt path this run reaches (any bullet above), run `python3 ~/.claude/scripts/incident-scan.py --repo-root .` in the orchestrator session before the `--run-end` call — the deterministic, read-only collector clusters this run's fresh deny-ledger/hook-event evidence, dedups against open + archived `incident_key`s, and enqueues ≤2 bug stubs (with `INCIDENT.md` evidence capsules) via the sanctioned `--enqueue-adhoc --type bug` path, so captures land at the top of the bug queue for the NEXT run. Relay its report verbatim (the summary line + any `➕ Enqueued ad-hoc bug …` announce lines are sanctioned output on the T7 report path). NON-BLOCKING: a non-zero exit prints one `⚠ incident-scan failed` warning and the run-end sequence continues unchanged — the scan must never wedge a terminal path. Once per run (skip if the scan already ran in this run-end sequence); on-demand reruns are `/incident-scan`.

   **Intervention-efficacy evaluation runs ONCE per run at the same end-of-run flush, BEFORE `--run-end` (intervention-efficacy-tracking D10-A).** Immediately alongside the incident scan above (same first-terminal trigger, same once-per-run rule), run `python3 ~/.claude/scripts/efficacy-eval.py --repo-root . --json` in the orchestrator session — the standalone evaluator (read-only over the telemetry ledger; sole post-capture writer of `docs/interventions/<id>.md` records) accrues each open hypothesis's post-ship run window and writes any DUE verdicts (CONFIRMED / REFUTED / INCONCLUSIVE per the D5 bands) into the records. A REFUTED verdict auto-enqueues `reconsider-<id>` via the sanctioned `--enqueue-adhoc --type bug` path (two-layer recurrence-guarded — at most once EVER per intervention); announce it exactly like any ad-hoc enqueue. Relay the JSON summary's verdict lines + any `needs_triage` escalations verbatim (an escalated INCONCLUSIVE hypothesis is information for the operator, never a halt). Record updates are committed docs — stage + commit them per the **§1c.6 flush-commit staging contract** below (`docs(interventions): efficacy verdicts — <run terminal>`; cloud run: the commit rides the run's final push to `main`, the same transport as the telemetry segment above) so verdicts survive container reclaim. NON-BLOCKING: a non-zero exit prints one `⚠ efficacy-eval failed` warning and the run-end sequence continues unchanged — the evaluator must never wedge a terminal path. Zero records / nothing due → clean no-op output, nothing to commit. On-demand reruns any time: `python3 ~/.claude/scripts/efficacy-eval.py --repo-root . [--dry-run]`.

   **Harness-change canary watch runs ONCE per run at the same end-of-run flush, BEFORE `--run-end` (harness-change-canary-rollback D6-A; identical to `/lazy-batch` §1c.6 — NO cloud divergence: the watcher is docs+state-dir-local and reads THIS environment's telemetry + keyed state dir).** Immediately alongside the efficacy evaluation above (same first-terminal trigger, same once-per-run rule), run `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root . --json` in the orchestrator session — the same evaluator in canary mode (read-only over the telemetry ledger + fresh incident sources; the SOLE writer of `canary.*` record fields + the trip-time `EVIDENCE.md`) accrues each OPEN canary window and, on a trip (D2 band regression or ≥2 attributable fresh incidents on the change's surface), flags-and-enqueues an evidence-bearing `canary-revert-<id>` bug stub via the sanctioned `--enqueue-adhoc --type bug` path (once EVER per canary — never a silent revert). Surface the JSON's `notify` line (`⚠ canary tripped: <id>`) exactly like any ad-hoc enqueue. Record updates + the seeded evidence are committed docs — stage + commit them per the **§1c.6 flush-commit staging contract** below (rides the run's final push to `main`, the same transport as the efficacy verdicts) so trips survive container reclaim. NON-BLOCKING: a non-zero exit prints one `⚠ efficacy-eval --canary failed` warning and the run-end sequence continues unchanged — the watcher must never wedge a terminal path (it is fail-open by construction). Zero open canaries → clean no-op. On-demand reruns any time: `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root . [--dry-run]`.

   **TWO-SCOPE flush — the interventions-bearing scope MUST be covered (interventions-telemetry-repo-scope-split-brain, D2; MIRRORS `/lazy-batch` §1c.6).** The efficacy review + canary invocations above run `--repo-root .` (this run's TARGET repo — AlgoBooth in cloud). Intervention records live ONLY in the **claude-config** checkout (they follow the FIX), while the telemetry that grades them accrues in the TARGET repo's `repo_key`-keyed state dir (it follows the RUN) — so a target-only flush sees the telemetry but NO records and can never produce a verdict (the zero-verdicts-ever symptom this fix closes). Therefore, immediately after the `--repo-root .` pair, run the SAME two evaluators a SECOND time rooted at the cloud container's claude-config checkout: `python3 ~/.claude/scripts/efficacy-eval.py --repo-root <claude-config checkout> --json` then `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root <claude-config checkout> --json` (the harden dispatch already runs against a claude-config checkout in this environment — use that checkout root). This claude-config-rooted evaluation sees the originating target repo's telemetry via the **Phase-1 merged read** (`read_intervention_telemetry` additionally merges the live run marker's target-repo keyed ledger); **cloud divergence:** the merged window also counts this environment's cloud runs via the committed `docs/telemetry/cloud/*.jsonl` segments (the same transport that carries cloud telemetry to `main`), so records and the telemetry that grades them meet at ONE `repo_root` even across container reclaim. **Both scopes are attested in the Phase-2 coverage breadcrumb** (`interventions_covered`) — which is exactly what discharges the tightened `--run-end` gate below; a target-only flush no longer satisfies it. Doc-write/commit ownership is UNCHANGED: the second scope's `docs/interventions/` (and any `docs/bugs/reconsider-*` / `canary-revert-*` seed) land in the claude-config checkout tree and are staged + committed there per the **§1c.6 flush-commit staging contract** below, riding the run's FINAL cloud push to `main` (the same transport as the efficacy verdicts) so verdicts survive container reclaim. The NON-BLOCKING `⚠ efficacy-eval failed` / `⚠ efficacy-eval --canary failed` posture is preserved for the second scope too.

   **KPI scorecard regen rides the SAME claude-config-rooted scope (efficacy-signal-integrity D4 — chosen option A; MIRRORS `/lazy-batch` §1c.6).** `docs/kpi/registry.json` lives only in claude-config, so a per-cycle regen gated on the RUN's own repo (AlgoBooth in cloud) never fires here. Immediately after the claude-config-rooted efficacy/canary pair above, in the SAME scope, run `python user/scripts/kpi-scorecard.py --repo-root <claude-config checkout>` (the same checkout root the pair above uses) — registry-gated correctly by construction. Stage + commit the regenerated `docs/kpi/SCORECARD.md` alongside the intervention records per the **§1c.6 flush-commit staging contract** below, riding the SAME final cloud push to `main` as the efficacy verdicts. NON-BLOCKING: a regen failure prints `⚠ kpi-scorecard (claude-config-scope) failed` and never blocks `--run-end`.

   **§1c.6 flush-commit staging contract (one-writer coordination — `end-of-run-flush-commit-absorbs-concurrent-writer-staged-files`; MIRRORS `/lazy-batch` §1c.6).** The efficacy/canary/KPI commit above runs in the **claude-config** checkout, where a `/harden-harness` agent may be concurrently writing. Stage + commit ONLY the specific artifacts THIS flush generated, **by explicit path**, via a **pathspec-scoped commit** — NEVER a broad `git add -A docs/*` / directory add (a blanket add absorbs a concurrent writer's staged, unrelated files — observed live: `115a991a "docs(interventions): efficacy verdicts"` swallowed a concurrent harden agent's `docs/bugs/*/SPEC.md`). The exact paths: the `docs/interventions/<id>.md` records the evaluators wrote this run (the `verdicts[].id`), the regenerated `docs/kpi/SCORECARD.md`, and any `docs/bugs/reconsider-<id>/` / `docs/bugs/canary-revert-<id>/` seed enqueued this run — committed with an explicit pathspec so only those paths are captured regardless of what else is staged: `git -C <claude-config checkout> commit -m "docs(interventions): efficacy verdicts — <terminal>" -- docs/interventions/<id>.md docs/kpi/SCORECARD.md docs/bugs/reconsider-<id>/`. The trailing `-- <paths>` commits ONLY the named paths; a concurrently-staged foreign file is left untouched, never absorbed. The sanctioned mechanism is `lazy_core.flush_commit_artifacts(repo_root, artifact_paths, message)`. If a foreign staged/dirty claude-config state is present, do NOT blanket-add to "clean it up" — commit only this flush's explicit paths and leave the foreign state for its owner.

   **`--run-end` MECHANICALLY ENFORCES the flush (efficacy-future-check-unenforced-orchestrator-prose D1; NO cloud divergence — the gate is state-dir-local).** The three scripts above each drop a run-scoped "efficacy flush ran this run" breadcrumb when they run (even on a clean no-op). `--run-end` REFUSES (exit 1, marker + registry LEFT IN PLACE) unless that breadcrumb is present — MIRRORING the unacked-hardening gate — so skipping the flush is impossible-by-accident. The gate applies to `--reason checkpoint` run-ends too. For the genuine "nothing due, deliberately skip" case, pass `--efficacy-skip-authorized` (operator-authorization ONLY, parallel to `--ack-unhardened`; printed into the run-end message under `efficacy_skip` for retro grading) — never passed autonomously. The trio + the commit stay orchestrator-owned (preserving terminal-commit ordering + the cloud final-push transport).
...
### 1c.7. Spike cycle handling (dispatch-class `sub_skill == "spike"` — cloud DEFERS the runtime proof to workstation)

If the probe returns `sub_skill == "spike"`, handle it HERE and return to Step 1a — do **NOT** fall through to the §1d real-skill dispatch. `spike` is a registered dispatch CLASS (`--emit-dispatch spike`, always Opus), the runtime-proof stage single-sourced in `~/.claude/skills/_components/spike-dispatch.md`. `compute_state` (via `lazy-state.py --cloud`) routes `sub_skill == "spike"` from two on-disk signals: a `**Spike:** required` PHASES.md line with no PASS `SPIKE_VERDICT.md` (Step 9.5), or a `BLOCKED.md` `blocker_kind: runtime-spike-verdict-pending` (Step 3, blocked resolver).

**Cloud DEFERS the runtime spike to a workstation run (the same discipline as `/investigate` and `/mcp-test`).** A runtime spike needs the live Tauri/MCP runtime, which is UNAVAILABLE in cloud — so the cloud orchestrator does **NOT** boot a runtime (there is no Step 1d.0 `--ensure-runtime` pre-boot in cloud), does **NOT** emit `--emit-dispatch spike` / dispatch a `spike` cycle cloud-side, and **NEVER** fabricates a verdict from static reasoning. Instead, RECORD the trigger and DEFER:

1. Write/append a PENDING note to the spike results doc — `SPIKE_VERDICT.md` in `{spec_path}` with `verdict: PENDING` + the proof goal + "deferred to workstation (cloud has no runtime)" (sentinel-scope write; HARD CONSTRAINT 1 holds).
2. Add one cycle-log line recording the spike trigger + defer; for the **blocked-resolver** signal, also note the defer in the `BLOCKED.md` `## Resolution`/notes (do NOT neutralize `BLOCKED.md` — the block stands until a workstation proves the verdict).
3. Do NOT dispatch. Return to Step 1a. The runtime proof is workstation-class: the feature advances only on a workstation `/lazy-batch` run that boots the runtime and produces a PASS/FAIL verdict (exactly as cloud defers `/mcp-test` via `__write_deferred_non_cloud__`). If the same spike re-routes on the next cloud probe, the in-session loop-guard surfaces it — the feature is genuinely workstation-blocked (park it / let the run reach `cloud-queue-exhausted`), never a cloud dispatch.

The runtime pre-boot, the PASS/FAIL orchestrator branching, and the provisional-eligibility carve-out (a Spike FAIL is NEVER provisionally auto-accepted, `written_by: spike` / `spike_verdict: fail` fail-closed in `lazy_core.provisional_eligibility`) are WORKSTATION concerns — see `/lazy-batch` §1c.7. The cloud orchestrator only records + defers; it never runs the proof.
...
*(Print the following block ONLY when the `gated_heads` probe key was non-empty at any point during the run — i.e., the dependency-aware skip-ahead advanced past one or more research-gated or BLOCKED queue heads. Omit entirely otherwise. **NO cloud divergence** — identical to `/lazy-batch` Step 2 gated-heads flush.)*
...
and will be the head of the next `/lazy-batch-cloud` invocation (or can be addressed manually).
...
**Identical dual-path shape to `/lazy-batch` Step 4.** Two paths gated by the `allow_research_skip` variable (set by the skill's own invocation flag, see Step 1a/1f above — a skill-level switch, distinct from any script flag).
Default path: strict halt on first `needs-research`, inline-prompt announcement, run `python3 ~/.claude/scripts/lazy-state.py --run-end`, STOP. Opt-in path: drop sentinel, advance loop, halt later on `queue-blocked-on-research`. See `~/.claude/skills/lazy-batch/SKILL.md` Step 4 for the full algorithm.
...
| Cycle-subagent execution model (workstation-recursive-subagent-dispatch, 2026-07-09) | sub-subagent dispatch PERMITTED — the dispatched skill's test-agent/impl-agent (etc.) orchestration is authoritative under the "WORKSTATION DISPATCH — LOAD-BEARING" guardrails | **inline override RETAINED** — cycle prompt carries "CLOUD OVERRIDE — LOAD-BEARING"; all sub-subagent work performed inline (`n/a (cloud-override)` retro grading unchanged) |
...
| Step 1d.0 runtime-readiness (M4 verdict consumption + guard-takeover) (long-build-and-runtime-ownership Phase 5) | **WORKSTATION-ONLY.** Step 1d.0 calls `lazy-state.py --ensure-runtime` ONCE and routes on the M4 verdict's `state ∈ {READY, STALE, HIJACKED, DEAD, BLOCKED}`: READY / STALE→READY / DEAD→READY proceed; HIJACKED/BLOCKED (foreign port never SIGKILLed; or recovery exhausted / unrecoverable `mcp_tools_present: false`) → `BLOCKED.md` `blocker_kind: mcp-runtime-unready` (verdict `terminal_blocker` body), no subagent dispatched. A guard-redirected long build (`long-build-ownership-guard.sh` bubbles `LONG-BUILD-OWNERSHIP-TAKEOVER`) is taken over by the orchestrator via `run_transient_build` + `promote_artifact_atomically`. | **NOT MIRRORED — verdict-consumption + guard-takeover are workstation-only.** Cloud's Step 9 returns `__write_deferred_non_cloud__` and NEVER routes a `mcp-test` sub_skill, so the cloud orchestrator never reaches Step 1d.0 — there is no runtime to boot or own in the cloud container (MCP validation is deferred to a workstation pass). The cloud mirror records this divergence rather than duplicating the runtime boot; the `long-build-ownership-guard.sh` hook itself is user-level and fires identically in any environment, but the orchestrator-takeover path is exercised only on the workstation, where the runtime boot + long builds run. |
...
| Budget guard — `--per-feature-cycle-cap <N>`, `--strict-research-halt`, `budget_guard` probe field, `queue-exhausted-budget-deferred` terminal, §1c.6 trip notification (point 5), gated-heads end-of-run flush (Step 2) | All present in `/lazy-batch`. | **NO cloud divergence** — budget guard + skip-ahead are environment-agnostic (lazy_core.py / lazy-state.py logic; no Tauri/MCP surface). Flags threaded through to `lazy-state.py --cloud` identically. The `budget_guard` probe field, the trip notification, the terminal handling, and the gated-heads Step 2 flush are all identical in both orchestrators. Recorded here per the coupled-pair rule; no `--cloud` delta. |
| Host-capability gate — `host-capability-saturated` terminal, `host_deferred_features` probe key, `DEFERRED_REQUIRES_HOST.md` skip (host-capability-declaration-for-gated-features Phase 6) | Present in `/lazy-batch`: the host-axis generalization of `device-queue-exhausted` — a feature declaring an absent `requires_host:` capability defers (re-openable) and the queue advances; an unregistered id fails fast to `BLOCKED.md`. | **NO cloud divergence** — the host-capability gate is workstation-axis (lazy_core.py / lazy-state.py logic). Cloud already defers ALL MCP/device work via `DEFERRED_NON_CLOUD.md`, so the host probe is **seldom reached** in cloud (cloud-saturated features hit `cloud-queue-exhausted` first), but the terminal handling, the §1c.6 halt notification, the `--terminal-reason` sanctioned set, and the `host_deferred_features` full-probe key are identical prose in both orchestrators. The `requires_host:` parse + unregistered-id fail-fast also mirror into `bug-state.py` (parity-audited). Recorded here per the coupled-pair rule; no `--cloud` delta. |
...
| Ad-hoc enqueue (Step 0.45, the skill's own invocation flag) | **MIRRORED (shared component `_components/adhoc-enqueue.md`).** Supplying that flag arms a one-time pre-loop bootstrap. It calls `lazy-state.py --enqueue-adhoc` (Bash) to prepend the referenced work to `queue.json`, seed `ADHOC_BRIEF.md`, and add a ROADMAP row; the first cycle's commit+push carries the bootstrap files. | same shared component + trigger, PLUS an immediate `git push` of the bootstrap right after enqueue (folds into guardrail B's per-batch push) so the new queue entry + brief survive container reclaim before the first cycle commits. |
...
| Per-cycle committed-doc regen — `LAZY_QUEUE.md` (mobile-queue-control) + `docs/kpi/SCORECARD.md` (friction-kpi-registry) | At each per-cycle commit point, BEFORE the `git add -A`: `python user/scripts/lazy-queue-doc.py --repo-root <repo_root>`, then `python user/scripts/kpi-scorecard.py --repo-root <repo_root>` **only when `<repo_root>/docs/kpi/registry.json` exists** (registry-gated; the KPI registry lives in claude-config — a no-op in repos without one, e.g. AlgoBooth today). Both are PURE reads, byte-stable (no wall-clock embed), orchestrator-invoked only (never on the state-script compute path), fail-open (a generator failure never blocks the commit). See the blockquote before Step 1c.5. | **same regen, same trigger, same gating** — no `--cloud` delta in the generators. Cloud nuance only: the regenerated docs ride the cycle commit that guardrails B/C push immediately, so they survive container reclaim for free. In AlgoBooth cloud runs both steps no-op today (no `docs/kpi/registry.json`; the `lazy-queue-doc.py` invocation is the operator-documented cross-repo add). Recorded here per the coupled-pair rule (this row also retro-records the `LAZY_QUEUE.md` wiring, which predates it but was never tabulated). |
| End-of-run claude-config-rooted KPI scorecard regen (efficacy-signal-integrity D4) | ADDITIONAL to the per-cycle row above: immediately after the claude-config-rooted efficacy/canary TWO-SCOPE pair at the end-of-run flush, run `python user/scripts/kpi-scorecard.py --repo-root ~/source/repos/claude-config` and commit `docs/kpi/SCORECARD.md` alongside `docs/interventions/`. Fixes the gap where the per-cycle row above never fires for a non-claude-config run (the registry lives only in claude-config). NON-BLOCKING. | **MIRRORED** — same claude-config-rooted invocation, but the SCORECARD.md commit rides the run's FINAL cloud push to `main` (same transport as the efficacy verdicts) instead of a per-cycle push, so it survives container reclaim. |
| Telemetry-ledger residency + run-end segment flush (harness-telemetry-ledger D5-B) | **CLOUD-SCOPED DIVERGENCE (partial).** Emission itself is MIRRORED — both state scripts append the same D4-B chokepoint events to `lazy-telemetry.jsonl` in the per-repo keyed state dir (script-owned, marker-gated, fail-open; the skills add nothing). Workstation residency is state-dir-only: no flush, no commit, `--run-end` output unchanged. | cloud `--run-end` additionally flushes the run's ledger segment to `docs/telemetry/cloud/<run_id colon-stripped>.jsonl` (script-owned) and surfaces `telemetry_flushed` in its JSON; the orchestrator commits+pushes that segment before the run-end PushNotification (§1c.6 item 2) so the run's telemetry survives container reclaim. The workstation trends aggregator (`pipeline_visualizer/trends.py`) reads state-dir ledgers PLUS these committed segments. |
| Intervention-efficacy end-of-run evaluation (intervention-efficacy-tracking D10-A) + TWO-SCOPE flush (interventions-telemetry-repo-scope-split-brain D2) | **MIRRORED (shared)** — the same once-per-run `python3 ~/.claude/scripts/efficacy-eval.py --repo-root . --json` invocation at the §1c.6 end-of-run flush (alongside the incident scan, BEFORE `--run-end`); record updates under `docs/interventions/` are committed docs in both environments; capture itself is script-owned inside `--apply-pseudo __mark_complete__`/`__mark_fixed__` (shared lazy_core — the skills add nothing). The flush additionally runs a SECOND `--repo-root <claude-config checkout>` evaluation (records live only in claude-config; the claude-config-rooted read merges the target-repo telemetry per Phase 1; both scopes attested in the Phase-2 breadcrumb that discharges the tightened `--run-end` gate). | same invocation; the record-update commit rides the run's FINAL cloud push to `main` (the telemetry-segment transport above) so verdicts survive container reclaim, and the evaluator's run-window accrual counts cloud runs via the committed `docs/telemetry/cloud/*.jsonl` segments merged with the state-dir ledger. **Cloud two-scope divergence:** the second scope roots at the cloud container's claude-config checkout and its merged window counts this environment's cloud runs via the committed `docs/telemetry/cloud/*.jsonl` segments. |
| Harness-change canary watch (harness-change-canary-rollback D6-A) | **MIRRORED (shared)** — the same once-per-run `python3 ~/.claude/scripts/efficacy-eval.py --canary --repo-root . --json` invocation at the §1c.6 end-of-run flush (alongside the efficacy evaluation, BEFORE `--run-end`); `canary.*` record updates + any trip `EVIDENCE.md` + `docs/bugs/canary-revert-*` seed are committed docs in both environments; the watcher is fail-open and reads THIS environment's telemetry + keyed state dir (the collector is docs+state-dir-local, so NO cloud divergence). | same invocation; the record-update + evidence commit rides the run's FINAL cloud push to `main` so canary trips survive container reclaim, and window accrual counts cloud runs via the committed `docs/telemetry/cloud/*.jsonl` segments merged with the state-dir ledger. |
...
- **Runtime-readiness routing (Step 1d.0) — NOT applicable in cloud (long-build-and-runtime-ownership Phase 5).** `/lazy-batch`'s Step 1d.0 M4-verdict consumption + guard-takeover is **workstation-only**: cloud's Step 9 returns `__write_deferred_non_cloud__` and never routes a `mcp-test` sub_skill, so the cloud orchestrator never boots a runtime, never calls `--ensure-runtime`, and never reaches the `state ∈ {HIJACKED, BLOCKED}` → `BLOCKED.md` `blocker_kind: mcp-runtime-unready` routing or the orchestrator long-build takeover. Recorded as a divergence in the **Differences from `/lazy-batch`** table; not duplicated here.
...
<!-- feature-budget-guard-and-skip-ahead Phase 4 (2026-06-20) — coupled-pair mirror note:
       - WU-1: argument-hint + Step 0 flag-parsing block gained `--per-feature-cycle-cap <N>` and
         `--strict-research-halt` entries (mirrored from lazy-batch; NO cloud divergence).
       - WU-1: §1c.6 PushNotification policy gained point 5 (budget-guard trip notification);
         §1c.6 point 2 halt list gained `queue-exhausted-budget-deferred`.
       - WU-1: terminal table (Step 1b) gained `queue-exhausted-budget-deferred`.
       - WU-1: Step 2 Final Batch Report gained "Gated heads skipped" flush block.
       - WU-1: "Differences from /lazy-batch" table gained a "Budget guard" row (NO cloud divergence).
       All items are environment-agnostic — budget guard + skip-ahead are lazy_core.py / lazy-state.py
       logic with no Tauri/MCP surface. `--cloud` is carried on all state-script calls identically. -->
