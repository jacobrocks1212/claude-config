---
name: cargo-release
description: Queue-routed AlgoBooth cargo release build (serialized through the machine-global build queue with hygiene + authoritative outcome banner). Workstation only.
argument-hint: "[pass-through args for the exec script]"
model: haiku
allowed-tools: ["Bash"]
---

# Cargo Release — Queue-Routed Rust Release Build

Run the AlgoBooth `cargo build --release` through the machine-global build queue, so it is
serialized against every other heavy build on the machine, gets the `rust-tauri` hygiene
profile (Job-Object process reap + cargo log-failure/no-output fidelity checks; no .NET
compiler recycle, no DLL sweep), and reports through the authoritative last-line outcome
banner.
...
`cargo build --release` is denied by `long-build-ownership-guard.sh` with the
...
1. Construct the command:
   ```
   cd <algobooth repo root> && powershell.exe -ExecutionPolicy Bypass -File "$HOME/.claude/scripts/build-queue.ps1" -Op cargo-release
   ```
   `-Exec` is omitted deliberately: the wrapper resolves it from this repo's ops manifest
   (`.claude/skill-config/build-queue-ops.json`, op `cargo-release`). An explicit `-Exec` would
...
3. A cold Rust release build routinely exceeds 10 minutes, so **default to the background +
   await pattern**: run the command with `run_in_background: true`. The
   `build-queue: enqueued as seq=N` line it returns is NOT an outcome — never end your turn or
   report a result on it. Follow the run to its authoritative result with the await helper
   ...
   `build-queue: seq=<N> op=cargo-release RESULT=<PASS|FAIL> (result_fidelity=...)` banner as
   its LAST stdout line, and exits with the build's exit code. On its distinct await-timeout
   exit (`124`, `result not yet present for seq=N`) the build is still running — re-run the
   helper or check `/build-queue-status`; NEVER treat a timeout as success, and do not hand-read
