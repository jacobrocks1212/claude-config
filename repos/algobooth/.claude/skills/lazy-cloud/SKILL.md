...
| `host-capability-saturated` | Host-axis generalization of `device-queue-exhausted` (`host-capability-declaration-for-gated-features`): the remaining feature(s) declare a `requires_host:` capability ABSENT on this host, deferred via `DEFERRED_REQUIRES_HOST.md` (carrying the missing ids). The `notify_message` names each feature + its missing id(s). Surface that a **capability-bearing host** is needed to re-open them. NO cloud divergence — same handling as `/lazy`. (Rare in cloud: cloud defers ALL MCP/device work first, so the host probe is seldom reached.) |
...
**Host-capability axis (host-capability-declaration-for-gated-features). NO cloud divergence.** A feature whose runtime validation needs an arbitrary named host capability declares it on-disk in a `requires_host:` set (ids drawn from a closed `lazy_core` registry). `lazy-state.py` probes the host (active invocation — runs the tool + checks the exit code, never `which()`/`exists()`), computes `missing = requires_host − host.present` (flat AND-set), and on a non-empty `missing` writes a re-openable `DEFERRED_REQUIRES_HOST.md` and skips — terminal `host-capability-saturated` when only capability-gated features remain (Step 2b). An unregistered id fails fast to `BLOCKED.md` (`blocker_kind: unknown-host-capability`), never a silent defer. The gate is **workstation-axis** — cloud already defers ALL MCP/device work via `DEFERRED_NON_CLOUD.md`, so the host probe is seldom reached in cloud (`cloud-queue-exhausted` fires first), but the terminal + sentinel handling is identical prose. An ungated feature (no `requires_host:`) is byte-identical to today. Same logic on `lazy-state.py --cloud` and without `--cloud`.
