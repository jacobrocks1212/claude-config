...
Audio invariants (AlgoBooth HARD requirement — applies when this cycle touches audio DSP):
  - If ANY work this cycle edits a file under `crates/audio-engine/src/`
    (voice, callback, dattorro, convolution, ...), READ
    `crates/audio-engine/INVARIANTS.md` BEFORE editing — including the ArcSwap
    Guard-across-`Arc<dyn Trait>` NO-OP invariant (a Guard obtained through a
    trait-object boundary is NOT zero-copy — use `load_full()`).
