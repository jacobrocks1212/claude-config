...
Record the OBSERVED ground truth (the actual tool calls + numbers, or the logged value) in a `## Validated Assumptions` note at the top of PHASES.md and in the affected phase's Integration Notes.
...
**When to skip (record the reason):** every load-bearing assumption is code-provable — pure logic, types, config, UI layout, or a behavior-preserving refactor with snapshot/golden coverage — and no boundary/runtime-observable behavior is in play. Note the skip in the plan ("no runtime-coupled assumptions; phases are pure <logic/refactor>").
