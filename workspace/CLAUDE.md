...
> **Per-machine variants:** this is the work-laptop/default workspace doc. Other machines project their own copy via `Machine = '<hostname>'`-keyed entries in `claude-config/manifest.psd1` (e.g. `workspace/CLAUDE.DESKTOP-GHTC5K6.md` on the personal workstation).
...
### Personal
- **claude-config/** — Canonical Claude Code configuration + the autonomous-pipeline harness (see below).
- **AlgoBooth** (Tauri/Strudel DJ app) is developed via **cloud sessions only** — the live repo was deleted from this machine (manifest.psd1 documents this). Other former personal repos (maestro, work-dashboard, scene-remixer, housing-locator, semantic-docs, zen-mcp-server) are no longer checked out locally; the work-logging plugin lives in `claude-config/user/plugins/local-tools/plugins/work-logging-plugin`.

## Platform Notes

This is a **Windows 11** development machine:
