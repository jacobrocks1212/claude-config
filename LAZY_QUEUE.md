...
| # | item | state | sev |
|---|------|-------|------|
| 1 | [run-end-gate-refusals-no-telemetry-event](docs/bugs/_archive/run-end-gate-refusals-no-telemetry-event/SPEC.md) | Complete | low |
| | status: Complete · phase 1/1 · next: done · The state scripts' `--run-end` gates refuse (exit 1, marker kept) — unacked-hardening-debt, the new efficacy-flush-missing gate, and checkpoint-authorization — WITHOUT emitting a telemetry event, so those refusals are invisible to the efficacy loop that measures harness health. | | |
